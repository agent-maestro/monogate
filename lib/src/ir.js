...
/**
 * Check that every DAG node has a direct lowering rule in the declared
 * JavaScript/Python sketches. This is structural contract evidence, not a
 * theorem about host-language floating point behavior.
 *
 * @param {string|object} expressionOrDag
 * @returns {object}
 */
export function checkStructuralLowering(expressionOrDag) {
  ...
  const ok = rows.every((row) => row.structural_lowering_verified);
  ...
  return {
    ...
    structural_lowering_verified: ok,
    rows,
    boundaries: {
...
export function certifyLowering(expressionOrDag, options = {}) {
  ...
  const structural = checkStructuralLowering(dag);
...
/**
 * Return the public-safe research status vocabulary used by EML IR packets.
 *
 * @returns {object}
 */
...
/**
 * Build the top-level shareable research evidence packet:
 * expression -> DAG -> replay -> lowering -> structural/sample checks -> status.
 *
 * @param {string|object} expressionOrDag
 * @param {{ samples?: Array<Record<string, number>>, tolerance?: number }} options
 * @returns {object}
...
/**
 * Validate the top-level evidence packet gates.
 *
 * @param {object} packet
