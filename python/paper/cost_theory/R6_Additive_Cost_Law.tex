...
\end{tabular}
\end{center}
The unified addition cost is 2 nodes via the construction
$\LEdiv(x,\DEML(y,1)) = x + y$ (ADD-T1, SuperBEST v5).
The former positive/general domain split (3n vs.\ 11n) is eliminated:
no absolute-value sub-routine is required for the general case.
\end{definition}
...
\texttt{add} at cost $c_{\mathrm{add}} = 2$ (SuperBEST v5, ADD-T1;
was 11 in v4) because field components and velocities can be negative.
...
\begin{align*}
  ...
  = 2 + 2 + 2 + 2 + 2 = 10.
\quad(\text{v5; was } 2+2+2+11+2=19 \text{ in v4})
\end{align*}
...
\texttt{add} nodes ($c_{\mathrm{add}} = 2$ each in v5):
\[
  \NC(\mathbf{F})
  = \NC(F_x) + \NC(F_y) + \NC(F_z) + c_{\mathrm{add}} + c_{\mathrm{add}}
  = 10 + 10 + 10 + 2 + 2
  \;=\; 34\mathrm{n}.
\quad(\text{v5; was } 57\mathrm{n} \text{ in v4})
\]
The observed cost 34n is thus the exact sum predicted by the Additive Cost Law:
three 10n branches combined via two general-domain \texttt{add} connectors.
The v4 cost was 57n (19n per branch + 2$\times$11n connecting adds).
ADD-T1 saves $(19-10)\times 3 + (11-2)\times 2 = 27 + 18 = 45$n vs.\ v4.
\end{example}

\begin{remark}[add cost governs the budget --- v4 vs.\ v5]
\label{rem:add11}
In v4, $c_{\mathrm{add}} = 11$ dominated signed sums.  The move from positive
to general domain cost $(11-3) \times 5 = 40$n extra (five add nodes).
In v5 (ADD-T1), $c_{\mathrm{add}} = 2$ for all real inputs: the domain
distinction is gone.  The Lorentz force drops from 57n (v4) to 34n (v5).
The Additive Cost Law remains valid --- it now reads off lower costs directly
from the same independence structure.
\end{remark}
...
Each $d_i$ branch contains \texttt{ln}, \texttt{div}, \texttt{add} (2n, v5),
\texttt{mul}, and \texttt{pow} nodes, contributing approximately 8n each (was $\approx$17n in v4 when \texttt{add}(general)=11n).
...
Ex.\ \ref{ex:lorentz}
  & Lorentz force 34n (v5) $= 3 \times 10\mathrm{n}$ branches
    $+ 2 \times 2\mathrm{n}$ add connectors; was 57n in v4 (ADD-T1 saves 23n). \\[4pt]
Ex.\ \ref{ex:bs}
