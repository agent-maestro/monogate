...
Addition $x + y$ (pos.\ domain) & \texttt{add} & 3  & both operands $\geq 0$ \\
...
\[
  \NC(E) \;=\; \sum_{i=1}^{\NOp(E)} c(\mathrm{op}_i)
          ...
          \;=\; 11 \cdot \NOp(E).
\]
The inequality $c(\mathrm{op}_i) \leq c_{\max} = 11$ holds for every
$\mathrm{op}_i \in \mathcal{F}_{16}$ by inspection of
Table~\ref{tab:superbest3}.  This establishes
$\Cost(E) \leq \NC(E) \leq 11 \cdot \NOp(E)$.
...
\]
The implicit constant is at most $c_{\max} = 11$.
\end{corollary}

\begin{proof}
By part~(i) of Theorem~\ref{thm:T40},
$\Cost(E) \leq 11 \cdot \NOp(E)$, which is the definition of
$O(\NOp(E))$ with constant $11$.
\end{proof}
...
\begin{enumerate}
  \item[\textup{(a)}] The leading constant in Corollary~\ref{cor:ON} is at most
        $c_{\max} = 11$ (attained by expressions consisting entirely of general
        \texttt{add} nodes).
  \item[\textup{(b)}] The leading constant is at least $c_{\min} = 1$ (attained
        ...
        \texttt{div} nodes).
  \item[\textup{(c)}] For expressions in the \emph{positive domain}
        (all operands guaranteed non-negative), the \texttt{add} cost is $3$,
        so the leading constant is at most $3$.
\end{enumerate}
\end{corollary}

\begin{proof}
(a) and (b) follow directly from the bound in part~(i) and the values
$c_{\max} = 11$, $c_{\min} = 1$ in Table~\ref{tab:superbest3}.
For (c), replace $c_{\max}$ with $c_{\max}^{+} = \max(3, 3, 2, 2, 2, 1, 1, 1) = 3$
when restricted to positive-domain operations where \texttt{add} costs $3$.
\end{proof}
...
per-term cost and the aggregation cost, and grows at most as
$\alpha_0 + 11$ in the general domain.
\end{remark}

\subsection{Summation Scaling Instances}

We instantiate Theorem~\ref{thm:T40b} for four standard scientific
summation formulas using the positive-domain cost $c_{\mathrm{add}} = 3$
throughout.
...
\midrule
Softmax $\sum e^{x_i}$               & 1 & 3 & $4N - 3$  & 4  \\
Shannon entropy $-\sum p_i \ln p_i$  & 3 & 3 & $6N - 3$  & 6  \\
Taylor series $\sum a_n x^n$         & 5 & 3 & $8N - 3$  & 8  \\
PK multi-compartment $\sum A_i e^{-\lambda_i t}$
                                     & 7 & 3 & $10N - 3$ & 10 \\
Worst-case (general add)             & $\alpha_0$ & 11 & $(\alpha_0+11)N - 11$ & $\alpha_0 + 11$ \\
\bottomrule
\end{tabular}
\end{table}

The leading coefficients form an arithmetic progression $4, 6, 8, 10, \ldots$
