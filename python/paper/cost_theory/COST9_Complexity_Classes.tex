...
compartments, softmax classes, etc.), examine the addition cost (2n unified in SuperBEST v5 via ADD-T1;
formerly 3n positive-domain vs.\ 11n general-domain), and verify the
...
Addition $x+y$       & \texttt{add}   & 2 (all real $x,y$; ADD-T1, SuperBEST v5) \\
\bottomrule
\end{tabular}
\end{table}

\noindent
In SuperBEST v5 (ADD-T1), $c_{\mathrm{add}} = 2$ for all real inputs:
$\LEdiv(x,\DEML(y,1)) = x+y$ in 2 nodes regardless of operand sign.
The positive/general split ($c_{\mathrm{add}} \in \{3, 11\}$) is eliminated.
Below, historical references to $c_{\mathrm{add}} = 3$ or $11$ reflect v3/v4
analysis; all current scaling laws use $c_{\mathrm{add}} = 2$.
...
fixed SuperBEST cost $\alpha_0$, then in SuperBEST v5 (all domains, ADD-T1):
\[
  \Cost = \alpha_0 \cdot N + 2(N-1) = (\alpha_0 + 2)N - 2.
\]
Historical v4 formulas: positive domain (3n): $(\alpha_0+3)N-3$;
general domain (11n): $(\alpha_0+11)N-11$.
All three are O(N) in $N$ with leading coefficient $(\alpha_0 + c_{\mathrm{add}})$.
\end{proposition}
...
\end{itemize}
In SuperBEST v5 (ADD-T1), signed additions cost $c_{\mathrm{add}} = 2$,
the same as subtraction.  The distinction between signed and unsigned
sums is no longer a cost driver.
\end{remark}

\begin{remark}[Subtraction vs.\ general addition --- v5 update]
In SuperBEST v3/v4, subtraction $x - y$ costs 2n while general-domain addition
costs 11n, making sub-vs-add rearrangement highly beneficial.
In SuperBEST v5 (ADD-T1), $c_{\mathrm{add}} = c_{\mathrm{sub}} = 2$: both cost
the same.  The optimization of rearranging sums as differences is no longer
relevant for cost reduction; choose the form that is most natural for the
formula.
\end{remark}
...
\begin{enumerate}
  ...
  \item \textbf{Domain distinction is obsolete in SuperBEST v5.}
        ADD-T1 proves $c_{\mathrm{add}} = 2$ for all real inputs.
        The positive-domain / general-domain split ($c \in \{3, 11\}$)
        is replaced by a single unified cost.  All scaling laws update
        from $(\alpha_0 + 3)N - 3$ to $(\alpha_0 + 2)N - 2$.
\end{enumerate}
