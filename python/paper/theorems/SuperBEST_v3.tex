...
\small monogate research \quad \texttt{contact@monogate.dev}}
\date{April 2026}
...
\begin{itemize}
  ...
  \item \textbf{R16-C1}: $\operatorname{recip}(x)=1/x$ requires only 1 node in
        $\mathcal{F}_{16}$ (down from 2; operator: $\ELSb(0,x)=e^0/x=1/x$).
\end{itemize}
The combined effect reduces the total from 21 (v1) to 20 (v2) to 19 (v3) to
\textbf{18 nodes (v4)}, raising the global compression ratio from 71.2\% to 72.6\%
to 74.0\% to $\mathbf{1 - 18/73 = 55/73 \approx 75.3\%}$.
...
$\mathrm{Naive\text{-}total} = 73$ nodes (fixed); v4 achieves
$\mathrm{SB\text{-}total} = 18$ nodes.
\end{definition}
...
v3 & sub: $3\to2$ (T33, F16) & 19 & 74.0\% & T33 \\
v4 & recip: $2\to1$ (R16-C1, $\ELSb$) & \textbf{18} & \textbf{75.3\%} & R16-C1 \\
\bottomrule
\end{tabular}
\end{center}
...
The global minimum total over $\mathcal{F}_{16}$ is exactly 18 nodes (updated
from v3 = 19 by R16-C1: recip drops from 2 to 1 node), and
\[
  \mathrm{savings} \;=\; 1 - \tfrac{18}{73} \;=\; \tfrac{55}{73} \;\approx\; 75.34\%
  \;\approx\; 75.3\%.
\]
\end{theorem}
...
\begin{table}[h]
\centering
\caption{SuperBEST v4: Minimum node counts, optimal operators, and savings.
         Changes from v2 are marked with $\dagger$; changes from v3 are marked with $\ddagger$.}
\label{tab:v3}
\smallskip
...
$1/x$        & 5  & 2 & \textbf{1}$^\ddagger$ & $\ELSb(0,x)$   & $\mathbf{80.0\%}^\ddagger$ & R16-C1 \\
...
\midrule
\textbf{Total} & \textbf{73} & \textbf{21} & \textbf{18}$^\ddagger$
  & & $\mathbf{75.3\%}^\ddagger$ & \\
\bottomrule
\end{tabular}
...
\smallskip
\footnotesize{%
$\dagger$ Updated from v2. $\ddagger$ Updated from v3 (R16-C1: recip $2\to1$ node via $\ELSb(0,x)$).
...
R16-C1 proof: \texttt{python/paper/cost\_theory/R16\_T08\_Structural\_Proofs.tex}, Section~5.
}
\end{table}
...
\medskip
\noindent\textbf{recip(x), 1 node (R16-C1).}
$\ELSb(0,x) = e^0/x = 1/x$.
The operator $\ELSb(a,b) = e^a/b$ with $a=0$, $b=x$ gives $1/x$ directly
in a single node, with leaves $0$ and $x$ both in $\mathcal{T}$.
Lower bound: 0 nodes impossible since no terminal equals $1/x$ for all
$x > 0$.  Optimum: 1 (updated from 2 by R16-C1).
...
$1+1+2+1+2+2+3+3 = \mathbf{18}$.
Row lower bounds are independent (each operation is computed by a separate
sub-tree), so the global minimum is 18 (updated from 19 by R16-C1).
$\text{savings} = 1 - 18/73 = 55/73 \approx 75.3\%$.
\end{proof}
...
\begin{itemize}
  \item \textbf{neg} (2 nodes): 1-node lower bound certified.
  \item \textbf{recip} (1 node): 0-node lower bound trivial (non-constant);
        $\ELSb(0,x)=1/x$ achieves 1 node (R16-C1).
  \item \textbf{mul, sub} (2 nodes each): 1-node lower bound certified.
  \item \textbf{add, pow} (3 nodes each): 2-node lower bound certified.
\end{itemize}
Hence the total is bounded below by $1+1+2+1+2+2+3+3 = 18$, and the savings
are bounded above by $1 - 18/73 = 75.3\%$ within the current 8-operation,
terminal-set-$\{x,y,0,1\}$, $\mathcal{F}_{16}$ framework.
Note: recip achieving 1 node means the floor has been lowered; no further
improvement is possible for exp, ln, recip, div since all are at 1 node.
\end{proposition}
