...
%
% NOTE (2026-04-20, SuperBEST v5 / ADD-T1):
%   This file has been superseded as the authoritative T30 reference by:
%     Depth_Spectrum_Final.tex
%   That file incorporates the ADD-T1 breakthrough (add=2n for ALL real x,y)
%   from the ADD-CASCADE session (SuperBEST v5), which confirms and strengthens
%   the depth-3 ceiling: addition now has EML depth 2 (not 3).
%   All results in this file remain valid; see Depth_Spectrum_Final.tex for the
%   definitive statement with the updated depth census.
%
...
\small \texttt{contact@monogate.dev}}
\date{April 2026}
...
\noindent\textbf{SuperBEST v5 update (ADD-T1, 2026-04-20).}
Subsequent to this file, the ADD-CASCADE session established that
$\mathrm{LEDIV}(x,\mathrm{DEML}(y,1)) = x + y$ in exactly 2 nodes (depth~2) for all
$x, y \in \mathbb{R}$.  This replaces the 3-node add\_pos and 11-node add\_gen entries.
The Proposition~\ref{prop:add} below (which establishes depth $\leq 3$ for addition)
remains correct but is now superseded by depth~2 via ADD-T1.
All census entries with depth $\leq 3$ continue to hold; depth bounds are not worsened.
For the updated census see \texttt{Depth\_Spectrum\_Final.tex}.
