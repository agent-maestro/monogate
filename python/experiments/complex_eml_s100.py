"""
S100 — Write Up the Near-Miss

Produces: python/paper/near_miss_obstruction.tex
Title: "Near-misses and transcendental obstructions in EML tree closure"
4-6 pages, experimental mathematics journal style.
"""
...
PAPER_BODY = r"""% near_miss_obstruction.tex
...
if __name__ == "__main__":
    paper_dir = Path("D:/monogate/python/paper")
    paper_dir.mkdir(parents=True, exist_ok=True)
    results_dir = Path(__file__).parent.parent / "results"
    results_dir.mkdir(exist_ok=True)

    out_tex = paper_dir / "near_miss_obstruction.tex"
    out_tex.write_text(PAPER_BODY, encoding="utf-8")
    print(f"Paper written: {out_tex}")
    print(f"  Lines: {len(PAPER_BODY.splitlines())}")

    result = {
        "session": "S100",
        "title": "Near-Miss Paper Draft",
        "paper_path": str(out_tex),
        "paper_lines": len(PAPER_BODY.splitlines()),
        "sections": [
            "Abstract",
            "1. Introduction",
            "2. Depth structure (Propositions: D5 boundedness, D6 phase transition)",
            "3. Near-miss and tan(1) obstruction (Theorem: transcendental obstruction)",
            "4. Convergence and density (Conjectures: density, i as accumulation point)",
            "5. Open problems",
        ],
        "results_tiered": {
            "D5_boundedness": "PROPOSITION (computational + proof sketch)",
            "D6_phase_transition": "PROPOSITION (constructive example)",
            "transcendental_obstruction": "THEOREM (conditional on Hermite-Lindemann, proved)",
            "density": "CONJECTURE",
            "i_as_accumulation_point": "CONJECTURE",
        },
        "target_venue": "Experimental Mathematics or Journal of Symbolic Computation",
        "pi_over_tan1": PI_OVER_TAN1,
        "tan1": TAN1,
    }

    out = results_dir / "s100_paper_draft.json"
