#!/usr/bin/env python3
"""FEF-P52 selected C ternary branch lowering gate."""
...
SCHEMA_VERSION = "monogate.fef_p52_c_ternary_branch_lowering_gate.v0"
PACKET_SCHEMA_VERSION = "monogate.fef_p52_c_ternary_reingest_packet.v0"
...
CASE_ID = "c_ternary_select_pos_v0"
SAMPLES = [{"args": [x], "labels": ["x"]} for x in [-2.0, -1.0e-12, 0.0, 1.0e-12, 2.0]]

CLAIM_FLAGS = {
    "selected_c_ternary_lowering_claim": False,
...
NON_CLAIMS = [
    "FEF-P52 records a selected C ternary lowering and re-ingest gate only.",
    "FEF-P52 does not claim general C branch/control-flow support.",
    "FEF-P52 does not claim C if-statement support.",
    "FEF-P52 does not claim Rust if support.",
    "FEF-P52 does not claim full non-generated source roundtrip.",
    "FEF-P52 does not claim full arbitrary C/Rust source roundtrip.",
    "FEF-P52 does not record reviewer approval or rejection.",
    "FEF-P52 does not publish a package.",
    "FEF-P52 does not enable checkout or commerce.",
    "FEF-P52 does not claim public readiness.",
    "FEF-P52 does not claim compiler correctness or formal semantic equivalence.",
    "FEF-P52 does not claim runtime performance.",
    "FEF-P52 does not claim all-free-target runtime execution or all-free-target roundtrip.",
    "FEF-P52 does not claim Verilog, Lean proof, zkproof, silicon, hardware, Pro-target, production, or all-target readiness.",
]
...
def compare_target(target: str, eml_path: Path, source_eml: str, tmp_path: Path) -> dict[str, Any]:
    ...
    if target == "c":
        generated_values = call_generated_c(generated_path, "select_pos", SAMPLES, tmp_path)
    elif target == "rust":
        generated_values = call_generated_rust(generated_path, "select_pos", SAMPLES, tmp_path)
    else:
        raise ValueError(f"unsupported FEF-P52 target: {target}")
    ...
    packet = {
        ...
        "frames": frames,
        "evidenceKind": "selected_c_ternary_source_derived_generated_target_reingest",
...
def build_payload() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="fef_p52_c_ternary_") as tmp:
        tmp_path = Path(tmp)
    ...
    payload = {
        ...
        "date": DATE,
        "status": STATUS,
        "artifactId": "fef-p52-c-ternary-branch-lowering-gate",
        ...
        "sourceFixture": {
            ...
            "sourceEmlContains": ["fn step01", "clamp(", "step01(", "select_pos"],
        },
        ...
        "releaseGates": [
            {"id": "selected_c_ternary_lowering", "status": "pass"},
            {"id": "selected_c_ternary_generated_c_reingest", "status": "pass"},
            {"id": "selected_c_ternary_generated_rust_reingest", "status": "pass"},
        ...
        "nextMilestones": [
            "Add C if-statement lowering or Rust if-expression lowering under a separate gate.",
            "Regenerate a branch blocker inventory after the next frontend slice.",
            "Record private reviewer response over P47-P52 before changing release posture.",
        ],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def summarize(packets: list[dict[str, Any]], p51_summary: dict[str, Any]) -> dict[str, Any]:
    return {
        "selectedCTernaryLoweringPass": True,
        "sourceCaseCount": 1,
...
def build_evidence_packet(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "reviewHighlights": [
            ...
            "C if-statements and Rust if remain blocked.",
        ],
        "validationCommands": [
            "python python/scripts/fef_p52_c_ternary_branch_lowering_gate.py --build --strict",
            "python -m pytest -q python/tests/test_fef_p52_c_ternary_branch_lowering_gate.py",
        ],
    }
...
def build_command_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "schemaVersion": "monogate.command_feed.fef_p52_c_ternary_branch_lowering_gate.v0",
        "date": DATE,
        "title": "FEF-P52 C Ternary Branch Lowering Gate",
        "status": payload["status"],
        "decision": payload["decision"],
        "summary": payload["summary"],
        "topFollowup": "Implement C if-statement lowering or Rust if-expression lowering under a separate gate.",
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
    ...
    return "\n".join(
        [
            ...
            *rows,
            ...
            "## Summary",
            "",
            f"- Selected C ternary lowering pass: `{summary['selectedCTernaryLoweringPass']}`",
...
def validate_packet(packet: dict[str, Any]) -> None:
    if packet["schemaVersion"] != PACKET_SCHEMA_VERSION:
        raise ValueError("invalid FEF-P52 packet schema")
    if packet["packetType"] != "fef_p52_c_ternary_reingest_packet_v0":
        raise ValueError("invalid FEF-P52 packet type")
    if packet["sourceLanguage"] != "c":
        raise ValueError("FEF-P52 source language must be C")
    if packet["sourceFeature"] != "ternary_select":
        raise ValueError("FEF-P52 source feature must be ternary_select")
    if packet["generatedTargetLanguage"] not in {"c", "rust"}:
        raise ValueError("FEF-P52 generated target must be C or Rust")
    if packet["recompiledTargetLanguage"] != "python":
        raise ValueError("FEF-P52 recompiled target must be Python")
    if packet["sourceFunctionCount"] != 2:
        raise ValueError("FEF-P52 source EML should include select_pos and step01")
    if packet["reingestedFunctionCount"] != 2:
        raise ValueError("FEF-P52 re-ingested target should include select_pos and step01")
    ...
    for frame in packet["frames"]:
        ...
        if frame["absErrorGeneratedVsExpectedReference"] > ATOL:
            raise ValueError(f"{packet['caseId']} frame missed ternary reference")
...
def validate_payload(payload: dict[str, Any]) -> None:
    if payload["schemaVersion"] != SCHEMA_VERSION:
        raise ValueError("invalid FEF-P52 schema")
    if payload["status"] != STATUS:
        raise ValueError("invalid FEF-P52 status")
    summary = payload["summary"]
    if summary["selectedCTernaryLoweringPass"] is not True:
        raise ValueError("selected C ternary lowering should pass")
    if summary["sourceCaseCount"] != 1:
        raise ValueError("expected one selected C ternary source case")
    if summary["packetCount"] != 2:
        raise ValueError("expected generated C and Rust re-ingest packets")
    if summary["passCount"] != 2:
        raise ValueError("all P52 packets must pass")
    if summary["packetSampleCount"] != 10:
        raise ValueError("unexpected P52 packet sample count")
    ...
    if summary["cTernaryBlockerClosed"] is not True:
        raise ValueError("C ternary blocker should be closed")
    for key in [
...
def build_outputs(
    ...
    result_path = out_dir / f"fef_p52_c_ternary_branch_lowering_gate_{STAMP}.json"
    report_path = report_dir / f"fef_p52_c_ternary_branch_lowering_gate_{STAMP}.md"
    evidence_path = evidence_dir / "fef_p52_c_ternary_branch_lowering_gate.json"
    feed_path = command_feed_dir / f"fef_p52_c_ternary_branch_lowering_gate_feed_{STAMP}.json"
...
def main() -> int:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/fef_p52_c_ternary_branch_lowering_gate")
    parser.add_argument("--packet-dir", type=Path, default=ROOT / "python/results/fef_p52_c_ternary_reingest_packets")
    ...
    print("FEF_P52_C_TERNARY_BRANCH_LOWERING_GATE_OK")
