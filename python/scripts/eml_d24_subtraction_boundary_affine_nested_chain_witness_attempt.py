#!/usr/bin/env python3
"""EML-D24 subtraction-boundary affine-nested chain MachLib witness attempt."""
...
from scripts import eml_d23_nested_family_next_branch_decision as d23  # noqa: E402

DATE = "2026-06-01"
STAMP = DATE.replace("-", "_")
SCHEMA_VERSION = "monogate.eml_subtraction_boundary_affine_nested_chain_witness_attempt.v0"
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    selector = d23.build_payload(atlas_gate_path)
    d23.validate_payload(selector)
    atlas_path = MACHLIB_ROOT / "MachLib" / "EMLAtlasWitness.lean"
    selected = {
        "name": "subtraction_boundary_affine_nested_chain_witness",
        "machlibName": "MachLib.Real.subtraction_boundary_affine_nested_chain_witness",
        "path": "../machlib/foundations/MachLib/EMLAtlasWitness.lean",
        "statement": "eml (log (x + y)) (exp (eml (log z) (exp y))) = (x + y) - (z - y) under 0 < x + y and 0 < z",
        "present": file_contains(atlas_path, "theorem subtraction_boundary_affine_nested_chain_witness"),
    ...
    summary = {
        ...
        "deeperChainPreviouslyParked": selector["summary"]["deeperChainPreviouslyParked"],
    ...
    return {
        ...
        "attemptType": "eml_subtraction_boundary_affine_nested_chain_witness_attempt_v0",
        "artifactId": "eml-d24-subtraction-boundary-affine-nested-chain-witness-attempt",
        "status": STATUS,
        "decision": "subtraction_boundary_affine_nested_chain_witness_checked_in_machlib",
        "date": DATE,
        "sourceBranchDecision": selector["artifactId"],
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceBranchDecision"] != "eml-d23-nested-family-next-branch-decision":
        raise ValueError("D24 must consume D23")
    ...
    if summary["leanTypecheckPerformed"] is not True or summary["lakeBuildPassed"] is not True:
        raise ValueError("D24 requires observed Lake build pass")
    ...
    if summary["negativeControlBlockedBySelector"] is not True:
        raise ValueError("D24 must preserve negative-control block")
    if summary["twoStageWitnessRecordedPrivately"] is not True:
        raise ValueError("D24 must consume the private two-stage surface review chain")
    if summary["deeperChainPreviouslyParked"] is not True:
        raise ValueError("D24 must preserve deeper-chain parked status")
    for key in [
        "copyReviewStarted",
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "artifactType": "eml_subtraction_boundary_affine_nested_chain_witness_attempt",
        "validationStatus": "pass",
        "semanticStrength": "scoped_machlib_affine_nested_subtraction_identity_witness_checked",
        "source": f"python/results/eml_d24_subtraction_boundary_affine_nested_chain_witness_attempt/eml_d24_subtraction_boundary_affine_nested_chain_witness_attempt_{STAMP}.json",
        "summary": payload["summary"],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "feedId": "eml_d24_subtraction_boundary_affine_nested_chain_witness_attempt_feed",
        "date": DATE,
        "status": payload["status"],
        "selectedWitnessName": payload["summary"]["selectedWitnessName"],
        "nextAction": "Surface D24 privately before any public copy, broader nested-family claim, or three-stage proof attempt.",
        "claimFlags": dict(CLAIM_FLAGS),
    }
...
def build_outputs(out_dir: Path, report_dir: Path, evidence_dir: Path, command_feed_dir: Path, atlas_gate_path: Path) -> dict[str, Any]:
    ...
    result_path = out_dir / f"eml_d24_subtraction_boundary_affine_nested_chain_witness_attempt_{STAMP}.json"
    report_path = report_dir / f"eml_d24_subtraction_boundary_affine_nested_chain_witness_attempt_{STAMP}.md"
    evidence_path = evidence_dir / "eml_d24_subtraction_boundary_affine_nested_chain_witness_attempt.json"
    feed_path = command_feed_dir / f"eml_d24_subtraction_boundary_affine_nested_chain_witness_attempt_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/eml_d24_subtraction_boundary_affine_nested_chain_witness_attempt")
...
def main() -> int:
    ...
    print("EML_D24_SUBTRACTION_BOUNDARY_AFFINE_NESTED_CHAIN_WITNESS_ATTEMPT_OK")
    return 0
