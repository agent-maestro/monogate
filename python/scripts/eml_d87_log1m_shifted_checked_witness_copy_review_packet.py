...
from scripts import eml_d86_log1m_shifted_surface_next_selector as d86  # noqa: E402

DATE = "2026-06-04"
STAMP = DATE.replace("-", "_")
SCHEMA_VERSION = "monogate.eml_log1m_shifted_checked_witness_copy_review_packet.v0"
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    selector = d86.build_payload(atlas_gate_path)
    d86.validate_payload(selector)
    row_caveats = [
        "Always describe this as one scoped guarded checked witness, not a log1m or logarithm theory.",
        ...
        "Keep D83 boundary controls visible: x = 1, x > 1, and unguarded statements remain blocked.",
        "Keep the D83/D82 duplicate-log1p block visible; the checked log1p-shifted lane is not fresh work.",
    ...
    summary = {
        ...
        "publicReady": False,
        "nextAction": "EML-D88 choose log1m-shifted branch pause/freeze, next bounded branch, private reviewer response intake, or human-approved public copy gate.",
    ...
    return {
        ...
        "reviewType": "eml_log1m_shifted_checked_witness_copy_review_packet_v0",
        "artifactId": "eml-d87-log1m-shifted-checked-witness-copy-review-packet",
        "status": STATUS,
        "decision": "log1m_shifted_checked_witness_copy_review_private_only_public_copy_held",
        "date": DATE,
        "sourceSelector": selector["artifactId"],
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceSelector"] != "eml-d86-log1m-shifted-surface-next-selector":
        raise ValueError("D87 must consume D86")
    ...
    if summary["guardCount"] != 1:
        raise ValueError("log1m-shifted copy review should preserve one guard")
    ...
    if summary["blockedGlobalPhraseCount"] != 13:
        raise ValueError("unexpected blocked phrase count")
    if summary["rowRequiredCaveatCount"] != 7 or summary["rowBlockedPhraseCount"] != 12:
    ...
    if summary["nextAction"] != (
        "EML-D88 choose log1m-shifted branch pause/freeze, next bounded branch, private reviewer response intake, or human-approved public copy gate."
    ):
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "artifactType": "eml_log1m_shifted_checked_witness_copy_review_packet",
        "validationStatus": "pass",
        "semanticStrength": "private_log1m_shifted_checked_witness_copy_review_public_copy_held",
        "source": f"python/results/eml_d87_log1m_shifted_checked_witness_copy_review_packet/eml_d87_log1m_shifted_checked_witness_copy_review_packet_{STAMP}.json",
        "summary": payload["summary"],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "feedId": "eml_d87_log1m_shifted_checked_witness_copy_review_packet_feed",
        "date": DATE,
...
def build_outputs(
    ...
    result_path = out_dir / f"eml_d87_log1m_shifted_checked_witness_copy_review_packet_{STAMP}.json"
    report_path = report_dir / f"eml_d87_log1m_shifted_checked_witness_copy_review_packet_{STAMP}.md"
    evidence_path = evidence_dir / "eml_d87_log1m_shifted_checked_witness_copy_review_packet.json"
    feed_path = command_feed_dir / f"eml_d87_log1m_shifted_checked_witness_copy_review_packet_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=ROOT / "python/results/eml_d87_log1m_shifted_checked_witness_copy_review_packet",
    )
...
def main() -> int:
    ...
    print("EML_D87_LOG1M_SHIFTED_CHECKED_WITNESS_COPY_REVIEW_PACKET_OK")
    return 0
