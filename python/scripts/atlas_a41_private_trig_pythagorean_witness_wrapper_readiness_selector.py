...
from scripts import atlas_a40_private_trig_pythagorean_theorem_lookup_gate as a40  # noqa: E402
...
SCHEMA_VERSION = "monogate.private_trig_pythagorean_witness_wrapper_readiness_selector.v0"
STATUS = "ATLAS_A41_PRIVATE_TRIG_PYTHAGOREAN_WITNESS_WRAPPER_READINESS_SELECTOR_PASS"
ARTIFACT_ID = "atlas-a41-private-trig-pythagorean-witness-wrapper-readiness-selector"
...
NEXT_RECOMMENDED_ARTIFACT = "ATLAS-A42 private trig pythagorean wrapper-or-alias attempt gate"
...
def build_readiness_options(source: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        {
            "optionId": "park_trig_pythagorean_after_lookup",
            ...
            "readinessReasons": [
                "The candidate has plausible local surfaces but Atlas consolidation may be higher leverage than another attempt gate.",
            ],
            "futureGateRequirements": [],
            "sourceSignals": None,
            "claimFlags": dict(CLAIM_FLAGS),
        },
        {
            ...
            "nextArtifact": "Future private trig pythagorean scope wording review",
            "readinessReasons": [
...
def build_payload(atlas_gate_path: Path, machlib_root: Path) -> dict[str, Any]:
    source = a40.build_payload(atlas_gate_path, machlib_root)
    a40.validate_payload(source)
    ...
    return build_claim_flagged_packet(
        ...
        artifact_type="private_trig_pythagorean_witness_wrapper_readiness_selector",
        status=STATUS,
        date=DATE,
        summary=summary,
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceArtifact"] != "atlas-a40-private-trig-pythagorean-theorem-lookup-gate":
        raise ValueError("ATLAS-A41 must consume ATLAS-A40")
    if summary["sourceReviewedDirectionId"] != SOURCE_DIRECTION_ID:
        raise ValueError("A41 must consume trig direction")
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return build_evidence_packet(
        ...
        source=f"python/results/atlas_a41_private_trig_pythagorean_witness_wrapper_readiness_selector/atlas_a41_private_trig_pythagorean_witness_wrapper_readiness_selector_{STAMP}.json",
        summary=payload["summary"],
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return build_command_feed(
        feed_id="atlas_a41_private_trig_pythagorean_witness_wrapper_readiness_selector_feed",
        date=DATE,
        status=payload["status"],
        next_action="Run ATLAS-A42 as a private trig pythagorean wrapper-or-alias attempt gate; do not edit MachLib, run Lean, or claim validity from A41.",
        claim_flags=payload["claimFlags"],
        fields={
...
def build_report(payload: dict[str, Any]) -> str:
    ...
    return render_markdown_report(
        title="ATLAS-A41 Private Trig Pythagorean Witness-Wrapper Readiness Selector",
        status=payload["status"],
...
def build_outputs(
    ...
    result_path = out_dir / f"atlas_a41_private_trig_pythagorean_witness_wrapper_readiness_selector_{STAMP}.json"
    report_path = report_dir / f"atlas_a41_private_trig_pythagorean_witness_wrapper_readiness_selector_{STAMP}.md"
    evidence_path = evidence_dir / "atlas_a41_private_trig_pythagorean_witness_wrapper_readiness_selector.json"
    feed_path = command_feed_dir / f"atlas_a41_private_trig_pythagorean_witness_wrapper_readiness_selector_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=ROOT / "python/results/atlas_a41_private_trig_pythagorean_witness_wrapper_readiness_selector",
    )
...
def main() -> int:
    ...
    print("ATLAS_A41_PRIVATE_TRIG_PYTHAGOREAN_WITNESS_WRAPPER_READINESS_SELECTOR_OK")
    return 0
