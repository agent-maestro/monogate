#!/usr/bin/env python3
"""EML-D67 probability logit checked-witness private surface review."""
...
from scripts import eml_d66_probability_logit_boundary_coordinate_witness_attempt as d66  # noqa: E402

DATE = "2026-06-03"
STAMP = DATE.replace("-", "_")
SCHEMA_VERSION = "monogate.eml_probability_logit_witness_surface_review.v0"
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    witness = d66.build_payload(atlas_gate_path)
    d66.validate_payload(witness)
    surface_rows = [
        surface_row(
            "machlib_witness_index_probability_logit_boundary",
            "machlib_private_index",
            "checked_witness_recorded_private",
            "scoped_guarded_probability_logit_coordinate_checked",
            "record_as_private_checked_witness",
            [
                "D66 checks MachLib.Real.probability_logit_boundary_coordinate_witness.",
                "The checked statement is guarded by 0 < p and p < 1.",
                "The witness is scoped to one probability-logit coordinate and does not create a broad probability or logit theory.",
            ],
            ["public readiness", "theorem discovery", "broad probability/logit theory"],
        ),
        surface_row(
            "probability_logit_guard_boundary",
            "candidate_boundary",
            "guarded_domain_boundary_required",
            "d65_negative_controls_and_blockers_preserved",
            "keep_two_probability_interval_guards_and_boundary_controls",
            [
                "D65 records two guards, two derived domain obligations, four negative controls, and four blockers.",
                "D66 preserves the guard shape 0 < p and p < 1.",
                "Boundary cases p = 0, p = 1, and unguarded statements remain blocked.",
            ],
            ["unguarded probability coordinate", "p = 0 boundary", "p = 1 boundary"],
        ),
        surface_row(
            "probability_logit_runtime_control_guardrail",
            ...
            [
                "D65 and D66 keep protected_log_and_log1p_remain_runtime_controls.",
                "The identity may support proof/teaching shape but does not replace log, log1p, or logit runtime behavior.",
                "Future copy must block runtime advantage and logarithmic replacement language.",
            ],
            ["log replacement", "log1p replacement", "logit runtime replacement", "runtime advantage"],
        ),
        surface_row(
            "advantage_lab_probability_logit_boundary",
            "advantage_lab",
            "runtime_control_remains_protected_log_and_log1p",
            "scoped_probability_logit_coordinate_witness_available",
        ...
        surface_row(
            "public_atlas_probability_logit_boundary",
            "public_surface",
            "held_private",
            "checked_machlib_witness_available_private",
            "require_human_copy_review_before_public_change",
            [
                "D66 proof status is real but public wording has not been reviewed.",
                "No monogate.org or monogate.dev public route is changed by D67.",
    ...
    summary = {
        ...
        "publicReady": False,
        "nextAction": "EML-D68 choose probability-logit checked-witness copy review, next bounded branch, or human-approved public copy gate without public promotion.",
        "claimFlagsAllFalse": all(value is False for value in CLAIM_FLAGS.values())
        and all(all(value is False for value in row["claimFlags"].values()) for row in surface_rows),
    }
    return {
        "schemaVersion": SCHEMA_VERSION,
        "reviewType": "eml_probability_logit_witness_surface_review_v0",
        "artifactId": "eml-d67-probability-logit-witness-surface-review",
        "status": STATUS,
        "decision": "surface_checked_probability_logit_witness_as_private_review_evidence_only",
        "date": DATE,
        "sourceWitnessAttempt": witness["artifactId"],
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceWitnessAttempt"] != "eml-d66-probability-logit-boundary-coordinate-witness-attempt":
        raise ValueError("D67 must consume D66")
    ...
    if summary["guardCount"] != 2:
        raise ValueError("probability-logit witness should preserve two guards")
    if summary["sourceDerivedDomainObligationCount"] != 2:
        raise ValueError("D65 derived domain obligation count drift")
    if summary["sourceNegativeControlCount"] != 4 or summary["sourceBlockerCount"] != 4:
        raise ValueError("D65 negative control/blocker counts drift")
    for key in [
    ...
    if summary["nextAction"] != "EML-D68 choose probability-logit checked-witness copy review, next bounded branch, or human-approved public copy gate without public promotion.":
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "artifactType": "eml_probability_logit_witness_surface_review",
        "validationStatus": "pass",
        "semanticStrength": "checked_probability_logit_witness_private_surface_review_no_public_update",
        "source": f"python/results/eml_d67_probability_logit_witness_surface_review/eml_d67_probability_logit_witness_surface_review_{STAMP}.json",
        "summary": payload["summary"],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "feedId": "eml_d67_probability_logit_witness_surface_review_feed",
        "date": DATE,
...
def build_outputs(out_dir: Path, report_dir: Path, evidence_dir: Path, command_feed_dir: Path, atlas_gate_path: Path) -> dict[str, Any]:
    ...
    result_path = out_dir / f"eml_d67_probability_logit_witness_surface_review_{STAMP}.json"
    report_path = report_dir / f"eml_d67_probability_logit_witness_surface_review_{STAMP}.md"
    evidence_path = evidence_dir / "eml_d67_probability_logit_witness_surface_review.json"
    feed_path = command_feed_dir / f"eml_d67_probability_logit_witness_surface_review_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/eml_d67_probability_logit_witness_surface_review")
...
def main() -> int:
    ...
    print("EML_D67_PROBABILITY_LOGIT_WITNESS_SURFACE_REVIEW_OK")
    return 0
