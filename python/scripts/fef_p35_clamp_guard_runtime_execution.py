#!/usr/bin/env python3
"""FEF-P35 runtime execution guard for generated clamp/guard targets."""
...
SCHEMA_VERSION = "monogate.fef_p35_clamp_guard_runtime_execution.v0"
EVIDENCE_SCHEMA_VERSION = "monogate.evidence_public_packet.v0"
STATUS = "FEF_P35_CLAMP_GUARD_RUNTIME_EXECUTION_PASS"
ATOL = 1e-12

FEF_P34_PATH = ROOT / "reports/evidence_packets/fef_p34_clamp_guard_free_target_guard.json"
SOURCE_FIXTURE_ID = p34.SOURCE_FIXTURE_ID
FUNCTION_NAME = "clamp_guard_mix"
SAMPLES = [{"args": [x], "labels": ["x"]} for x in [-2.0, -1.0, -0.25, 0.0, 0.5, 1.0, 2.5]]
...
NON_CLAIMS = [
    "FEF-P35 records runtime execution for installed toolchains over one selected clamp/guard fixture.",
    "FEF-P35 does not execute all 13 free targets.",
    "FEF-P35 does not claim arbitrary branch/control-flow support.",
    "FEF-P35 does not claim all free targets are public-ready.",
    "FEF-P35 does not claim compiler correctness or formal semantic equivalence.",
    "FEF-P35 does not publish a package, enable checkout, or claim production readiness.",
    "FEF-P35 does not claim runtime performance, Verilog, silicon, hardware, Pro-target, or all-target readiness.",
]
...
def reference_values(samples: list[dict[str, Any]]) -> list[float]:
    ...
    for sample in samples:
        x = float(sample["args"][0])
        values.append(max(-1.0, min(1.0, x)))
    return values
...
def call_python(path: Path, samples: list[dict[str, Any]]) -> list[float]:
    spec = importlib.util.spec_from_file_location("fef_p35_clamp_guard_generated", path)
...
def call_c(path: Path, samples: list[dict[str, Any]], tmp_path: Path) -> list[float]:
    ...
    calls = "\n".join(
        f'    printf("%.17g\\n", clamp_guard_mix({c_float(sample["args"][0])}));'
        for sample in samples
    )
    runner.write_text(
        "#include <stdio.h>\n"
...
def call_cpp(path: Path, samples: list[dict[str, Any]], tmp_path: Path) -> list[float]:
    ...
    header = tmp_path / "clamp_guard_mix.hpp"
    header.write_text(path.read_text(encoding="utf-8"), encoding="utf-8")
    runner = tmp_path / "cpp_runner.cpp"
    binary = tmp_path / "cpp_runner"
    calls = "\n".join(
        f'    std::cout << std::setprecision(17) << forge::clamp_guard_mix::clamp_guard_mix({c_float(sample["args"][0])}) << "\\n";'
        for sample in samples
    )
    runner.write_text(
...
def call_rust(path: Path, samples: list[dict[str, Any]], tmp_path: Path) -> list[float]:
    ...
    calls = "\n".join(
        f'    println!("{{:.17e}}", clamp_guard_mix({rust_float(sample["args"][0])}));'
        for sample in samples
    )
    runner.write_text(
...
def call_java(path: Path, samples: list[dict[str, Any]], tmp_path: Path) -> list[float]:
    ...
    generated = tmp_path / "ClampGuardMix.java"
    generated.write_text(path.read_text(encoding="utf-8"), encoding="utf-8")
    runner = tmp_path / "ClampGuardMixRunner.java"
    calls = "\n".join(
        f'        System.out.printf("%.17g%n", ClampGuardMix.clamp_guard_mix({c_float(sample["args"][0])}));'
        for sample in samples
    )
    runner.write_text(
        "public final class ClampGuardMixRunner {\n"
    ...
    if build["returnCode"] != 0:
        raise RuntimeError(build["outputExcerpt"])
    result = run([java, "-cp", str(tmp_path), "ClampGuardMixRunner"], timeout=30)
    if result["returnCode"] != 0:
        raise RuntimeError(result["outputExcerpt"])
    return parse_stdout_values(result["stdout"])
...
def _build_payload_cached() -> dict[str, Any]:
    fef_p34 = json.loads(FEF_P34_PATH.read_text(encoding="utf-8"))
    with tempfile.TemporaryDirectory(prefix="fef_p35_clamp_runtime_") as tmp:
        tmp_path = Path(tmp)
        source_path = tmp_path / "clamp_guard_mix.eml"
        source_path.write_text(p34.SOURCE_FIXTURE, encoding="utf-8")
        rows = [build_runtime_row(source_path, target, ext, tmp_path) for target, ext in RUNTIME_TARGETS]
    payload = {
        "schemaVersion": SCHEMA_VERSION,
        "date": DATE,
        "status": STATUS,
        "artifactId": "fef-p35-clamp-guard-runtime-execution",
        "decision": "selected_clamp_guard_generated_targets_execute_and_match_reference",
        "sourceFixture": SOURCE_FIXTURE_ID,
        "sampleGrid": SAMPLES,
        "reference": {
            "function": "max(-1.0, min(1.0, x))",
            "values": reference_values(SAMPLES),
        },
        "runtimeRows": rows,
        "summary": summarize(rows),
        "fefP34Link": {
            "path": str(FEF_P34_PATH.relative_to(ROOT)),
            "reviewDecision": fef_p34["reviewDecision"],
        },
        "releaseGates": [
        ...
        "nextMilestones": [
            "Add Go/Kotlin/C#/MATLAB runtime execution checks when local toolchains are available.",
            "Fold runtime execution rows into the selected fixture matrix view.",
...
def build_evidence_packet(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "validationCommands": [
            "python python/scripts/fef_p35_clamp_guard_runtime_execution.py --build --strict",
            "python -m pytest -q python/tests/test_fef_p35_clamp_guard_runtime_execution.py",
        ],
    }
...
def build_command_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "schemaVersion": "monogate.command_feed.fef_p35_clamp_guard_runtime_execution.v0",
        "date": DATE,
        "title": "FEF-P35 Clamp/Guard Runtime Execution",
        "status": payload["status"],
        "decision": payload["decision"],
        "summary": payload["summary"],
        "topFollowup": "Add runtime execution checks for additional installed free-target toolchains or fold runtime rows into the fixture matrix.",
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def validate_payload(payload: dict[str, Any]) -> None:
    if payload["schemaVersion"] != SCHEMA_VERSION:
        raise ValueError("invalid FEF-P35 schema")
    summary = payload["summary"]
...
def build_outputs(out_dir: Path, report_dir: Path, evidence_dir: Path, command_feed_dir: Path) -> dict[str, Any]:
    ...
    result_path = out_dir / f"fef_p35_clamp_guard_runtime_execution_{STAMP}.json"
    report_path = report_dir / f"fef_p35_clamp_guard_runtime_execution_{STAMP}.md"
    evidence_path = evidence_dir / "fef_p35_clamp_guard_runtime_execution.json"
    feed_path = command_feed_dir / f"fef_p35_clamp_guard_runtime_execution_feed_{STAMP}.json"
...
def main() -> int:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/fef_p35_clamp_guard_runtime_execution")
    ...
    print("FEF_P35_CLAMP_GUARD_RUNTIME_EXECUTION_OK")
