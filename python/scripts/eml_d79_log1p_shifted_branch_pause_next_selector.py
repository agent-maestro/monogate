#!/usr/bin/env python3
"""EML-D79 log1p-shifted copy-review next-action selector."""
...
from scripts import eml_d78_log1p_shifted_checked_witness_copy_review_packet as d78  # noqa: E402

DATE = "2026-06-03"
STAMP = DATE.replace("-", "_")
SCHEMA_VERSION = "monogate.eml_log1p_shifted_branch_pause_next_selector.v0"
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    review = d78.build_payload(atlas_gate_path)
    d78.validate_payload(review)
    options = [
        decision_option(
            "log1p_shifted_branch_pause_freeze_packet",
            "private_pause_freeze_lane",
            "selected_next",
            88,
            "EML-D80 log1p-shifted branch pause and checked-witness copy freeze packet",
            [
                "D75 checked the witness, D76 surfaced it privately, D77 selected copy review, and D78 completed private copy review.",
                ...
                "The pause keeps the log1p-shifted witness reviewer-usable without treating private copy review as public approval.",
            ],
            [
                "define exactly what is frozen",
                "preserve D78 caveats, blocked phrases, and the shifted positive-domain guard",
        ...
        decision_option(
            ...
            [
                "A new bounded identity branch remains useful after the log1p-shifted copy boundary is frozen.",
                "Starting it immediately would skip stabilization of the D78 copy-review boundary.",
            ],
            [
        ...
        decision_option(
            ...
            [
                ...
                "It has higher guard and negative-control risk than freezing the just-reviewed log1p-shifted witness copy.",
            ],
            [
        ...
        decision_option(
            ...
            42,
            "Future human-approved log1p-shifted public copy gate",
            [
                "D78 produced private-reviewable wording, but no human approval is recorded.",
    ...
    summary = {
        ...
        "d76SurfaceRowCount": review["summary"]["d76SurfaceRowCount"],
        "d78CopyReviewStarted": review["summary"]["copyReviewStarted"],
        "d78PrivateCopyReviewOnly": review["summary"]["privateCopyReviewOnly"],
        "d78CheckedWitnessCopyReviewOnly": review["summary"]["checkedWitnessCopyReviewOnly"],
        "d78WitnessRowCount": review["summary"]["witnessRowCount"],
        "d78RequiredCaveatCount": review["summary"]["requiredCaveatCount"],
        "d78BlockedGlobalPhraseCount": review["summary"]["blockedGlobalPhraseCount"],
        "d78RowRequiredCaveatCount": review["summary"]["rowRequiredCaveatCount"],
        "d78RowBlockedPhraseCount": review["summary"]["rowBlockedPhraseCount"],
    ...
    return {
        ...
        "selectorType": "eml_log1p_shifted_branch_pause_next_selector_v0",
        "artifactId": "eml-d79-log1p-shifted-branch-pause-next-selector",
        "status": STATUS,
        "decision": "select_log1p_shifted_branch_pause_freeze_packet",
        "date": DATE,
        "sourceReview": review["artifactId"],
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceReview"] != "eml-d78-log1p-shifted-checked-witness-copy-review-packet":
        raise ValueError("D79 must consume D78")
    if summary["sourceSelectedOptionId"] != "log1p_shifted_checked_witness_copy_review_packet":
        raise ValueError("unexpected D78 selected option")
    ...
    for key in [
        "d78CopyReviewStarted",
        "d78PrivateCopyReviewOnly",
        "d78CheckedWitnessCopyReviewOnly",
        "nextActionSelected",
    ...
    if summary["guardCount"] != 1 or summary["sourceDerivedDomainObligationCount"] != 2:
        raise ValueError("log1p-shifted guard/domain counts drifted")
    ...
    if summary["d76SurfaceRowCount"] != 5:
        raise ValueError("D76 row count drift")
    if summary["d78WitnessRowCount"] != 1:
        raise ValueError("expected one D78 witness row")
    if summary["d78RequiredCaveatCount"] != 9 or summary["d78BlockedGlobalPhraseCount"] != 12:
        raise ValueError("D78 caveat/blocker counts drifted")
    if summary["d78RowRequiredCaveatCount"] != 6 or summary["d78RowBlockedPhraseCount"] != 10:
        raise ValueError("D78 row copy boundary counts drifted")
    ...
    if summary["selectedOptionId"] != "log1p_shifted_branch_pause_freeze_packet":
        raise ValueError("unexpected selected option")
    if summary["selectedNextArtifact"] != "EML-D80 log1p-shifted branch pause and checked-witness copy freeze packet":
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "artifactType": "eml_log1p_shifted_branch_pause_next_selector",
        "validationStatus": "pass",
        "semanticStrength": "private_selector_log1p_shifted_pause_freeze_choice_no_public_copy_no_implementation",
        "source": f"python/results/eml_d79_log1p_shifted_branch_pause_next_selector/eml_d79_log1p_shifted_branch_pause_next_selector_{STAMP}.json",
        "summary": payload["summary"],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "feedId": "eml_d79_log1p_shifted_branch_pause_next_selector_feed",
        "date": DATE,
        "status": payload["status"],
        "selectedOptionId": payload["summary"]["selectedOptionId"],
        "selectedNextArtifact": payload["summary"]["selectedNextArtifact"],
        "nextAction": "Run EML-D80 as a private log1p-shifted branch pause and checked-witness copy freeze packet.",
        "claimFlags": dict(CLAIM_FLAGS),
    }
...
def build_outputs(
    ...
    result_path = out_dir / f"eml_d79_log1p_shifted_branch_pause_next_selector_{STAMP}.json"
    report_path = report_dir / f"eml_d79_log1p_shifted_branch_pause_next_selector_{STAMP}.md"
    evidence_path = evidence_dir / "eml_d79_log1p_shifted_branch_pause_next_selector.json"
    feed_path = command_feed_dir / f"eml_d79_log1p_shifted_branch_pause_next_selector_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/eml_d79_log1p_shifted_branch_pause_next_selector")
...
def main() -> int:
    ...
    print("EML_D79_LOG1P_SHIFTED_BRANCH_PAUSE_NEXT_SELECTOR_OK")
    return 0
