#!/usr/bin/env python3
"""FEF-P126 Rust source-preserving fixture expected rows and checker."""
...
from scripts import fef_p125_second_source_preserving_fixture_checker as p125  # noqa: E402

DATE = "2026-06-01"
STAMP = DATE.replace("-", "_")
SCHEMA_VERSION = "monogate.fef_p126_rust_source_preserving_fixture_checker.v0"
...
P125_PACKET = ROOT / "reports/evidence_packets/fef_p125_second_source_preserving_fixture_checker.json"
P125_RESULT = ROOT / "python/results/fef_p125_second_source_preserving_fixture_checker/fef_p125_second_source_preserving_fixture_checker_2026_06_01.json"
SELECTED_FIXTURE_ID = "rust_if_expr_source_layout_v0"
...
NON_CLAIMS = [
    "FEF-P126 records expected rows, checker rows, and negative controls for one Rust source-preserving fixture only.",
    "FEF-P126 does not parse non-generated Rust source.",
    "FEF-P126 does not re-emit non-generated Rust source.",
    "FEF-P126 does not run a preservation oracle.",
    "FEF-P126 does not execute source, generated, or re-ingested code.",
    "FEF-P126 does not validate token, whitespace, comment, formatting, or source-layout fidelity.",
    "FEF-P126 does not implement source-preserving roundtrip support.",
    "FEF-P126 does not widen Forge or eFrog frontend lowering.",
    "FEF-P126 preserves the P120-P125 private review hold boundary.",
    "FEF-P126 does not record reviewer approval or rejection.",
    "FEF-P126 does not claim general branch/control-flow support.",
    "FEF-P126 does not claim full non-generated source roundtrip.",
    "FEF-P126 does not claim arbitrary C/Rust source-family support.",
    "FEF-P126 does not claim compiler correctness or formal semantic equivalence.",
    "FEF-P126 does not claim runtime performance, public readiness, package publication, checkout, or production readiness.",
]

EXPECTED_ROWS = [
    {"id": "if_expression_open_line", "category": "layout", "expectedKind": "exact_line_strict", "expectedValue": "if x > 0.0 {", "rationale": "Rust if-expression opening line is explicit."},
    {"id": "then_tail_expr_line", "category": "expression_tail", "expectedKind": "exact_line_strict", "expectedValue": "    x", "rationale": "The then branch is a tail expression, not a return statement."},
    {"id": "else_opening_line", "category": "layout", "expectedKind": "exact_line_strict", "expectedValue": "} else {", "rationale": "The else boundary is explicit and line-oriented in the sketch."},
    {"id": "else_tail_expr_line", "category": "expression_tail", "expectedKind": "exact_line_strict", "expectedValue": "    0.0", "rationale": "The else branch is also a tail expression."},
    {"id": "if_before_else_order", "category": "token_order", "expectedKind": "relative_order", "expectedValue": ["if x > 0.0", "else"], "rationale": "The if token must precede the else token in the sketch."},
    {"id": "else_token_present", "category": "token_presence", "expectedKind": "presence", "expectedValue": "else", "rationale": "The selected Rust fixture is explicit if/else expression layout."},
    {"id": "line_count", "category": "layout", "expectedKind": "line_count", "expectedValue": 5, "rationale": "The five-line sketch bounds the expected Rust layout surface."},
]

NEGATIVE_CONTROLS = [
    {
        "id": "missing_else_negative_control_v0",
        "mutation": "remove_else_boundary",
        "sourceSketch": "if x > 0.0 {\n    x\n}\n    0.0",
        "expectedFailedRows": ["else_opening_line", "if_before_else_order", "else_token_present", "line_count"],
    },
    {
        "id": "changed_then_tail_negative_control_v0",
        "mutation": "change_then_tail_expression",
        "sourceSketch": "if x > 0.0 {\n    y\n} else {\n    0.0\n}",
        "expectedFailedRows": ["then_tail_expr_line"],
    },
    {
        "id": "single_line_rust_layout_negative_control_v0",
        "mutation": "collapse_if_expression_layout",
        "sourceSketch": "if x > 0.0 { x } else { 0.0 }",
        "expectedFailedRows": ["if_expression_open_line", "then_tail_expr_line", "else_opening_line", "else_tail_expr_line", "line_count"],
    },
]
...
def check_row(source: str, row: dict[str, Any]) -> dict[str, Any]:
    ...
    if kind == "exact_line_strict":
        ...
        matched = row["expectedValue"] in source
...
def build_summary(p125_packet: dict[str, Any], p125_payload: dict[str, Any], fixture: dict[str, Any], rows: list[dict[str, Any]], checks: list[dict[str, Any]], controls: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "sourcePacketCount": 1,
        "p125ValidationPass": p125_packet["validationStatus"] == "pass",
        "p125ClaimFlagsAllFalse": all(value is False for value in p125_packet["claimFlags"].values()),
        "p125CheckerPassCount": p125_payload["summary"]["checkerPassCount"],
        "p125ReviewerDecisionRecorded": p125_payload["summary"]["reviewerDecisionRecorded"],
        "p125ImplementationHeldPendingReview": p125_payload["summary"]["p124ImplementationHeldPendingReview"],
...
def build_payload() -> dict[str, Any]:
    ...
    p125_packet = read_json(P125_PACKET)
    p125_payload = read_json(P125_RESULT)
    p125.validate_payload(p125_payload)
    ...
    payload = {
        ...
        "date": DATE,
        "status": STATUS,
        "artifactId": "fef-p126-rust-source-preserving-fixture-checker",
        "decision": "rust_source_preserving_fixture_expected_rows_checker_negative_controls_recorded_support_blocked",
        "sourcePacket": {
            "phase": "P125",
            "packetPath": str(P125_PACKET.relative_to(ROOT)),
            "resultPath": str(P125_RESULT.relative_to(ROOT)),
            "reviewDecision": p125_packet["reviewDecision"],
            "validationStatus": p125_packet["validationStatus"],
        },
        "selectedFixture": fixture,
        "expectedRows": rows,
        "checkerRows": checks,
        "negativeControls": controls,
        "summary": build_summary(p125_packet, p125_payload, fixture, rows, checks, controls),
        "releaseGates": [
            {"id": "rust_source_preserving_fixture_checker", "status": "recorded"},
        ...
        "allowedPrivateReviewerStatements": [
            "P126 records expected rows, checker rows, and negative controls for rust_if_expr_source_layout_v0.",
            "P126 matches all seven expected rows and all three negative controls fail closed.",
            "P126 keeps source parsing, source re-emission, preservation-oracle execution, and source fidelity validation blocked.",
        ],
        "blockedStatements": [
        ...
        "nextMilestones": [
            "Add the Rust early-return source-order fixture through the same discipline.",
            "Record actual private reviewer response to P47-P126 if one exists.",
...
def build_evidence_packet(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "title": "FEF-P126 Rust Source-Preserving Fixture Checker",
        "reviewDecision": payload["decision"],
        "validationStatus": "pass",
        "semanticStrength": "rust_source_preserving_fixture_checker_recorded_support_blocked",
        "semanticReview": payload["summary"],
        "claimBoundary": "Rust source-preserving fixture expected rows/checker/negative controls only; no source parsing, source re-emission, preservation-oracle execution, token/comment/layout fidelity validation, source-preserving support, frontend widening, branch re-ingest, full source roundtrip, arbitrary source-family, public readiness, compiler correctness, formal equivalence, or runtime performance claim.",
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
        "reviewHighlights": [
            "P126 selects rust_if_expr_source_layout_v0 as the first Rust source-preserving fixture lane.",
            "Seven expected rows pass against the stored Rust source sketch.",
            "Three negative controls fail closed with expected failed rows.",
        ],
        "validationCommands": [
            "python python/scripts/fef_p126_rust_source_preserving_fixture_checker.py --build --strict",
            "python -m pytest -q python/tests/test_fef_p126_rust_source_preserving_fixture_checker.py",
        ],
    }
...
def build_command_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "schemaVersion": "monogate.command_feed.fef_p126_rust_source_preserving_fixture_checker.v0",
        "date": DATE,
        "title": "FEF-P126 Rust Source-Preserving Fixture Checker",
        "status": payload["status"],
        "decision": payload["decision"],
        "summary": payload["summary"],
        "topFollowup": "Add the Rust early-return source-order fixture through the same expected-row/checker/negative-control discipline.",
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def validate_payload(payload: dict[str, Any]) -> None:
    if payload["schemaVersion"] != SCHEMA_VERSION:
        raise ValueError("invalid FEF-P126 schema")
    if payload["status"] != STATUS:
        raise ValueError("invalid FEF-P126 status")
    p125.validate_payload(read_json(P125_RESULT))
    ...
    for key in [
        "p125ValidationPass",
        "p125ClaimFlagsAllFalse",
        "p125ImplementationHeldPendingReview",
    ...
    if summary["p125ReviewerDecisionRecorded"] is not False:
        raise ValueError("P125 reviewer decision must remain not recorded")
    ...
    if summary["negativeControlExpectedFailureCount"] != 10 or summary["negativeControlPassRowCount"] != 11:
...
def build_outputs(out_dir: Path, report_dir: Path, evidence_dir: Path, command_feed_dir: Path) -> dict[str, Any]:
    ...
    result_path = out_dir / f"fef_p126_rust_source_preserving_fixture_checker_{STAMP}.json"
    report_path = report_dir / f"fef_p126_rust_source_preserving_fixture_checker_{STAMP}.md"
    evidence_path = evidence_dir / "fef_p126_rust_source_preserving_fixture_checker.json"
    feed_path = command_feed_dir / f"fef_p126_rust_source_preserving_fixture_checker_feed_{STAMP}.json"
...
def main() -> int:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/fef_p126_rust_source_preserving_fixture_checker")
    ...
    print("FEF_P126_RUST_SOURCE_PRESERVING_FIXTURE_CHECKER_OK")
