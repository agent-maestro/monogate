...
from scripts import eml_d12_next_identity_witness_selector as d12  # noqa: E402

DATE = "2026-06-01"
STAMP = DATE.replace("-", "_")
SCHEMA_VERSION = "monogate.eml_ln_from_eml_witness_attempt.v0"
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    selector = d12.build_payload(atlas_gate_path)
    d12.validate_payload(selector)
    atlas_path = MACHLIB_ROOT / "MachLib" / "EMLAtlasWitness.lean"
    selected = {
        "name": "ln_from_eml_boundary_witness",
        "machlibName": "MachLib.Real.ln_from_eml_boundary_witness",
        "path": "../machlib/foundations/MachLib/EMLAtlasWitness.lean",
        "statement": "eml 1 (eml (eml 1 y) 1) = log y under 0 < y",
        "present": file_contains(atlas_path, "theorem ln_from_eml_boundary_witness"),
    ...
    summary = {
        ...
        "runtimeLoweringControl": "standard_log_remains_runtime_control",
    ...
    return {
        ...
        "attemptType": "eml_ln_from_eml_witness_attempt_v0",
        "artifactId": "eml-d13-ln-from-eml-witness-attempt",
        "status": STATUS,
        "decision": "ln_from_eml_boundary_witness_checked_in_machlib",
        "date": DATE,
        "sourceSelector": selector["artifactId"],
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceSelector"] != "eml-d12-next-identity-witness-selector":
        raise ValueError("D13 must consume D12")
    ...
    if summary["leanTypecheckPerformed"] is not True or summary["lakeBuildPassed"] is not True:
        raise ValueError("D13 requires observed Lake build pass")
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "artifactType": "eml_ln_from_eml_witness_attempt",
        "validationStatus": "pass",
        "semanticStrength": "scoped_machlib_identity_witness_checked",
        "source": f"python/results/eml_d13_ln_from_eml_witness_attempt/eml_d13_ln_from_eml_witness_attempt_{STAMP}.json",
        "summary": payload["summary"],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "feedId": "eml_d13_ln_from_eml_witness_attempt_feed",
        "date": DATE,
        "status": payload["status"],
        "selectedWitnessName": payload["summary"]["selectedWitnessName"],
        "nextAction": "Refresh Atlas proof status for ln_from_eml without public promotion.",
        "claimFlags": dict(CLAIM_FLAGS),
    }
...
def build_outputs(out_dir: Path, report_dir: Path, evidence_dir: Path, command_feed_dir: Path, atlas_gate_path: Path) -> dict[str, Any]:
    ...
    result_path = out_dir / f"eml_d13_ln_from_eml_witness_attempt_{STAMP}.json"
    report_path = report_dir / f"eml_d13_ln_from_eml_witness_attempt_{STAMP}.md"
    evidence_path = evidence_dir / "eml_d13_ln_from_eml_witness_attempt.json"
    feed_path = command_feed_dir / f"eml_d13_ln_from_eml_witness_attempt_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/eml_d13_ln_from_eml_witness_attempt")
...
def main() -> int:
    ...
    print("EML_D13_LN_FROM_EML_WITNESS_ATTEMPT_OK")
    return 0
