...
from scripts import eml_d73_bounded_identity_branch_candidate_selector as d73  # noqa: E402

DATE = "2026-06-03"
STAMP = DATE.replace("-", "_")
SCHEMA_VERSION = "monogate.eml_log1p_shifted_boundary_coordinate_feasibility.v0"
...
FEASIBILITY_ITEMS = [
    {
        "itemId": "selected_branch_matches_d73",
        "status": "satisfied",
        "evidence": "D73 selected log1p_shifted_boundary_coordinate as selected_next.",
    ...
    },
    {
        ...
        "status": "satisfied",
        "evidence": "D73 records no duplicate checked witness for the selected log1p-shifted coordinate.",
        "reviewNote": "The candidate is a fresh guarded coordinate, not a relabeling of an existing checked witness.",
    },
]

NEGATIVE_CONTROLS = [
    {
        "controlId": "x_minus_one_boundary_blocked",
        "status": "blocked_by_guard",
        "blockedCondition": "x = -1",
        "reason": "log (1 + x) is outside the guarded real-log rewrite domain.",
    },
    {
        "controlId": "x_below_minus_one_blocked",
        "status": "blocked_by_guard",
        "blockedCondition": "x < -1",
        "reason": "1 + x is not positive, so the guarded exp-log rewrite is unavailable.",
    },
    {
        "controlId": "unguarded_log1p_shifted_coordinate_blocked",
        "status": "blocked_by_guard",
    ...
    },
    {
        ...
        "reason": "D74 records proof-shape feasibility only and keeps protected log/log1p runtime controls.",
    },
]
...
BLOCKERS = [
    ...
    },
    {
        ...
        "severity": "hard_blocker",
        "description": "D74 records feasibility only; any proof, Lean typecheck, or MachLib edit requires a separate D75 phase.",
    },
    {
        "blockerId": "broad_log1p_language",
        "severity": "hard_blocker",
        "description": "The packet must not broaden the single guarded coordinate into all log1p, logarithm, or shifted-domain identities.",
    },
]
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    selector = d73.build_payload(atlas_gate_path)
    d73.validate_payload(selector)
    selected = selected_candidate_by_id(selector, "log1p_shifted_boundary_coordinate")
    proposed_witness = {
        "candidateId": selected["candidateId"],
        "family": selected["family"],
        "sourceFrontierId": selected["sourceFrontierId"],
        "proposedMachlibName": "MachLib.Real.log1p_shifted_boundary_coordinate_witness",
        ...
        "proposedStatement": "0 < 1 + x -> eml (log (1 + x)) (exp 1) = x",
        ...
        "nextArtifact": "EML-D75 log1p shifted boundary coordinate MachLib witness attempt or blocker",
    }
    summary = {
        ...
        "sourceFrozenGuardCount": selector["summary"]["sourceFrozenGuardCount"],
    ...
    return {
        ...
        "packetType": "eml_log1p_shifted_boundary_coordinate_feasibility_v0",
        "artifactId": "eml-d74-log1p-shifted-boundary-coordinate-feasibility-packet",
        "status": STATUS,
        "decision": "record_log1p_shifted_boundary_coordinate_feasibility",
        "date": DATE,
        "sourceCandidateSelector": selector["artifactId"],
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceCandidateSelector"] != "eml-d73-bounded-identity-branch-candidate-selector":
        raise ValueError("D74 must consume D73")
    if summary["sourceSelectedCandidateId"] != "log1p_shifted_boundary_coordinate":
        raise ValueError("D74 must preserve the D73 selected candidate")
    if summary["sourceSelectedFamily"] != "guarded_log1p_shifted_coordinate":
        raise ValueError("D74 must preserve the D73 selected family")
    ...
    if summary["sourceSelectedProposedStatement"] != "0 < 1 + x -> eml (log (1 + x)) (exp 1) = x":
        raise ValueError("unexpected source proposed statement")
    if summary["sourceSelectedNextArtifact"] != "EML-D74 log1p shifted boundary coordinate feasibility packet":
        raise ValueError("unexpected D73 next artifact")
    if summary["sourceRuntimeLoweringControl"] != "protected_log_and_log1p_remain_runtime_controls":
        raise ValueError("D73 runtime control drift")
    if summary["sourceFrozenWitnessName"] != "MachLib.Real.probability_logit_boundary_coordinate_witness":
        raise ValueError("D71 frozen witness drift")
    if summary["sourceFrozenStatement"] != "0 < p -> p < 1 -> eml (log p) (exp (log (1 - p))) = p - log (1 - p)":
        raise ValueError("D71 frozen statement drift")
    if summary["sourceFrozenGuardCount"] != 2:
        raise ValueError("D71 frozen guard count drift")
    if summary["sourceFrozenCaveatCount"] != 9 or summary["sourceFrozenBlockedPhraseCount"] != 12:
        raise ValueError("D71 frozen caveat/blocker counts drifted")
    if witness["proposedMachlibName"] != "MachLib.Real.log1p_shifted_boundary_coordinate_witness":
        raise ValueError("unexpected proposed MachLib name")
    if witness["proposedStatement"] != "0 < 1 + x -> eml (log (1 + x)) (exp 1) = x":
    ...
    if summary["nextArtifact"] != "EML-D75 log1p shifted boundary coordinate MachLib witness attempt or blocker":
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "artifactType": "eml_log1p_shifted_boundary_coordinate_feasibility_packet",
        "validationStatus": "pass",
        "semanticStrength": "private_guarded_feasibility_packet_no_machlib_edit_no_typecheck_no_runtime_claim",
        "source": f"python/results/eml_d74_log1p_shifted_boundary_coordinate_feasibility_packet/eml_d74_log1p_shifted_boundary_coordinate_feasibility_packet_{STAMP}.json",
        "summary": payload["summary"],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "feedId": "eml_d74_log1p_shifted_boundary_coordinate_feasibility_packet_feed",
        "date": DATE,
        ...
        "nextAction": "Run EML-D75 only as a scoped guarded MachLib witness attempt or precise blocker.",
        "claimFlags": dict(CLAIM_FLAGS),
    }
...
def build_outputs(out_dir: Path, report_dir: Path, evidence_dir: Path, command_feed_dir: Path, atlas_gate_path: Path) -> dict[str, Any]:
    ...
    result_path = out_dir / f"eml_d74_log1p_shifted_boundary_coordinate_feasibility_packet_{STAMP}.json"
    report_path = report_dir / f"eml_d74_log1p_shifted_boundary_coordinate_feasibility_packet_{STAMP}.md"
    evidence_path = evidence_dir / "eml_d74_log1p_shifted_boundary_coordinate_feasibility_packet.json"
    feed_path = command_feed_dir / f"eml_d74_log1p_shifted_boundary_coordinate_feasibility_packet_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/eml_d74_log1p_shifted_boundary_coordinate_feasibility_packet")
...
def main() -> int:
    ...
    print("EML_D74_LOG1P_SHIFTED_BOUNDARY_COORDINATE_FEASIBILITY_OK")
    return 0
