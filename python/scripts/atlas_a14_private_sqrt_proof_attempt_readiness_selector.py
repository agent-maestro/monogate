...
from scripts import atlas_a13_private_scoped_sqrt_proof_attempt_gate_packet as a13  # noqa: E402
...
SCHEMA_VERSION = "monogate.private_sqrt_proof_attempt_readiness_selector.v0"
STATUS = "ATLAS_A14_PRIVATE_SQRT_PROOF_ATTEMPT_READINESS_SELECTOR_PASS"
ARTIFACT_ID = "atlas-a14-private-sqrt-proof-attempt-readiness-selector"
...
SELECTED_OPTION_ID = "recommend_future_scoped_sqrt_attempt_packet"
NEXT_RECOMMENDED_ARTIFACT = "ATLAS-A15 private scoped sqrt proof-attempt packet"

TRUE_CLAIM_FLAGS = {
    "atlas_a13_consumed",
    "proof_attempt_readiness_selector_created",
    "gate_packet_reviewed",
    "scope_budget_reviewed",
    "abort_conditions_reviewed",
    "future_attempt_packet_recommended",
...
CLAIM_FLAGS = {
    "atlas_a13_consumed": True,
    "proof_attempt_readiness_selector_created": True,
    "gate_packet_reviewed": True,
    "scope_budget_reviewed": True,
    "abort_conditions_reviewed": True,
    "future_attempt_packet_recommended": True,
    ...
    "scoped_attempt_packet_created": False,
...
def build_options(source: dict[str, Any]) -> list[dict[str, Any]]:
    gate = source["proofAttemptGate"]
    return [
        {
            "optionId": SELECTED_OPTION_ID,
            "selectionStatus": "selected_next",
            "decision": "recommend_attempt_packet_without_starting_attempt",
        ...
        },
        {
            ...
            "readinessReasons": ["The gate packet is already useful as bounded Atlas process evidence."],
            "remainingBlocks": ["no public copy approval", "target lower bound remains unreached"],
            "gateSignals": None,
            "claimFlags": dict(CLAIM_FLAGS),
        },
        {
            ...
            "nextArtifact": "Park sqrt candidate before proof attempt",
            "readinessReasons": ["Parking remains valid if an actual attempt is not worth the proof cost."],
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    ...
    summary = {
        ...
        "proofAttemptReadinessSelectorCreated": True,
        "gatePacketReviewed": True,
        "scopeBudgetReviewed": True,
        ...
        "futureAttemptPacketRecommended": True,
        "scopedAttemptPacketCreated": False,
    ...
    return build_claim_flagged_packet(
        ...
        artifact_type="private_sqrt_proof_attempt_readiness_selector",
        status=STATUS,
        date=DATE,
        summary=summary,
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceArtifact"] != "atlas-a13-private-scoped-sqrt-proof-attempt-gate-packet":
        raise ValueError("ATLAS-A14 must consume ATLAS-A13")
    ...
    if summary["reviewCheckpointCount"] != 5:
    ...
    if summary["selectedDecision"] != "recommend_attempt_packet_without_starting_attempt":
    ...
    for key in [
        ...
        "futureAttemptPacketRecommended",
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return build_evidence_packet(
        ...
        artifact_type="private_sqrt_proof_attempt_readiness_selector",
        semantic_strength="private_selector_recommends_future_sqrt_attempt_packet_no_attempt_no_validity_no_lean",
        source=f"python/results/atlas_a14_private_sqrt_proof_attempt_readiness_selector/atlas_a14_private_sqrt_proof_attempt_readiness_selector_{STAMP}.json",
        summary=payload["summary"],
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return build_command_feed(
        feed_id="atlas_a14_private_sqrt_proof_attempt_readiness_selector_feed",
        date=DATE,
        status=payload["status"],
        next_action="Run ATLAS-A15 as a private scoped sqrt proof-attempt packet.",
        claim_flags=payload["claimFlags"],
        fields={
...
def build_report(payload: dict[str, Any]) -> str:
    summary_rows = [
        ...
        ("scoped attempt packet created", payload["summary"]["scopedAttemptPacketCreated"]),
    ...
    readiness_lines = [f"- {item}" for item in payload["selectedOption"]["readinessReasons"]]
    ...
    return render_markdown_report(
        title="ATLAS-A14 Private Sqrt Proof-Attempt Readiness Selector",
        status=payload["status"],
        summary_rows=summary_rows,
        sections=[("Readiness Reasons", readiness_lines), ("Remaining Blocks", block_lines), ("Options", option_lines)],
        non_claims=payload["nonClaims"],
    )
...
def build_outputs(
    ...
    result_path = out_dir / f"atlas_a14_private_sqrt_proof_attempt_readiness_selector_{STAMP}.json"
    report_path = report_dir / f"atlas_a14_private_sqrt_proof_attempt_readiness_selector_{STAMP}.md"
    evidence_path = evidence_dir / "atlas_a14_private_sqrt_proof_attempt_readiness_selector.json"
    feed_path = command_feed_dir / f"atlas_a14_private_sqrt_proof_attempt_readiness_selector_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=ROOT / "python/results/atlas_a14_private_sqrt_proof_attempt_readiness_selector",
    )
...
def main() -> int:
    ...
    print("ATLAS_A14_PRIVATE_SQRT_PROOF_ATTEMPT_READINESS_SELECTOR_OK")
    return 0
