...
from scripts import atlas_a32_private_exp_negation_wrapper_or_alias_attempt_gate as a32  # noqa: E402
...
SCHEMA_VERSION = "monogate.private_exp_negation_bounded_wrapper_attempt_artifact.v0"
STATUS = "ATLAS_A33_PRIVATE_EXP_NEGATION_BOUNDED_WRAPPER_ATTEMPT_ARTIFACT_PASS"
ARTIFACT_ID = "atlas-a33-private-exp-negation-bounded-wrapper-attempt-artifact"
SOURCE_DIRECTION_ID = "exp_negation_multiplicative_identity_direction"
MACHLIB_NAME = "MachLib.Real.exp_negation_multiplicative_identity_witness"
MACHLIB_FILE = "foundations/MachLib/EMLAtlasWitness.lean"
DEPENDENCY_IDENTIFIER = "MachLib.HyperbolicPreservation.exp_mul_exp_neg"
NEXT_RECOMMENDED_ARTIFACT = "ATLAS-A34 private exp-negation checked-wrapper surface review"
...
def build_checked_wrapper(source: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "runtimeControl": "standard_exp_remains_runtime_control",
        "claimFlags": dict(CLAIM_FLAGS),
    }


def build_payload(atlas_gate_path: Path, machlib_root: Path) -> dict[str, Any]:
    source = a32.build_payload(atlas_gate_path, machlib_root)
    a32.validate_payload(source)
    ...
    return build_claim_flagged_packet(
        ...
        artifact_type="private_exp_negation_bounded_wrapper_attempt_artifact",
        status=STATUS,
        date=DATE,
        summary=summary,
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceArtifact"] != "atlas-a32-private-exp-negation-wrapper-or-alias-attempt-gate":
        raise ValueError("ATLAS-A33 must consume ATLAS-A32")
    if summary["sourceReviewedDirectionId"] != SOURCE_DIRECTION_ID:
        raise ValueError("A33 must consume exp-negation direction")
    ...
    if summary["sourceAtlasRowCount"] != 13 or summary["atlasRowCount"] != 14:
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return build_evidence_packet(
        ...
        source=f"python/results/atlas_a33_private_exp_negation_bounded_wrapper_attempt_artifact/atlas_a33_private_exp_negation_bounded_wrapper_attempt_artifact_{STAMP}.json",
        summary=payload["summary"],
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return build_command_feed(
        feed_id="atlas_a33_private_exp_negation_bounded_wrapper_attempt_artifact_feed",
        date=DATE,
        status=payload["status"],
        next_action="Run ATLAS-A34 as a private checked-wrapper surface review; keep public, runtime, product, and EML-companion claims blocked.",
        claim_flags=payload["claimFlags"],
        fields={
...
def build_report(payload: dict[str, Any]) -> str:
    ...
    return render_markdown_report(
        title="ATLAS-A33 Private Exp-Negation Bounded Wrapper Attempt Artifact",
        status=payload["status"],
...
def build_outputs(
    ...
    result_path = out_dir / f"atlas_a33_private_exp_negation_bounded_wrapper_attempt_artifact_{STAMP}.json"
    report_path = report_dir / f"atlas_a33_private_exp_negation_bounded_wrapper_attempt_artifact_{STAMP}.md"
    evidence_path = evidence_dir / "atlas_a33_private_exp_negation_bounded_wrapper_attempt_artifact.json"
    feed_path = command_feed_dir / f"atlas_a33_private_exp_negation_bounded_wrapper_attempt_artifact_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=ROOT / "python/results/atlas_a33_private_exp_negation_bounded_wrapper_attempt_artifact",
    )
...
def main() -> int:
    ...
    print("ATLAS_A33_PRIVATE_EXP_NEGATION_BOUNDED_WRAPPER_ATTEMPT_ARTIFACT_OK")
    return 0
