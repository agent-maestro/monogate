#!/usr/bin/env python3
"""GB-VIS-A12 private adapter intake feed guard packet."""
...
from scripts import gb_vis_a11_private_adapter_intake_snapshot as a11  # noqa: E402

DATE = "2026-06-02"
STAMP = DATE.replace("-", "_")
SCHEMA_VERSION = "monogate.private_adapter_intake_feed_guard.v0"
EVIDENCE_SCHEMA_VERSION = "monogate.evidence_public_packet.v0"
STATUS = "GB_VIS_A12_PRIVATE_ADAPTER_INTAKE_FEED_GUARD_PASS"

EXPECTED_SOURCE_FEED_ID = "gb_vis_a11_private_adapter_intake_snapshot_feed"
EXPECTED_SOURCE_STATUS = "GB_VIS_A11_PRIVATE_ADAPTER_INTAKE_SNAPSHOT_PASS"
EXPECTED_SOURCE_DECISION = "record_private_adapter_intake_snapshot_no_renderer_execution_no_public_promotion"

ALLOWED_TRUE_SOURCE_CLAIM_FLAGS = {
    "private_adapter_intake_snapshot_recorded",
    "gb_vis_a10_adapter_intake_guard_consumed",
...
CLAIM_FLAGS = {
    "private_adapter_intake_feed_guard_recorded": True,
    "gb_vis_a11_adapter_intake_snapshot_consumed": True,
...
NON_CLAIMS = [
    "GB-VIS-A12 records a private adapter intake feed guard only; it does not implement, execute, or pixel-render a renderer.",
    "GB-VIS-A12 rebuilds and guards the GB-VIS-A11 command feed without accepting laptop artifacts, proving visualization correctness, renderer soundness, validator soundness, a Galois connection, abstract interpretation correctness, compiler correctness, formal equivalence, or full EML semantics.",
    "GB-VIS-A12 does not update public surfaces, runtime behavior, MachLib, production validators, laptop-owned repos, electronics repos, or course artifacts.",
]
...
def feed_guard_rows(source_feed: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        {
            "guardRowId": "gb_vis_a12_feed_guard:feed_id",
            "observed": source_feed["feedId"],
            "expected": EXPECTED_SOURCE_FEED_ID,
            "status": "pass",
        },
        {
            "guardRowId": "gb_vis_a12_feed_guard:status",
            "observed": source_feed["status"],
            "expected": EXPECTED_SOURCE_STATUS,
            "status": "pass",
        },
        {
            "guardRowId": "gb_vis_a12_feed_guard:decision",
            "observed": source_feed["decision"],
            "expected": EXPECTED_SOURCE_DECISION,
            "status": "pass",
        },
        {
            "guardRowId": "gb_vis_a12_feed_guard:next_action_is_private",
            "observed": "without public promotion" in source_feed["nextAction"],
            "expected": True,
            "status": "pass",
        },
        {
            "guardRowId": "gb_vis_a12_feed_guard:allowed_true_claim_flags",
            "observed": true_flags,
            "expected": sorted(ALLOWED_TRUE_SOURCE_CLAIM_FLAGS),
            "status": "pass",
        },
        {
            "guardRowId": "gb_vis_a12_feed_guard:blocked_claim_flags_false",
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    ...
    source_feed = a11.build_feed(source)
    rows = feed_guard_rows(source_feed)
    for row in rows:
        if row["observed"] != row["expected"]:
            raise ValueError(f"adapter intake feed guard row failed: {row['guardRowId']}")
    summary = {
        "sourceAdapterIntakeSnapshot": source["artifactId"],
        ...
        "privateAdapterIntakeFeedGuardRecorded": True,
        "gbVisA11AdapterIntakeSnapshotConsumed": True,
        ...
        "publicReady": False,
        "nextAction": "ACT-A13 reviewer promotion hold gate or GB-VIS-A13 private adapter promotion hold gate without public promotion.",
        "claimFlagsBounded": all(
            CLAIM_FLAGS[key] is True
            for key in [
                "private_adapter_intake_feed_guard_recorded",
                "gb_vis_a11_adapter_intake_snapshot_consumed",
                "source_feed_rebuilt",
    ...
    return {
        ...
        "packetType": "private_adapter_intake_feed_guard_v0",
        "artifactId": "gb-vis-a12-private-adapter-intake-feed-guard",
        "status": STATUS,
        "decision": "record_private_adapter_intake_feed_guard_no_renderer_execution_no_public_promotion",
        "date": DATE,
        "sourceAdapterIntakeSnapshot": source["artifactId"],
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceAdapterIntakeSnapshot"] != "gb-vis-a11-private-adapter-intake-snapshot":
        raise ValueError("GB-VIS-A12 must consume GB-VIS-A11")
    ...
    for row in payload["feedGuardRows"]:
        if row["status"] != "pass" or row["observed"] != row["expected"]:
            raise ValueError("adapter intake feed guard row must pass exactly")
    for key in [
        "privateAdapterIntakeFeedGuardRecorded",
        "gbVisA11AdapterIntakeSnapshotConsumed",
        "sourceFeedRebuilt",
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "artifactType": "private_adapter_intake_feed_guard",
        "validationStatus": "pass",
        "semanticStrength": "private_adapter_intake_feed_guard_no_renderer_execution_no_public_update",
        "source": f"python/results/gb_vis_a12_private_adapter_intake_feed_guard/gb_vis_a12_private_adapter_intake_feed_guard_{STAMP}.json",
        "summary": payload["summary"],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "feedId": "gb_vis_a12_private_adapter_intake_feed_guard_feed",
        "date": DATE,
...
def build_report(payload: dict[str, Any]) -> str:
    lines = [
        "# GB-VIS-A12 Private Adapter Intake Feed Guard",
        "",
        f"Status: `{payload['status']}`",
        "",
        "GB-VIS-A12 records a private adapter intake feed guard without implementing or executing a renderer.",
        "",
        "| Count | Value |",
        "|---|---|",
...
def build_outputs(out_dir: Path, report_dir: Path, evidence_dir: Path, command_feed_dir: Path, atlas_gate_path: Path) -> dict[str, Any]:
    ...
    result_path = out_dir / f"gb_vis_a12_private_adapter_intake_feed_guard_{STAMP}.json"
    report_path = report_dir / f"gb_vis_a12_private_adapter_intake_feed_guard_{STAMP}.md"
    evidence_path = evidence_dir / "gb_vis_a12_private_adapter_intake_feed_guard.json"
    feed_path = command_feed_dir / f"gb_vis_a12_private_adapter_intake_feed_guard_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/gb_vis_a12_private_adapter_intake_feed_guard")
...
def main() -> int:
    ...
    print("GB_VIS_A12_PRIVATE_ADAPTER_INTAKE_FEED_GUARD_OK")
    return 0
