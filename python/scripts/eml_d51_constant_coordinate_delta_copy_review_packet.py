...
from scripts import eml_d50_constant_coordinate_next_action_selector as d50  # noqa: E402

DATE = "2026-06-02"
STAMP = DATE.replace("-", "_")
SCHEMA_VERSION = "monogate.eml_constant_coordinate_delta_copy_review_packet.v0"
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    selector = d50.build_payload(atlas_gate_path)
    d50.validate_payload(selector)
    row = witness_copy_row(
        "constant_coordinate_zero_exp_two",
        "MachLib.Real.constant_coordinate_zero_exp_two_witness",
        "A checked MachLib witness records the constant-coordinate identity eml 0 (exp (1 + 1)) = -1; this is the local Lean spelling of the D47 source target eml 0 (exp 2) = -1.",
        [
            "Always distinguish the D47 source statement eml 0 (exp 2) = -1 from the checked Lean statement eml 0 (exp (1 + 1)) = -1.",
            "Say MachLib's current Real numerals cover 0 and 1, so 2 is represented locally as 1 + 1.",
            "Describe this as one scoped checked witness, not theorem discovery or a broad constant-coordinate family.",
            "Preserve the non-duplicate boundary against MachLib.Real.constants_zero_one_e_boundary_witness.",
            "Keep standard log/exp and arithmetic as the semantic and runtime controls.",
        ...
        [
            ...
            "runtime advantage",
            "all constants",
            "all constant-coordinate identities",
            "duplicate constants bundle",
            "unchecked exp 2 Lean theorem",
            "public ready",
            "theorem discovery",
        ],
        "standard log/exp and arithmetic remain runtime controls",
    )
    required_caveats = [
    ...
    summary = {
        ...
        "publicReady": False,
        "nextAction": "EML-D52 choose constant-coordinate pause/freeze, next bounded branch, or human-approved public copy gate.",
    ...
    return {
        ...
        "reviewType": "eml_constant_coordinate_delta_copy_review_packet_v0",
        "artifactId": "eml-d51-constant-coordinate-delta-copy-review-packet",
        "status": STATUS,
        "decision": "constant_coordinate_delta_copy_review_private_only_public_copy_held",
        "date": DATE,
        "sourceSelector": selector["artifactId"],
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceSelector"] != "eml-d50-constant-coordinate-next-action-selector":
        raise ValueError("D51 must consume D50")
    ...
    if summary["guardCount"] != 0:
        raise ValueError("constant-coordinate copy review should not add guards")
    ...
    if summary["nextAction"] != "EML-D52 choose constant-coordinate pause/freeze, next bounded branch, or human-approved public copy gate.":
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "artifactType": "eml_constant_coordinate_delta_copy_review_packet",
        "validationStatus": "pass",
        "semanticStrength": "private_constant_coordinate_delta_copy_review_public_copy_held",
        "source": f"python/results/eml_d51_constant_coordinate_delta_copy_review_packet/eml_d51_constant_coordinate_delta_copy_review_packet_{STAMP}.json",
        "summary": payload["summary"],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "feedId": "eml_d51_constant_coordinate_delta_copy_review_packet_feed",
        "date": DATE,
...
def build_outputs(out_dir: Path, report_dir: Path, evidence_dir: Path, command_feed_dir: Path, atlas_gate_path: Path) -> dict[str, Any]:
    ...
    result_path = out_dir / f"eml_d51_constant_coordinate_delta_copy_review_packet_{STAMP}.json"
    report_path = report_dir / f"eml_d51_constant_coordinate_delta_copy_review_packet_{STAMP}.md"
    evidence_path = evidence_dir / "eml_d51_constant_coordinate_delta_copy_review_packet.json"
    feed_path = command_feed_dir / f"eml_d51_constant_coordinate_delta_copy_review_packet_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/eml_d51_constant_coordinate_delta_copy_review_packet")
...
def main() -> int:
    ...
    print("EML_D51_CONSTANT_COORDINATE_DELTA_COPY_REVIEW_PACKET_OK")
    return 0
