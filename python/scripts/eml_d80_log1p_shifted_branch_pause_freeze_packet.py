...
from scripts import eml_d79_log1p_shifted_branch_pause_next_selector as d79  # noqa: E402

DATE = "2026-06-03"
STAMP = DATE.replace("-", "_")
SCHEMA_VERSION = "monogate.eml_log1p_shifted_branch_pause_freeze_packet.v0"
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    selector = d79.build_payload(atlas_gate_path)
    d79.validate_payload(selector)
    frozen_caveats = [
        "This checked-witness copy freeze is private-only.",
        "The checked statement is 0 < 1 + x -> eml (log (1 + x)) (exp 1) = x.",
        "The witness name is MachLib.Real.log1p_shifted_boundary_coordinate_witness.",
        "The shifted positive-domain guard, 0 < 1 + x, remains required.",
        "Protected log and log1p remain the runtime and domain controls.",
        "D74 negative controls for x = -1, x < -1, and unguarded statements remain blocked.",
        "The witness is one scoped MachLib theorem name, not a broad log1p or logarithm theory.",
    ...
    freeze_rows = [
        freeze_row(
            "log1p_shifted_boundary_coordinate_checked_copy",
    ...
    parked_options = [
        parked_option(
            ...
            "parked_after_log1p_shifted_pause",
            "Available only through a later selector after the log1p-shifted checked-witness copy boundary is frozen.",
        ),
        parked_option(
        ...
        parked_option(
            ...
            "D80 freezes private copy boundaries but records no human approval for public use.",
        ),
    ]
    summary = {
        ...
        "d76SurfaceRowCount": selector["summary"]["d76SurfaceRowCount"],
        "frozenCaveatCount": len(frozen_caveats),
        "frozenBlockedPhraseCount": len(frozen_blocked_phrases),
        "sourceD78RequiredCaveatCount": selector["summary"]["d78RequiredCaveatCount"],
        "sourceD78BlockedGlobalPhraseCount": selector["summary"]["d78BlockedGlobalPhraseCount"],
        "sourceD78RowRequiredCaveatCount": selector["summary"]["d78RowRequiredCaveatCount"],
        "sourceD78RowBlockedPhraseCount": selector["summary"]["d78RowBlockedPhraseCount"],
        ...
        "publicReady": False,
        "nextAction": "EML-D81 select the next private post-pause action without public promotion.",
    ...
    return {
        ...
        "packetType": "eml_log1p_shifted_branch_pause_freeze_packet_v0",
        "artifactId": "eml-d80-log1p-shifted-branch-pause-freeze-packet",
        "status": STATUS,
        "decision": "pause_log1p_shifted_branch_and_freeze_checked_witness_copy",
        "date": DATE,
        "sourceSelector": selector["artifactId"],
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceSelector"] != "eml-d79-log1p-shifted-branch-pause-next-selector":
        raise ValueError("D80 must consume D79")
    if summary["selectedOptionId"] != "log1p_shifted_branch_pause_freeze_packet":
        raise ValueError("unexpected D79 selected option")
    if summary["selectedNextArtifact"] != "EML-D80 log1p-shifted branch pause and checked-witness copy freeze packet":
        raise ValueError("unexpected D79 next artifact")
    ...
    if summary["sourceD78RequiredCaveatCount"] != 9 or summary["sourceD78BlockedGlobalPhraseCount"] != 12:
        raise ValueError("D78 caveat/blocker counts drifted")
    if summary["sourceD78RowRequiredCaveatCount"] != 6 or summary["sourceD78RowBlockedPhraseCount"] != 10:
    ...
    if summary["nextAction"] != "EML-D81 select the next private post-pause action without public promotion.":
    ...
    for row in payload["freezeRows"]:
        if row["machlibName"] != "MachLib.Real.log1p_shifted_boundary_coordinate_witness":
        ...
        if row["guards"] != ["0 < 1 + x"]:
            raise ValueError("log1p-shifted freeze row must preserve the shifted guard")
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "artifactType": "eml_log1p_shifted_branch_pause_freeze_packet",
        "validationStatus": "pass",
        "semanticStrength": "private_log1p_shifted_checked_witness_copy_frozen_public_copy_held_no_new_proof",
        "source": f"python/results/eml_d80_log1p_shifted_branch_pause_freeze_packet/eml_d80_log1p_shifted_branch_pause_freeze_packet_{STAMP}.json",
        "summary": payload["summary"],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "feedId": "eml_d80_log1p_shifted_branch_pause_freeze_packet_feed",
        "date": DATE,
...
def build_outputs(
    ...
    result_path = out_dir / f"eml_d80_log1p_shifted_branch_pause_freeze_packet_{STAMP}.json"
    report_path = report_dir / f"eml_d80_log1p_shifted_branch_pause_freeze_packet_{STAMP}.md"
    evidence_path = evidence_dir / "eml_d80_log1p_shifted_branch_pause_freeze_packet.json"
    feed_path = command_feed_dir / f"eml_d80_log1p_shifted_branch_pause_freeze_packet_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/eml_d80_log1p_shifted_branch_pause_freeze_packet")
...
def main() -> int:
    ...
    print("EML_D80_LOG1P_SHIFTED_BRANCH_PAUSE_FREEZE_PACKET_OK")
    return 0
