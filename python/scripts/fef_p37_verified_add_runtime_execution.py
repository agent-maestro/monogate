#!/usr/bin/env python3
"""FEF-P37 runtime execution guard for generated verified_add targets."""
...
SCHEMA_VERSION = "monogate.fef_p37_verified_add_runtime_execution.v0"
EVIDENCE_SCHEMA_VERSION = "monogate.evidence_public_packet.v0"
STATUS = "FEF_P37_VERIFIED_ADD_RUNTIME_EXECUTION_PASS"
ATOL = 1e-12

FEF_P36_PATH = ROOT / "reports/evidence_packets/fef_p36_free_target_capability_matrix.json"
SOURCE_FIXTURE_ID = "examples/verified_add.eml"
SOURCE_PATH = FORGE_ROOT / SOURCE_FIXTURE_ID
FUNCTION_NAME = "add_nonneg"
SAMPLES = [
    {"args": [0.0, 0.0], "labels": ["a", "b"]},
    {"args": [1.0, 2.0], "labels": ["a", "b"]},
    {"args": [0.25, 0.75], "labels": ["a", "b"]},
    {"args": [10.0, 0.5], "labels": ["a", "b"]},
    {"args": [123.456, 0.001], "labels": ["a", "b"]},
    {"args": [1.0e-9, 2.0e-9], "labels": ["a", "b"]},
]
...
NON_CLAIMS = [
    "FEF-P37 records runtime execution for installed toolchains over one selected verified_add fixture.",
    "FEF-P37 does not execute all 13 free targets.",
    "FEF-P37 does not claim all free targets are public-ready.",
    "FEF-P37 does not claim compiler correctness or formal semantic equivalence.",
    "FEF-P37 does not publish a package, enable checkout, or claim production readiness.",
    "FEF-P37 does not claim runtime performance, Verilog, silicon, hardware, Pro-target, or all-target readiness.",
]
...
def reference_values(samples: list[dict[str, Any]]) -> list[float]:
    return [float(sample["args"][0]) + float(sample["args"][1]) for sample in samples]
...
def call_python(path: Path, samples: list[dict[str, Any]]) -> list[float]:
    spec = importlib.util.spec_from_file_location("fef_p37_verified_add_generated", path)
...
def call_c(path: Path, samples: list[dict[str, Any]], tmp_path: Path) -> list[float]:
    ...
    calls = "\n".join(
        f'    printf("%.17g\\n", add_nonneg({c_float(sample["args"][0])}, {c_float(sample["args"][1])}));'
...
def call_cpp(path: Path, samples: list[dict[str, Any]], tmp_path: Path) -> list[float]:
    ...
    header = tmp_path / "verified_add.hpp"
    header.write_text(path.read_text(encoding="utf-8"), encoding="utf-8")
    runner = tmp_path / "cpp_runner.cpp"
    binary = tmp_path / "cpp_runner"
    calls = "\n".join(
        f'    std::cout << std::setprecision(17) << forge::verified_add::add_nonneg({c_float(sample["args"][0])}, {c_float(sample["args"][1])}) << "\\n";'
        for sample in samples
    )
    runner.write_text(
...
def call_rust(path: Path, samples: list[dict[str, Any]], tmp_path: Path) -> list[float]:
    ...
    calls = "\n".join(
        f'    println!("{{:.17e}}", add_nonneg({rust_float(sample["args"][0])}, {rust_float(sample["args"][1])}));'
        for sample in samples
    )
    runner.write_text(
...
def call_java(path: Path, samples: list[dict[str, Any]], tmp_path: Path) -> list[float]:
    ...
    generated = tmp_path / "VerifiedAdd.java"
    generated.write_text(path.read_text(encoding="utf-8"), encoding="utf-8")
    runner = tmp_path / "VerifiedAddRunner.java"
    calls = "\n".join(
        f'        System.out.printf("%.17g%n", VerifiedAdd.add_nonneg({c_float(sample["args"][0])}, {c_float(sample["args"][1])}));'
        for sample in samples
    )
    runner.write_text(
        "public final class VerifiedAddRunner {\n"
    ...
    if build["returnCode"] != 0:
        raise RuntimeError(build["outputExcerpt"])
    result = run([java, "-cp", str(tmp_path), "VerifiedAddRunner"], timeout=30)
    if result["returnCode"] != 0:
        raise RuntimeError(result["outputExcerpt"])
    return parse_stdout_values(result["stdout"])
...
def build_runtime_row(target: str, ext: str, tmp_path: Path) -> dict[str, Any]:
...
def _build_payload_cached() -> dict[str, Any]:
    fef_p36 = json.loads(FEF_P36_PATH.read_text(encoding="utf-8"))
    with tempfile.TemporaryDirectory(prefix="fef_p37_verified_add_runtime_") as tmp:
        rows = [build_runtime_row(target, ext, Path(tmp)) for target, ext in RUNTIME_TARGETS]
    payload = {
        "schemaVersion": SCHEMA_VERSION,
        "date": DATE,
        "status": STATUS,
        "artifactId": "fef-p37-verified-add-runtime-execution",
        "decision": "selected_verified_add_generated_targets_execute_and_match_reference",
        "sourceFixture": SOURCE_FIXTURE_ID,
        "sampleGrid": SAMPLES,
        "reference": {
            "function": "a + b",
            "values": reference_values(SAMPLES),
        },
        "runtimeRows": rows,
        "summary": summarize(rows),
        "fefP36Link": {
            "path": str(FEF_P36_PATH.relative_to(ROOT)),
            "reviewDecision": fef_p36["reviewDecision"],
        },
        "releaseGates": [
        ...
        "nextMilestones": [
            "Add runtime execution checks for runtime_helper_mix on the same installed toolchain set.",
            "Fold verified_add runtime execution rows into the selected capability matrix view.",
...
def build_evidence_packet(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "validationCommands": [
            "python python/scripts/fef_p37_verified_add_runtime_execution.py --build --strict",
            "python -m pytest -q python/tests/test_fef_p37_verified_add_runtime_execution.py",
        ],
    }
...
def build_command_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "schemaVersion": "monogate.command_feed.fef_p37_verified_add_runtime_execution.v0",
        "date": DATE,
        "title": "FEF-P37 Verified Add Runtime Execution",
        "status": payload["status"],
        "decision": payload["decision"],
        "summary": payload["summary"],
        "topFollowup": "Add runtime execution for runtime_helper_mix or fold verified_add runtime rows into the capability matrix.",
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def validate_payload(payload: dict[str, Any]) -> None:
    if payload["schemaVersion"] != SCHEMA_VERSION:
        raise ValueError("invalid FEF-P37 schema")
    summary = payload["summary"]
...
def build_outputs(out_dir: Path, report_dir: Path, evidence_dir: Path, command_feed_dir: Path) -> dict[str, Any]:
    ...
    result_path = out_dir / f"fef_p37_verified_add_runtime_execution_{STAMP}.json"
    report_path = report_dir / f"fef_p37_verified_add_runtime_execution_{STAMP}.md"
    evidence_path = evidence_dir / "fef_p37_verified_add_runtime_execution.json"
    feed_path = command_feed_dir / f"fef_p37_verified_add_runtime_execution_feed_{STAMP}.json"
...
def main() -> int:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/fef_p37_verified_add_runtime_execution")
    ...
    print("FEF_P37_VERIFIED_ADD_RUNTIME_EXECUTION_OK")
