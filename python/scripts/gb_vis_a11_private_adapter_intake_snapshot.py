#!/usr/bin/env python3
"""GB-VIS-A11 private adapter intake snapshot packet."""
...
from scripts import gb_vis_a10_private_adapter_intake_guard as a10  # noqa: E402

DATE = "2026-06-02"
STAMP = DATE.replace("-", "_")
SCHEMA_VERSION = "monogate.private_adapter_intake_snapshot.v0"
EVIDENCE_SCHEMA_VERSION = "monogate.evidence_public_packet.v0"
STATUS = "GB_VIS_A11_PRIVATE_ADAPTER_INTAKE_SNAPSHOT_PASS"

CLAIM_FLAGS = {
    "private_adapter_intake_snapshot_recorded": True,
    "gb_vis_a10_adapter_intake_guard_consumed": True,
...
NON_CLAIMS = [
    "GB-VIS-A11 records a private adapter intake snapshot only; it does not implement, execute, or pixel-render a renderer.",
    "GB-VIS-A11 snapshots GB-VIS-A10 intake guard evidence without accepting a laptop artifact, proving visualization correctness, renderer soundness, validator soundness, a Galois connection, abstract interpretation correctness, compiler correctness, formal equivalence, or full EML semantics.",
    "GB-VIS-A11 does not update public surfaces, runtime behavior, MachLib, production validators, laptop-owned repos, electronics repos, or course artifacts.",
]
...
def build_snapshot(source: dict[str, Any], snapshot_id: str) -> dict[str, Any]:
    return {
        ...
        "snapshotFormat": "monogate.private_adapter_intake_snapshot.v0",
        "sourceAdapterIntakeGuard": source["artifactId"],
...
def snapshot_checks(baseline: dict[str, Any], observed: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for check in checks:
        if check["baseline"] != check["observed"]:
            raise ValueError(f"private adapter intake snapshot mismatch: {check['field']}")
    return checks
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    ...
    baseline = build_snapshot(source, "gb_vis_a11_baseline_private_adapter_intake_snapshot")
    observed = build_snapshot(source, "gb_vis_a11_observed_private_adapter_intake_snapshot")
    ...
    summary = {
        ...
        "privateAdapterIntakeSnapshotRecorded": True,
        "gbVisA10AdapterIntakeGuardConsumed": True,
        ...
        "publicReady": False,
        "nextAction": "ACT-A12 reviewer intake feed guard or GB-VIS-A12 private adapter intake feed guard without public promotion.",
    ...
    return {
        ...
        "packetType": "private_adapter_intake_snapshot_v0",
        "artifactId": "gb-vis-a11-private-adapter-intake-snapshot",
        "status": STATUS,
        "decision": "record_private_adapter_intake_snapshot_no_renderer_execution_no_public_promotion",
        "date": DATE,
        "sourceAdapterIntakeGuard": source["artifactId"],
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceAdapterIntakeGuard"] != "gb-vis-a10-private-adapter-intake-guard":
        raise ValueError("GB-VIS-A11 must consume GB-VIS-A10")
    ...
    if summary["sourceClaimFlagCount"] != 34 or summary["blockedSourceClaimFlagCount"] != 21:
    ...
    for check in payload["snapshotChecks"]:
        if check["status"] != "pass" or check["baseline"] != check["observed"]:
            raise ValueError("private adapter intake snapshot check must pass exactly")
    for key in [
        "privateAdapterIntakeSnapshotRecorded",
        "gbVisA10AdapterIntakeGuardConsumed",
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "artifactType": "private_adapter_intake_snapshot",
        "validationStatus": "pass",
        "semanticStrength": "private_adapter_intake_snapshot_no_renderer_execution_no_public_update",
        "source": f"python/results/gb_vis_a11_private_adapter_intake_snapshot/gb_vis_a11_private_adapter_intake_snapshot_{STAMP}.json",
        "summary": payload["summary"],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "feedId": "gb_vis_a11_private_adapter_intake_snapshot_feed",
        "date": DATE,
...
def build_report(payload: dict[str, Any]) -> str:
    lines = [
        "# GB-VIS-A11 Private Adapter Intake Snapshot",
        "",
        f"Status: `{payload['status']}`",
        "",
        "GB-VIS-A11 records a private adapter intake snapshot without implementing or executing a renderer.",
        "",
        "| Count | Value |",
        "|---|---|",
...
def build_outputs(out_dir: Path, report_dir: Path, evidence_dir: Path, command_feed_dir: Path, atlas_gate_path: Path) -> dict[str, Any]:
    ...
    result_path = out_dir / f"gb_vis_a11_private_adapter_intake_snapshot_{STAMP}.json"
    report_path = report_dir / f"gb_vis_a11_private_adapter_intake_snapshot_{STAMP}.md"
    evidence_path = evidence_dir / "gb_vis_a11_private_adapter_intake_snapshot.json"
    feed_path = command_feed_dir / f"gb_vis_a11_private_adapter_intake_snapshot_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/gb_vis_a11_private_adapter_intake_snapshot")
...
def main() -> int:
    ...
    print("GB_VIS_A11_PRIVATE_ADAPTER_INTAKE_SNAPSHOT_OK")
    return 0
