#!/usr/bin/env python3
"""EML-D93 log1p affine-scaled boundary coordinate MachLib witness attempt."""
...
from scripts import eml_d92_log1p_affine_scaled_boundary_coordinate_feasibility_packet as d92  # noqa: E402

DATE = "2026-06-04"
STAMP = DATE.replace("-", "_")
SCHEMA_VERSION = "monogate.eml_d93_log1p_affine_scaled_boundary_coordinate_witness_attempt.v0"
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    feasibility = d92.build_payload(atlas_gate_path)
    d92.validate_payload(feasibility)
    ...
    checked_witness = {
        ...
        "proofShape": [
            "specialize atlas_subtraction_boundary_witness at 1 + a * x and 1",
            "retain 0 < 1 + a * x as explicit domain guard",
            "rewrite eml (log (1 + a * x)) (exp 1) to (1 + a * x) - 1",
    ...
    summary = {
        ...
        "sourceDuplicateShiftedBlocksPreserved": feasibility["summary"]["duplicateShiftedBlocksPreserved"],
        ...
        "publicReady": False,
        "nextArtifact": "EML-D94 log1p affine-scaled checked-witness private surface review",
    ...
    return {
        ...
        "attemptType": "eml_d93_log1p_affine_scaled_boundary_coordinate_witness_attempt_v0",
        "artifactId": "eml-d93-log1p-affine-scaled-boundary-coordinate-witness-attempt",
        "status": STATUS,
        "decision": "checked_log1p_affine_scaled_boundary_coordinate_witness",
        "date": DATE,
        "sourceFeasibilityPacket": feasibility["artifactId"],
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    expected_statement = "0 < 1 + a * x -> eml (log (1 + a * x)) (exp 1) = a * x"
    if payload["schemaVersion"] != SCHEMA_VERSION:
        raise ValueError("unexpected schema")
    if payload["sourceFeasibilityPacket"] != "eml-d92-log1p-affine-scaled-boundary-coordinate-feasibility-packet":
        raise ValueError("D93 must consume D92")
    ...
    if summary["sourceProposedMachlibName"] != "MachLib.Real.log1p_affine_scaled_boundary_coordinate_witness":
    ...
    if summary["sourceDuplicateShiftedBlocksPreserved"] is not True:
        raise ValueError("source duplicate block drifted")
    if summary["machlibName"] != "MachLib.Real.log1p_affine_scaled_boundary_coordinate_witness":
    ...
    if summary["nextArtifact"] != "EML-D94 log1p affine-scaled checked-witness private surface review":
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "artifactType": "eml_d93_log1p_affine_scaled_boundary_coordinate_witness_attempt",
        "validationStatus": "pass",
        "semanticStrength": "private_checked_guarded_machlib_witness_no_runtime_or_public_claim",
        "source": f"python/results/eml_d93_log1p_affine_scaled_boundary_coordinate_witness_attempt/eml_d93_log1p_affine_scaled_boundary_coordinate_witness_attempt_{STAMP}.json",
        "summary": payload["summary"],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "feedId": "eml_d93_log1p_affine_scaled_boundary_coordinate_witness_attempt_feed",
        "date": DATE,
        ...
        "nextAction": "Run EML-D94 as a private checked-witness surface review; do not promote public copy.",
        "claimFlags": dict(CLAIM_FLAGS),
    }
...
def build_outputs(
    ...
    result_path = out_dir / f"eml_d93_log1p_affine_scaled_boundary_coordinate_witness_attempt_{STAMP}.json"
    report_path = report_dir / f"eml_d93_log1p_affine_scaled_boundary_coordinate_witness_attempt_{STAMP}.md"
    evidence_path = evidence_dir / "eml_d93_log1p_affine_scaled_boundary_coordinate_witness_attempt.json"
    feed_path = command_feed_dir / f"eml_d93_log1p_affine_scaled_boundary_coordinate_witness_attempt_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=ROOT / "python/results/eml_d93_log1p_affine_scaled_boundary_coordinate_witness_attempt",
    )
...
def main() -> int:
    ...
    print("EML_D93_LOG1P_AFFINE_SCALED_BOUNDARY_COORDINATE_WITNESS_ATTEMPT_OK")
    return 0
