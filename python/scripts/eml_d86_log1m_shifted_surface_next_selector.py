#!/usr/bin/env python3
"""EML-D86 log1m-shifted private surface next-action selector."""
...
from scripts import eml_d85_log1m_shifted_witness_surface_review as d85  # noqa: E402

DATE = "2026-06-04"
STAMP = DATE.replace("-", "_")
SCHEMA_VERSION = "monogate.eml_log1m_shifted_surface_next_selector.v0"
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    surface = d85.build_payload(atlas_gate_path)
    d85.validate_payload(surface)
    runtime_row = surface_row_by_id(surface, "log1m_shifted_runtime_control_guardrail")
    guard_row = surface_row_by_id(surface, "log1m_shifted_guard_boundary")
    public_row = surface_row_by_id(surface, "public_atlas_log1m_shifted_boundary")
    options = [
        selector_option(
            "log1m_shifted_checked_witness_copy_review_packet",
            "private_copy_review_lane",
            "selected_next",
            87,
            "EML-D87 log1m-shifted checked-witness copy review packet",
            [
                "D84 checked the witness and D85 surfaced five private rows, but public-safe wording is still unreviewed.",
                "A copy-review packet can freeze allowed phrases, guard caveats, runtime caveats, duplicate-log1p caveats, and blocked claims before any public gate is considered.",
                "This keeps the checked witness useful for reviewers while preserving protected log and log1p as runtime controls.",
            ],
            [
                "must preserve the shifted positive-domain guard 0 < 1 - x",
                "must preserve protected log/log1p runtime-control language",
                "must preserve the duplicate-log1p block",
                "must block log, log1p, runtime replacement, and runtime advantage claims",
        ...
        selector_option(
            ...
            [
                "A next bounded branch remains valuable after the log1m-shifted witness wording is stabilized.",
                "Starting another branch first would leave the checked log1m-shifted witness with only surface rows and no copy boundary.",
            ],
            [
                "requires exactly one non-duplicate candidate",
                "must not start MachLib proof work in the selector",
                "must preserve D85 public, runtime, and duplicate-log1p holds",
            ],
        ),
        selector_option(
            ...
            [
                ...
                "It carries higher guard and negative-control risk than stabilizing the already checked log1m-shifted witness copy.",
            ],
            [
        ...
        selector_option(
            ...
            [
                "D85 records public Atlas as held private.",
                "Human approval should come after private copy review, not before the guard, runtime, and duplicate-log1p caveats are frozen.",
            ],
            [
    ...
    summary = {
        ...
        "sourceDuplicateLog1pBlockPreserved": surface["summary"]["sourceDuplicateLog1pBlockPreserved"],
        "duplicateLog1pBlockPreserved": surface["summary"]["duplicateLog1pBlockPreserved"],
        "d85SurfaceRowCount": surface["summary"]["surfaceRowCount"],
        "d85CheckedWitnessRecordedPrivately": surface["summary"]["checkedWitnessRecordedPrivately"],
        "d85CandidateProved": surface["summary"]["candidateProved"],
        "d85BuildPassed": surface["summary"]["buildPassed"],
    ...
    return {
        ...
        "selectorType": "eml_log1m_shifted_surface_next_selector_v0",
        "artifactId": "eml-d86-log1m-shifted-surface-next-selector",
        "status": STATUS,
        "decision": "select_log1m_shifted_checked_witness_copy_review_packet",
        "date": DATE,
        "sourceSurfaceReview": surface["artifactId"],
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceSurfaceReview"] != "eml-d85-log1m-shifted-witness-surface-review":
        raise ValueError("D86 must consume D85")
    ...
    if summary["sourceDuplicateLog1pBlockPreserved"] is not True or summary["duplicateLog1pBlockPreserved"] is not True:
        raise ValueError("duplicate-log1p block drift")
    if summary["d85SurfaceRowCount"] != 5:
        raise ValueError("expected five D85 rows")
    for key in ["d85CheckedWitnessRecordedPrivately", "d85CandidateProved", "d85BuildPassed"]:
    ...
    if summary["selectedOptionId"] != "log1m_shifted_checked_witness_copy_review_packet":
        raise ValueError("unexpected selected option")
    if summary["selectedNextArtifact"] != "EML-D87 log1m-shifted checked-witness copy review packet":
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "artifactType": "eml_log1m_shifted_surface_next_selector",
        "validationStatus": "pass",
        "semanticStrength": "private_selector_checked_witness_copy_review_next_no_public_update",
        "source": f"python/results/eml_d86_log1m_shifted_surface_next_selector/eml_d86_log1m_shifted_surface_next_selector_{STAMP}.json",
        "summary": payload["summary"],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "feedId": "eml_d86_log1m_shifted_surface_next_selector_feed",
        "date": DATE,
        "status": payload["status"],
        "selectedOptionId": payload["summary"]["selectedOptionId"],
        "selectedNextArtifact": payload["summary"]["selectedNextArtifact"],
        "nextAction": "Run EML-D87 as a private log1m-shifted checked-witness copy review packet; do not promote public copy.",
        "claimFlags": dict(CLAIM_FLAGS),
    }
...
def build_outputs(
    ...
    result_path = out_dir / f"eml_d86_log1m_shifted_surface_next_selector_{STAMP}.json"
    report_path = report_dir / f"eml_d86_log1m_shifted_surface_next_selector_{STAMP}.md"
    evidence_path = evidence_dir / "eml_d86_log1m_shifted_surface_next_selector.json"
    feed_path = command_feed_dir / f"eml_d86_log1m_shifted_surface_next_selector_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/eml_d86_log1m_shifted_surface_next_selector")
...
def main() -> int:
    ...
    print("EML_D86_LOG1M_SHIFTED_SURFACE_NEXT_SELECTOR_OK")
    return 0
