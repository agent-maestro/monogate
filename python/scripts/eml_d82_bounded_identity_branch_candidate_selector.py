...
from scripts import eml_d81_post_log1p_shifted_pause_next_selector as d81  # noqa: E402
...
SELECTED_NEXT_ARTIFACT = "EML-D83 log1m shifted boundary coordinate feasibility packet"
SELECTED_STATEMENT = "0 < 1 - x -> eml (log (1 - x)) (exp 1) = -x"
...
CLAIM_FLAGS = {
    ...
    "source_d81_selector_observed": True,
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    source = d81.build_payload(atlas_gate_path)
    d81.validate_payload(source)
    ...
    candidates = [
        candidate_row(
            ...
            [
                "It mirrors the checked log1p-shifted lane with the opposite shifted boundary, giving the next phase a bounded and guard-explicit feasibility question.",
                "The candidate is syntactically fresh relative to the checked log1p-shifted witness and can be evaluated before any proof or MachLib edit begins.",
                "The protected log/log1p runtime controls remain untouched while the candidate is only selected for feasibility review.",
            ],
            [
                "requires D83 feasibility packet before any proof attempt",
        ...
        candidate_row(
            ...
            [
                "D80 already froze this checked witness copy for handoff stability.",
                "Reselecting it as fresh candidate work would duplicate the current MachLib witness lane.",
            ],
            [
                "already checked as MachLib.Real.log1p_shifted_boundary_coordinate_witness",
                "already frozen by D80 and observed by D81",
            ],
        ...
        candidate_row(
            ...
            [
                ...
                "It should wait until the simpler shifted-boundary candidate receives feasibility review.",
            ],
            [
    ...
    duplicate = next(candidate for candidate in candidates if candidate["candidateId"] == "log1p_shifted_boundary_coordinate")
    summary = {
        ...
        "sourceD80FrozenWitnessName": source_summary["frozenWitnessName"],
        "sourceD80FrozenCheckedStatement": source_summary["frozenCheckedStatement"],
        "sourceD80FrozenGuards": list(source_summary["frozenGuards"]),
        ...
        "log1mShiftedBoundaryCandidateSelected": True,
        "freshCandidateNonDuplicateSelected": True,
        "sourceD81SelectorObserved": True,
    ...
    return {
        ...
        "artifactId": "eml-d82-bounded-identity-branch-candidate-selector",
        "status": STATUS,
        "decision": "select_log1m_shifted_boundary_coordinate_for_later_feasibility",
        "date": DATE,
        "sourceSelector": source["artifactId"],
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceSelector"] != "eml-d81-post-log1p-shifted-pause-next-selector":
        raise ValueError("D82 must consume D81")
    if summary["sourceSelectedOptionId"] != "next_bounded_identity_branch_selector":
        raise ValueError("D81 must have selected the bounded identity branch selector")
    if summary["sourceSelectedNextArtifact"] != "EML-D82 bounded identity branch candidate selector":
        raise ValueError("D81 next artifact drift")
    ...
    if summary["selectedFamily"] != "guarded_log1m_shifted_coordinate":
    ...
    if summary["selectedGuardCount"] != 1 or summary["selectedGuards"] != ["0 < 1 - x"]:
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "semanticStrength": "private_selector_fresh_log1m_shifted_candidate_no_feasibility_no_proof_no_runtime_no_public",
        "source": f"python/results/eml_d82_bounded_identity_branch_candidate_selector/eml_d82_bounded_identity_branch_candidate_selector_{STAMP}.json",
        "summary": payload["summary"],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "feedId": "eml_d82_bounded_identity_branch_candidate_selector_feed",
        "date": DATE,
        "status": payload["status"],
        "selectedCandidateId": payload["summary"]["selectedCandidateId"],
        "selectedNextArtifact": payload["summary"]["selectedNextArtifact"],
        "blockedDuplicateCandidateId": payload["summary"]["blockedDuplicateCandidateId"],
        "nextAction": "Run EML-D83 as a log1m-shifted boundary coordinate feasibility packet.",
        "claimFlags": dict(CLAIM_FLAGS),
    }
...
def build_report(payload: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- blocked duplicate: `{payload['summary']['blockedDuplicateCandidateId']}`",
            f"- source frozen witness: `{payload['summary']['sourceD80FrozenWitnessName']}`",
...
def build_outputs(
    ...
    result_path = out_dir / f"eml_d82_bounded_identity_branch_candidate_selector_{STAMP}.json"
    report_path = report_dir / f"eml_d82_bounded_identity_branch_candidate_selector_{STAMP}.md"
    evidence_path = evidence_dir / "eml_d82_bounded_identity_branch_candidate_selector.json"
    feed_path = command_feed_dir / f"eml_d82_bounded_identity_branch_candidate_selector_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/eml_d82_bounded_identity_branch_candidate_selector")
...
def main() -> int:
    ...
    print("EML_D82_BOUNDED_IDENTITY_BRANCH_CANDIDATE_SELECTOR_OK")
    return 0
