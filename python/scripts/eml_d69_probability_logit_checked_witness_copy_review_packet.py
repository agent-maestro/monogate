...
from scripts import eml_d68_probability_logit_surface_next_selector as d68  # noqa: E402

DATE = "2026-06-03"
STAMP = DATE.replace("-", "_")
SCHEMA_VERSION = "monogate.eml_probability_logit_checked_witness_copy_review_packet.v0"
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    selector = d68.build_payload(atlas_gate_path)
    d68.validate_payload(selector)
    row_caveats = [
        "Always describe this as one scoped guarded checked witness, not a logit theory.",
        "Preserve the checked statement with both guards: 0 < p and p < 1.",
        "Say protected log and log1p remain the runtime controls.",
        "Keep D65 boundary controls visible: p = 0, p = 1, and unguarded statements remain blocked.",
    ...
    row = witness_copy_row(
        ...
        "A checked MachLib witness records the scoped guarded identity 0 < p -> p < 1 -> eml (log p) (exp (log (1 - p))) = p - log (1 - p); protected log and log1p remain runtime controls.",
        row_caveats,
        row_blocked_phrases,
    ...
    required_caveats = [
        ...
        "Both probability interval guards, 0 < p and p < 1, must remain visible.",
    ...
    summary = {
        ...
        "publicReady": False,
        "nextAction": "EML-D70 choose probability-logit branch pause/freeze, next bounded branch, or human-approved public copy gate.",
    ...
    return {
        ...
        "reviewType": "eml_probability_logit_checked_witness_copy_review_packet_v0",
        "artifactId": "eml-d69-probability-logit-checked-witness-copy-review-packet",
        "status": STATUS,
        "decision": "probability_logit_checked_witness_copy_review_private_only_public_copy_held",
        "date": DATE,
        "sourceSelector": selector["artifactId"],
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceSelector"] != "eml-d68-probability-logit-surface-next-selector":
        raise ValueError("D69 must consume D68")
    ...
    if summary["guardCount"] != 2:
        raise ValueError("probability-logit copy review should preserve two guards")
    ...
    if summary["nextAction"] != "EML-D70 choose probability-logit branch pause/freeze, next bounded branch, or human-approved public copy gate.":
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "artifactType": "eml_probability_logit_checked_witness_copy_review_packet",
        "validationStatus": "pass",
        "semanticStrength": "private_probability_logit_checked_witness_copy_review_public_copy_held",
        "source": f"python/results/eml_d69_probability_logit_checked_witness_copy_review_packet/eml_d69_probability_logit_checked_witness_copy_review_packet_{STAMP}.json",
        "summary": payload["summary"],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "feedId": "eml_d69_probability_logit_checked_witness_copy_review_packet_feed",
        "date": DATE,
...
def build_outputs(out_dir: Path, report_dir: Path, evidence_dir: Path, command_feed_dir: Path, atlas_gate_path: Path) -> dict[str, Any]:
    ...
    result_path = out_dir / f"eml_d69_probability_logit_checked_witness_copy_review_packet_{STAMP}.json"
    report_path = report_dir / f"eml_d69_probability_logit_checked_witness_copy_review_packet_{STAMP}.md"
    evidence_path = evidence_dir / "eml_d69_probability_logit_checked_witness_copy_review_packet.json"
    feed_path = command_feed_dir / f"eml_d69_probability_logit_checked_witness_copy_review_packet_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/eml_d69_probability_logit_checked_witness_copy_review_packet")
...
def main() -> int:
    ...
    print("EML_D69_PROBABILITY_LOGIT_CHECKED_WITNESS_COPY_REVIEW_PACKET_OK")
    return 0
