#!/usr/bin/env python3
"""EML-D85 log1m shifted checked-witness private surface review."""
...
from scripts import eml_d84_log1m_shifted_boundary_coordinate_witness_attempt as d84  # noqa: E402

DATE = "2026-06-04"
STAMP = DATE.replace("-", "_")
SCHEMA_VERSION = "monogate.eml_log1m_shifted_witness_surface_review.v0"
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    witness = d84.build_payload(atlas_gate_path)
    d84.validate_payload(witness)
    surface_rows = [
        surface_row(
            "machlib_witness_index_log1m_shifted_boundary",
            "machlib_private_index",
            "checked_witness_recorded_private",
            "scoped_guarded_log1m_shifted_coordinate_checked",
            "record_as_private_checked_witness",
            [
                "D84 checks MachLib.Real.log1m_shifted_boundary_coordinate_witness.",
                "The checked statement is guarded by 0 < 1 - x.",
                "The witness is scoped to one shifted logarithm coordinate and does not create a broad log1m or log1p theory.",
            ],
            ["public readiness", "theorem discovery", "broad log1m theory"],
        ),
        surface_row(
            "log1m_shifted_guard_boundary",
            "candidate_boundary",
            "shifted_positive_domain_boundary_required",
            "d83_negative_controls_and_blockers_preserved",
            "keep_shifted_positive_guard_and_boundary_controls",
            [
                "D83 records one guard, two derived domain obligations, four negative controls, and four blockers.",
                "D84 preserves the guard shape 0 < 1 - x.",
                "Boundary cases x = 1, x > 1, and unguarded statements remain blocked.",
            ],
            ["unguarded log1m-shifted coordinate", "x = 1 boundary", "x > 1 boundary"],
        ),
        surface_row(
            "log1m_shifted_runtime_control_guardrail",
            ...
            [
                "D83 and D84 keep protected_log_and_log1p_remain_runtime_controls.",
        ...
        surface_row(
            "advantage_lab_log1m_shifted_boundary",
            "advantage_lab",
            "runtime_control_remains_protected_log_and_log1p",
            "scoped_log1m_shifted_coordinate_witness_available",
        ...
        surface_row(
            "public_atlas_log1m_shifted_boundary",
            "public_surface",
            "held_private",
            "checked_machlib_witness_available_private",
            "require_human_copy_review_before_public_change",
            [
                "D84 proof status is real but public wording has not been reviewed.",
                "No monogate.org or monogate.dev public route is changed by D85.",
                "Safe public wording must preserve guard, runtime-control, duplicate-log1p, and non-broadness caveats.",
            ],
    ...
    summary = {
        ...
        "sourceDuplicateLog1pBlockPreserved": witness["summary"]["sourceDuplicateLog1pBlockPreserved"],
        "duplicateLog1pBlockPreserved": witness["summary"]["duplicateLog1pBlockPreserved"],
        ...
        "publicReady": False,
        "nextAction": "EML-D86 choose log1m-shifted checked-witness copy review, next bounded branch, or human-approved public copy gate without public promotion.",
        "claimFlagsAllFalse": all(value is False for value in CLAIM_FLAGS.values())
        and all(all(value is False for value in row["claimFlags"].values()) for row in surface_rows),
    }
    return {
        "schemaVersion": SCHEMA_VERSION,
        "reviewType": "eml_log1m_shifted_witness_surface_review_v0",
        "artifactId": "eml-d85-log1m-shifted-witness-surface-review",
        "status": STATUS,
        "decision": "surface_checked_log1m_shifted_witness_as_private_review_evidence_only",
        "date": DATE,
        "sourceWitnessAttempt": witness["artifactId"],
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceWitnessAttempt"] != "eml-d84-log1m-shifted-boundary-coordinate-witness-attempt":
        raise ValueError("D85 must consume D84")
    ...
    for key in ["checkedWitnessRecordedPrivately", "candidateProved", "buildPassed", "duplicateLog1pBlockPreserved"]:
    ...
    if summary["guardCount"] != 1:
        raise ValueError("log1m-shifted witness should preserve one guard")
    if summary["sourceDerivedDomainObligationCount"] != 2:
        raise ValueError("D83 derived domain obligation count drift")
    if summary["sourceNegativeControlCount"] != 4 or summary["sourceBlockerCount"] != 4:
        raise ValueError("D83 negative control/blocker counts drift")
    for key in [
    ...
    if summary["nextAction"] != "EML-D86 choose log1m-shifted checked-witness copy review, next bounded branch, or human-approved public copy gate without public promotion.":
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "artifactType": "eml_log1m_shifted_witness_surface_review",
        "validationStatus": "pass",
        "semanticStrength": "checked_log1m_shifted_witness_private_surface_review_no_public_update",
        "source": f"python/results/eml_d85_log1m_shifted_witness_surface_review/eml_d85_log1m_shifted_witness_surface_review_{STAMP}.json",
        "summary": payload["summary"],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "feedId": "eml_d85_log1m_shifted_witness_surface_review_feed",
        "date": DATE,
...
def build_outputs(
    ...
    result_path = out_dir / f"eml_d85_log1m_shifted_witness_surface_review_{STAMP}.json"
    report_path = report_dir / f"eml_d85_log1m_shifted_witness_surface_review_{STAMP}.md"
    evidence_path = evidence_dir / "eml_d85_log1m_shifted_witness_surface_review.json"
    feed_path = command_feed_dir / f"eml_d85_log1m_shifted_witness_surface_review_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/eml_d85_log1m_shifted_witness_surface_review")
...
def main() -> int:
    ...
    print("EML_D85_LOG1M_SHIFTED_WITNESS_SURFACE_REVIEW_OK")
    return 0
