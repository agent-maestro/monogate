#!/usr/bin/env python3
"""FEF-P54 selected C if/else clamp branch lowering gate."""
...
SCHEMA_VERSION = "monogate.fef_p54_c_if_else_clamp_branch_lowering_gate.v0"
PACKET_SCHEMA_VERSION = "monogate.fef_p54_c_if_else_clamp_reingest_packet.v0"
EVIDENCE_SCHEMA_VERSION = "monogate.evidence_public_packet.v0"
STATUS = "FEF_P54_C_IF_ELSE_CLAMP_BRANCH_LOWERING_GATE_PASS"
...
CLAIM_FLAGS = {
    "selected_c_if_else_clamp_lowering_claim": False,
...
NON_CLAIMS = [
    "FEF-P54 records a selected C if/else clamp lowering and re-ingest gate only.",
    "FEF-P54 does not claim general C branch/control-flow support.",
    "FEF-P54 does not claim C if-statement support.",
    "FEF-P54 does not claim Rust if support.",
    "FEF-P54 does not claim full non-generated source roundtrip.",
    "FEF-P54 does not claim full arbitrary C/Rust source roundtrip.",
    "FEF-P54 does not record reviewer approval or rejection.",
    "FEF-P54 does not publish a package.",
    "FEF-P54 does not enable checkout or commerce.",
    "FEF-P54 does not claim public readiness.",
    "FEF-P54 does not claim compiler correctness or formal semantic equivalence.",
    "FEF-P54 does not claim runtime performance.",
    "FEF-P54 does not claim all-free-target runtime execution or all-free-target roundtrip.",
    "FEF-P54 does not claim Verilog, Lean proof, zkproof, silicon, hardware, Pro-target, production, or all-target readiness.",
]
...
def compare_target(target: str, eml_path: Path, source_eml: str, tmp_path: Path) -> dict[str, Any]:
    ...
    if target == "c":
        ...
        raise ValueError(f"unsupported FEF-P54 target: {target}")
    ...
    packet = {
        ...
        "packetType": "fef_p54_c_if_else_clamp_reingest_packet_v0",
        "date": DATE,
        "caseId": f"{CASE_ID}_{target}_reingest",
        "sourceCaseId": CASE_ID,
        "sourceLanguage": "c",
        "sourceFeature": "if_else_clamp",
        ...
        "frames": frames,
        "evidenceKind": "selected_c_if_else_clamp_source_derived_generated_target_reingest",
...
def build_payload() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="fef_p54_c_if_else_clamp_") as tmp:
        tmp_path = Path(tmp)
    ...
    payload = {
        ...
        "date": DATE,
        "status": STATUS,
        "artifactId": "fef-p54-c-if-else-clamp-branch-lowering-gate",
        "decision": "selected_c_if_else_clamp_lowering_reingest_passed_general_branch_blocked",
        "sourceFixture": {
            "caseId": CASE_ID,
            "sourceLanguage": "c",
        ...
        "releaseGates": [
            {"id": "selected_c_if_else_clamp_lowering", "status": "pass"},
            {"id": "selected_c_if_else_clamp_generated_c_reingest", "status": "pass"},
            {"id": "selected_c_if_else_clamp_generated_rust_reingest", "status": "pass"},
        ...
        "allowedPrivateClaims": [
            "One selected C if/else clamp fixture now lowers to a guarded affine selector.",
            "The selected C if/else clamp fixture emits generated C/Rust targets that re-ingest through eFrog and recompile to Python.",
            "The selected C/Rust generated target runtimes match re-ingested Python outputs over 14 packet-sample comparisons.",
            "P54 closes the P51 selected C if/else clamp blocker only; arbitrary C if-statement forms and Rust if remain blocked.",
        ],
        "blockedClaims": [
        ...
        "nextMilestones": [
            "Add Rust if-expression lowering under a separate gate.",
...
def summarize(packets: list[dict[str, Any]], p51_summary: dict[str, Any]) -> dict[str, Any]:
    return {
        "selectedCIfElseClampLoweringPass": True,
        "sourceCaseCount": 1,
        ...
        "cIfElseClampBlockerClosed": True,
...
def build_evidence_packet(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "title": "FEF-P54 C If/Else Clamp Branch Lowering Gate",
        "reviewDecision": payload["decision"],
        "validationStatus": "pass",
        "semanticStrength": "selected_c_if_else_clamp_lowering_reingest_general_branch_blocked",
        "semanticReview": payload["summary"],
        "claimBoundary": "Selected C if/else clamp lowering and re-ingest only; no general C branch/control-flow support, C if-statement support, Rust if support, full source roundtrip, arbitrary source-family, package publication, checkout, public readiness, compiler correctness, formal equivalence, runtime performance, all-free-target runtime, all-free-target roundtrip, hardware, silicon, or proof claim.",
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
        "reviewHighlights": [
            "P54 closes the selected P51 C if/else clamp blocker with an affine selector plus step01 lowering.",
            ...
            "Arbitrary C if-statement forms and Rust if remain blocked.",
        ],
        "validationCommands": [
            "python python/scripts/fef_p54_c_if_else_clamp_branch_lowering_gate.py --build --strict",
            "python -m pytest -q python/tests/test_fef_p54_c_if_else_clamp_branch_lowering_gate.py",
        ],
    }
...
def build_command_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "schemaVersion": "monogate.command_feed.fef_p54_c_if_else_clamp_branch_lowering_gate.v0",
        "date": DATE,
        "title": "FEF-P54 C If/Else Clamp Branch Lowering Gate",
        "status": payload["status"],
        "decision": payload["decision"],
        "summary": payload["summary"],
        "topFollowup": "Implement Rust if-expression lowering or record private reviewer response over P47-P54.",
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def render_report(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# FEF-P54 C If/Else Clamp Branch Lowering Gate",
            "",
            f"Date: {DATE}",
            "",
            ...
            "",
            "FEF-P54 closes one narrow branch blocker: selected C if/else clamp expressions.",
            "",
            *rows,
            "",
            "## Summary",
            "",
            f"- Selected C if/else clamp lowering pass: `{summary['selectedCIfElseClampLoweringPass']}`",
            ...
            *[f"- {claim}" for claim in payload["blockedClaims"]],
            ...
            "## Boundary",
            "",
            "- Selected C if/else clamp lowering and re-ingest only.",
...
def validate_packet(packet: dict[str, Any]) -> None:
    if packet["schemaVersion"] != PACKET_SCHEMA_VERSION:
        raise ValueError("invalid FEF-P54 packet schema")
    if packet["packetType"] != "fef_p54_c_if_else_clamp_reingest_packet_v0":
        raise ValueError("invalid FEF-P54 packet type")
    if packet["sourceLanguage"] != "c":
        raise ValueError("FEF-P54 source language must be C")
    if packet["sourceFeature"] != "if_else_clamp":
        raise ValueError("FEF-P54 source feature must be if_else_clamp")
    if packet["generatedTargetLanguage"] not in {"c", "rust"}:
        raise ValueError("FEF-P54 generated target must be C or Rust")
    if packet["recompiledTargetLanguage"] != "python":
        raise ValueError("FEF-P54 recompiled target must be Python")
    if packet["sourceFunctionCount"] != 2:
        raise ValueError("FEF-P54 source EML should include clamp and step01")
    if packet["reingestedFunctionCount"] != 2:
        raise ValueError("FEF-P54 re-ingested target should include clamp and step01")
    ...
    for frame in packet["frames"]:
        ...
        if frame["absErrorGeneratedVsExpectedReference"] > ATOL:
            raise ValueError(f"{packet['caseId']} frame missed if/else clamp reference")
...
def validate_payload(payload: dict[str, Any]) -> None:
    if payload["schemaVersion"] != SCHEMA_VERSION:
        raise ValueError("invalid FEF-P54 schema")
    if payload["status"] != STATUS:
        raise ValueError("invalid FEF-P54 status")
    summary = payload["summary"]
    if summary["selectedCIfElseClampLoweringPass"] is not True:
        raise ValueError("selected C if/else clamp lowering should pass")
    if summary["sourceCaseCount"] != 1:
        raise ValueError("expected one selected C if/else clamp source case")
    if summary["packetCount"] != 2:
        raise ValueError("expected generated C and Rust re-ingest packets")
    if summary["passCount"] != 2:
        raise ValueError("all P54 packets must pass")
    if summary["packetSampleCount"] != 14:
        raise ValueError("unexpected P54 packet sample count")
    ...
    if summary["cIfElseClampBlockerClosed"] is not True:
        raise ValueError("C if/else clamp blocker should be closed")
    for key in [
...
def build_outputs(
    ...
    result_path = out_dir / f"fef_p54_c_if_else_clamp_branch_lowering_gate_{STAMP}.json"
    report_path = report_dir / f"fef_p54_c_if_else_clamp_branch_lowering_gate_{STAMP}.md"
    evidence_path = evidence_dir / "fef_p54_c_if_else_clamp_branch_lowering_gate.json"
    feed_path = command_feed_dir / f"fef_p54_c_if_else_clamp_branch_lowering_gate_feed_{STAMP}.json"
...
def main() -> int:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/fef_p54_c_if_else_clamp_branch_lowering_gate")
    parser.add_argument("--packet-dir", type=Path, default=ROOT / "python/results/fef_p54_c_if_else_clamp_reingest_packets")
    ...
    print("FEF_P54_C_IF_ELSE_CLAMP_BRANCH_LOWERING_GATE_OK")
