...
from scripts import eml_d61_expm1_boundary_copy_review_next_selector as d61  # noqa: E402

DATE = "2026-06-02"
STAMP = DATE.replace("-", "_")
SCHEMA_VERSION = "monogate.eml_expm1_boundary_branch_pause_freeze_packet.v0"
...
def freeze_row(
    ...
    non_duplicate_witness_name: str,
    ...
    return {
        ...
        "nonDuplicateWitnessName": non_duplicate_witness_name,
        "duplicatesExistingExpBranchWitness": False,
        "guards": [],
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    selector = d61.build_payload(atlas_gate_path)
    d61.validate_payload(selector)
    frozen_caveats = [
    ...
    freeze_rows = [
        freeze_row(
            "expm1_boundary_identity_checked_copy",
            selector["summary"]["selectedWitnessName"],
            selector["summary"]["checkedStatement"],
            "MachLib.Real.atlas_exp_from_eml_witness",
            frozen_caveats,
            frozen_blocked_phrases,
    ...
    parked_options = [
        parked_option(
            ...
            "parked_after_expm1_boundary_pause",
            "Available only through a later selector after the expm1 checked-witness copy boundary is frozen.",
        ),
        parked_option(
        ...
        parked_option(
            ...
            "D62 freezes private copy boundaries but records no human approval for public use.",
        ),
    ]
    summary = {
        ...
        "sourceD60RequiredCaveatCount": selector["summary"]["d60RequiredCaveatCount"],
        "sourceD60BlockedGlobalPhraseCount": selector["summary"]["d60BlockedGlobalPhraseCount"],
        ...
        "publicReady": False,
        "nextAction": "EML-D63 select the next private post-pause action without public promotion.",
    ...
    return {
        ...
        "packetType": "eml_expm1_boundary_branch_pause_freeze_packet_v0",
        "artifactId": "eml-d62-expm1-boundary-branch-pause-freeze-packet",
        "status": STATUS,
        "decision": "pause_expm1_boundary_branch_and_freeze_checked_witness_copy",
        "date": DATE,
        "sourceSelector": selector["artifactId"],
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceSelector"] != "eml-d61-expm1-boundary-copy-review-next-selector":
        raise ValueError("D62 must consume D61")
    if summary["selectedOptionId"] != "expm1_boundary_pause_freeze_packet":
        raise ValueError("unexpected D61 selected option")
    if summary["selectedNextArtifact"] != "EML-D62 expm1-boundary branch pause and checked-witness copy freeze packet":
        raise ValueError("unexpected D61 next artifact")
    ...
    if summary["sourceD60RequiredCaveatCount"] != 8 or summary["sourceD60BlockedGlobalPhraseCount"] != 10:
    ...
    if summary["nextAction"] != "EML-D63 select the next private post-pause action without public promotion.":
    ...
    for row in payload["freezeRows"]:
        if row["machlibName"] != "MachLib.Real.expm1_boundary_identity_witness":
            raise ValueError("unexpected freeze row witness")
        if row["checkedStatement"] != "eml x (exp 1) = exp x - 1":
            raise ValueError("unexpected row checked statement")
        if row["guards"] != []:
            raise ValueError("expm1 freeze row should have no guards")
        if row["nonDuplicateWitnessName"] != "MachLib.Real.atlas_exp_from_eml_witness":
            raise ValueError("row non-duplicate boundary drift")
        if row["duplicatesExistingExpBranchWitness"] is not False:
            raise ValueError("row must remain non-duplicate")
        if row["runtimeControl"] != "protected_expm1_remains_runtime_control":
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "artifactType": "eml_expm1_boundary_branch_pause_freeze_packet",
        "validationStatus": "pass",
        "semanticStrength": "private_expm1_boundary_checked_witness_copy_frozen_public_copy_held_no_new_proof",
        "source": f"python/results/eml_d62_expm1_boundary_branch_pause_freeze_packet/eml_d62_expm1_boundary_branch_pause_freeze_packet_{STAMP}.json",
        "summary": payload["summary"],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "feedId": "eml_d62_expm1_boundary_branch_pause_freeze_packet_feed",
        "date": DATE,
...
def build_outputs(
    ...
    result_path = out_dir / f"eml_d62_expm1_boundary_branch_pause_freeze_packet_{STAMP}.json"
    report_path = report_dir / f"eml_d62_expm1_boundary_branch_pause_freeze_packet_{STAMP}.md"
    evidence_path = evidence_dir / "eml_d62_expm1_boundary_branch_pause_freeze_packet.json"
    feed_path = command_feed_dir / f"eml_d62_expm1_boundary_branch_pause_freeze_packet_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/eml_d62_expm1_boundary_branch_pause_freeze_packet")
...
def main() -> int:
    ...
    print("EML_D62_EXPM1_BOUNDARY_BRANCH_PAUSE_FREEZE_PACKET_OK")
    return 0
