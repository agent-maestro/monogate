#!/usr/bin/env python3
"""FEF-P55 selected Rust if expression branch lowering gate."""
...
SCHEMA_VERSION = "monogate.fef_p55_rust_if_expression_branch_lowering_gate.v0"
PACKET_SCHEMA_VERSION = "monogate.fef_p55_rust_if_expression_reingest_packet.v0"
...
CASE_ID = "rust_if_expr_relu_v0"
SAMPLES = [{"args": [x], "labels": ["x"]} for x in [-2.0, -1.0e-12, 0.0, 1.0e-12, 2.0]]

CLAIM_FLAGS = {
    "selected_rust_if_expression_lowering_claim": False,
...
NON_CLAIMS = [
    "FEF-P55 records a selected Rust if expression lowering and re-ingest gate only.",
    "FEF-P55 does not claim general Rust branch/control-flow support.",
    "FEF-P55 does not claim Rust if-return support.",
    "FEF-P55 does not claim full non-generated source roundtrip.",
    "FEF-P55 does not claim full arbitrary C/Rust source roundtrip.",
    "FEF-P55 does not record reviewer approval or rejection.",
    "FEF-P55 does not publish a package.",
    "FEF-P55 does not enable checkout or commerce.",
    "FEF-P55 does not claim public readiness.",
    "FEF-P55 does not claim compiler correctness or formal semantic equivalence.",
    "FEF-P55 does not claim runtime performance.",
    "FEF-P55 does not claim all-free-target runtime execution or all-free-target roundtrip.",
    "FEF-P55 does not claim Verilog, Lean proof, zkproof, silicon, hardware, Pro-target, production, or all-target readiness.",
]
...
def compare_target(target: str, eml_path: Path, source_eml: str, tmp_path: Path) -> dict[str, Any]:
    ...
    if target == "c":
        ...
        raise ValueError(f"unsupported FEF-P55 target: {target}")
    ...
    for frame in frames:
        ...
        frame["values"] = {
            ...
            "expectedRustIfExpressionReference": expected_value(x),
        }
        frame["absErrorGeneratedVsExpectedReference"] = abs(
            frame["values"]["sourceDerivedGeneratedTargetRuntime"]
            - frame["values"]["expectedRustIfExpressionReference"]
        )
    packet = {
        "schemaVersion": PACKET_SCHEMA_VERSION,
        "packetType": "fef_p55_rust_if_expression_reingest_packet_v0",
        "date": DATE,
        "caseId": f"{CASE_ID}_{target}_reingest",
        "sourceCaseId": CASE_ID,
        "sourceLanguage": "rust",
        "sourceFeature": "if_expression",
        ...
        "frames": frames,
        "evidenceKind": "selected_rust_if_expression_source_derived_generated_target_reingest",
...
def build_payload() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="fef_p55_rust_if_expression_") as tmp:
        tmp_path = Path(tmp)
    ...
    payload = {
        ...
        "date": DATE,
        "status": STATUS,
        "artifactId": "fef-p55-rust-if-expression-branch-lowering-gate",
        ...
        "releaseGates": [
            {"id": "selected_rust_if_expression_lowering", "status": "pass"},
            {"id": "selected_rust_if_expression_generated_c_reingest", "status": "pass"},
            {"id": "selected_rust_if_expression_generated_rust_reingest", "status": "pass"},
        ...
        "nextMilestones": [
            "Add Rust if-return lowering under a separate gate.",
...
def summarize(packets: list[dict[str, Any]], p51_summary: dict[str, Any]) -> dict[str, Any]:
    return {
        "selectedRustIfExpressionLoweringPass": True,
        "sourceCaseCount": 1,
        ...
        "rustIfExpressionBlockerClosed": True,
...
def build_evidence_packet(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "title": "FEF-P55 Rust If Expression Branch Lowering Gate",
        "reviewDecision": payload["decision"],
        "validationStatus": "pass",
        "semanticStrength": "selected_rust_if_expression_lowering_reingest_general_branch_blocked",
        "semanticReview": payload["summary"],
        "claimBoundary": "Selected Rust if expression lowering and re-ingest only; no general Rust branch/control-flow support, Rust if-return support, full source roundtrip, arbitrary source-family, package publication, checkout, public readiness, compiler correctness, formal equivalence, runtime performance, all-free-target runtime, all-free-target roundtrip, hardware, silicon, or proof claim.",
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
        "reviewHighlights": [
            "P55 closes the selected P51 Rust if expression blocker with an affine selector plus step01 lowering.",
            ...
            "Rust if-return remains blocked.",
        ],
        "validationCommands": [
            "python python/scripts/fef_p55_rust_if_expression_branch_lowering_gate.py --build --strict",
            "python -m pytest -q python/tests/test_fef_p55_rust_if_expression_branch_lowering_gate.py",
        ],
    }
...
def build_command_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "schemaVersion": "monogate.command_feed.fef_p55_rust_if_expression_branch_lowering_gate.v0",
        "date": DATE,
        "title": "FEF-P55 Rust If Expression Branch Lowering Gate",
        "status": payload["status"],
        "decision": payload["decision"],
        "summary": payload["summary"],
        "topFollowup": "Implement Rust if-return lowering or record private reviewer response over P47-P55.",
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
    ...
    return "\n".join(
        [
            ...
            *rows,
            ...
            "## Summary",
            "",
            f"- Selected Rust if expression lowering pass: `{summary['selectedRustIfExpressionLoweringPass']}`",
...
def validate_packet(packet: dict[str, Any]) -> None:
    if packet["schemaVersion"] != PACKET_SCHEMA_VERSION:
        raise ValueError("invalid FEF-P55 packet schema")
    if packet["packetType"] != "fef_p55_rust_if_expression_reingest_packet_v0":
        raise ValueError("invalid FEF-P55 packet type")
    if packet["sourceLanguage"] != "rust":
        raise ValueError("FEF-P55 source language must be Rust")
    if packet["sourceFeature"] != "if_expression":
        raise ValueError("FEF-P55 source feature must be if_expression")
    if packet["generatedTargetLanguage"] not in {"c", "rust"}:
        raise ValueError("FEF-P55 generated target must be C or Rust")
    if packet["recompiledTargetLanguage"] != "python":
        raise ValueError("FEF-P55 recompiled target must be Python")
    if packet["sourceFunctionCount"] != 2:
        raise ValueError("FEF-P55 source EML should include relu and step01")
    if packet["reingestedFunctionCount"] != 2:
        raise ValueError("FEF-P55 re-ingested target should include relu and step01")
    ...
    for frame in packet["frames"]:
        ...
        if frame["absErrorGeneratedVsExpectedReference"] > ATOL:
            raise ValueError(f"{packet['caseId']} frame missed if expression reference")
...
def validate_payload(payload: dict[str, Any]) -> None:
    if payload["schemaVersion"] != SCHEMA_VERSION:
        raise ValueError("invalid FEF-P55 schema")
    if payload["status"] != STATUS:
        raise ValueError("invalid FEF-P55 status")
    summary = payload["summary"]
    if summary["selectedRustIfExpressionLoweringPass"] is not True:
        raise ValueError("selected Rust if expression lowering should pass")
    if summary["sourceCaseCount"] != 1:
        raise ValueError("expected one selected Rust if expression source case")
    if summary["packetCount"] != 2:
        raise ValueError("expected generated C and Rust re-ingest packets")
    if summary["passCount"] != 2:
        raise ValueError("all P55 packets must pass")
    if summary["packetSampleCount"] != 10:
        raise ValueError("unexpected P55 packet sample count")
    ...
    if summary["rustIfExpressionBlockerClosed"] is not True:
        raise ValueError("Rust if expression blocker should be closed")
    for key in [
...
def build_outputs(
    ...
    result_path = out_dir / f"fef_p55_rust_if_expression_branch_lowering_gate_{STAMP}.json"
    report_path = report_dir / f"fef_p55_rust_if_expression_branch_lowering_gate_{STAMP}.md"
    evidence_path = evidence_dir / "fef_p55_rust_if_expression_branch_lowering_gate.json"
    feed_path = command_feed_dir / f"fef_p55_rust_if_expression_branch_lowering_gate_feed_{STAMP}.json"
...
def main() -> int:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/fef_p55_rust_if_expression_branch_lowering_gate")
    parser.add_argument("--packet-dir", type=Path, default=ROOT / "python/results/fef_p55_rust_if_expression_reingest_packets")
    ...
    print("FEF_P55_RUST_IF_EXPRESSION_BRANCH_LOWERING_GATE_OK")
