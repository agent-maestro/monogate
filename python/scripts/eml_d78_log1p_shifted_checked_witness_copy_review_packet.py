...
from scripts import eml_d77_log1p_shifted_surface_next_selector as d77  # noqa: E402

DATE = "2026-06-03"
STAMP = DATE.replace("-", "_")
SCHEMA_VERSION = "monogate.eml_log1p_shifted_checked_witness_copy_review_packet.v0"
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    selector = d77.build_payload(atlas_gate_path)
    d77.validate_payload(selector)
    row_caveats = [
        "Always describe this as one scoped guarded checked witness, not a log1p theory.",
        "Preserve the checked statement with the shifted positive-domain guard: 0 < 1 + x.",
        "Say protected log and log1p remain the runtime controls.",
        "Keep D74 boundary controls visible: x = -1, x < -1, and unguarded statements remain blocked.",
    ...
    row = witness_copy_row(
        ...
        "A checked MachLib witness records the scoped guarded identity 0 < 1 + x -> eml (log (1 + x)) (exp 1) = x; protected log and log1p remain runtime controls.",
        row_caveats,
        row_blocked_phrases,
    ...
    required_caveats = [
        ...
        "The shifted positive-domain guard, 0 < 1 + x, must remain visible.",
    ...
    summary = {
        ...
        "publicReady": False,
        "nextAction": "EML-D79 choose log1p-shifted branch pause/freeze, next bounded branch, or human-approved public copy gate.",
    ...
    return {
        ...
        "reviewType": "eml_log1p_shifted_checked_witness_copy_review_packet_v0",
        "artifactId": "eml-d78-log1p-shifted-checked-witness-copy-review-packet",
        "status": STATUS,
        "decision": "log1p_shifted_checked_witness_copy_review_private_only_public_copy_held",
        "date": DATE,
        "sourceSelector": selector["artifactId"],
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceSelector"] != "eml-d77-log1p-shifted-surface-next-selector":
        raise ValueError("D78 must consume D77")
    ...
    if summary["guardCount"] != 1:
        raise ValueError("log1p-shifted copy review should preserve one guard")
    ...
    if summary["requiredCaveatCount"] != 9:
        raise ValueError("unexpected caveat count")
    if summary["blockedGlobalPhraseCount"] != 12:
        raise ValueError("unexpected blocked phrase count")
    if summary["rowRequiredCaveatCount"] != 6 or summary["rowBlockedPhraseCount"] != 10:
    ...
    if summary["nextAction"] != "EML-D79 choose log1p-shifted branch pause/freeze, next bounded branch, or human-approved public copy gate.":
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "artifactType": "eml_log1p_shifted_checked_witness_copy_review_packet",
        "validationStatus": "pass",
        "semanticStrength": "private_log1p_shifted_checked_witness_copy_review_public_copy_held",
        "source": f"python/results/eml_d78_log1p_shifted_checked_witness_copy_review_packet/eml_d78_log1p_shifted_checked_witness_copy_review_packet_{STAMP}.json",
        "summary": payload["summary"],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "feedId": "eml_d78_log1p_shifted_checked_witness_copy_review_packet_feed",
        "date": DATE,
...
def build_outputs(
    ...
    result_path = out_dir / f"eml_d78_log1p_shifted_checked_witness_copy_review_packet_{STAMP}.json"
    report_path = report_dir / f"eml_d78_log1p_shifted_checked_witness_copy_review_packet_{STAMP}.md"
    evidence_path = evidence_dir / "eml_d78_log1p_shifted_checked_witness_copy_review_packet.json"
    feed_path = command_feed_dir / f"eml_d78_log1p_shifted_checked_witness_copy_review_packet_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=ROOT / "python/results/eml_d78_log1p_shifted_checked_witness_copy_review_packet",
    )
...
def main() -> int:
    ...
    print("EML_D78_LOG1P_SHIFTED_CHECKED_WITNESS_COPY_REVIEW_PACKET_OK")
    return 0
