#!/usr/bin/env python3
"""EML-D68 probability-logit private surface next-action selector."""
...
from scripts import eml_d67_probability_logit_witness_surface_review as d67  # noqa: E402

DATE = "2026-06-03"
STAMP = DATE.replace("-", "_")
SCHEMA_VERSION = "monogate.eml_probability_logit_surface_next_selector.v0"
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    surface = d67.build_payload(atlas_gate_path)
    d67.validate_payload(surface)
    runtime_row = surface_row_by_id(surface, "probability_logit_runtime_control_guardrail")
    guard_row = surface_row_by_id(surface, "probability_logit_guard_boundary")
    public_row = surface_row_by_id(surface, "public_atlas_probability_logit_boundary")
    options = [
        selector_option(
            "probability_logit_checked_witness_copy_review_packet",
            "private_copy_review_lane",
            "selected_next",
            86,
            "EML-D69 probability-logit checked-witness copy review packet",
            [
                "D66 checked the witness and D67 surfaced five private rows, but public-safe wording is still unreviewed.",
                "A copy-review packet can freeze allowed phrases, guard caveats, runtime caveats, and blocked claims before any public gate is considered.",
                "This keeps the checked witness useful for reviewers while preserving protected log and log1p as runtime controls.",
            ],
            [
                "must preserve both probability interval guards",
                "must preserve protected log/log1p runtime-control language",
                "must block log, log1p, logit replacement and runtime advantage claims",
        ...
        selector_option(
            ...
            [
                "A next bounded branch remains valuable after the probability-logit witness wording is stabilized.",
                "Starting another branch first would leave the checked probability-logit witness with only surface rows and no copy boundary.",
            ],
            [
                "requires exactly one non-duplicate candidate",
                "must not start MachLib proof work in the selector",
                "must preserve D67 public and runtime holds",
            ],
        ),
        selector_option(
            ...
            [
                ...
                "It carries higher guard and negative-control risk than stabilizing the already checked probability-logit witness copy.",
            ],
            [
        ...
        selector_option(
            ...
            [
                "D67 records public Atlas as held private.",
                "Human approval should come after private copy review, not before the guard and runtime caveats are frozen.",
            ],
            [
                "requires explicit human approval",
                "requires copy-reviewed caveats and blocked phrases",
                "must not imply public readiness, runtime advantage, or log/log1p/logit replacement",
            ],
        ),
    ]
    ...
    summary = {
        ...
        "d67SurfaceRowCount": surface["summary"]["surfaceRowCount"],
        "d67CheckedWitnessRecordedPrivately": surface["summary"]["checkedWitnessRecordedPrivately"],
        "d67CandidateProved": surface["summary"]["candidateProved"],
        "d67BuildPassed": surface["summary"]["buildPassed"],
    ...
    return {
        ...
        "selectorType": "eml_probability_logit_surface_next_selector_v0",
        "artifactId": "eml-d68-probability-logit-surface-next-selector",
        "status": STATUS,
        "decision": "select_probability_logit_checked_witness_copy_review_packet",
        "date": DATE,
        "sourceSurfaceReview": surface["artifactId"],
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceSurfaceReview"] != "eml-d67-probability-logit-witness-surface-review":
        raise ValueError("D68 must consume D67")
    ...
    if summary["d67SurfaceRowCount"] != 5:
        raise ValueError("expected five D67 rows")
    for key in ["d67CheckedWitnessRecordedPrivately", "d67CandidateProved", "d67BuildPassed"]:
    ...
    if summary["selectedOptionId"] != "probability_logit_checked_witness_copy_review_packet":
        raise ValueError("unexpected selected option")
    if summary["selectedNextArtifact"] != "EML-D69 probability-logit checked-witness copy review packet":
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "artifactType": "eml_probability_logit_surface_next_selector",
        "validationStatus": "pass",
        "semanticStrength": "private_selector_checked_witness_copy_review_next_no_public_update",
        "source": f"python/results/eml_d68_probability_logit_surface_next_selector/eml_d68_probability_logit_surface_next_selector_{STAMP}.json",
        "summary": payload["summary"],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "feedId": "eml_d68_probability_logit_surface_next_selector_feed",
        "date": DATE,
        "status": payload["status"],
        "selectedOptionId": payload["summary"]["selectedOptionId"],
        "selectedNextArtifact": payload["summary"]["selectedNextArtifact"],
        "nextAction": "Run EML-D69 as a private probability-logit checked-witness copy review packet; do not promote public copy.",
        "claimFlags": dict(CLAIM_FLAGS),
    }
...
def build_outputs(out_dir: Path, report_dir: Path, evidence_dir: Path, command_feed_dir: Path, atlas_gate_path: Path) -> dict[str, Any]:
    ...
    result_path = out_dir / f"eml_d68_probability_logit_surface_next_selector_{STAMP}.json"
    report_path = report_dir / f"eml_d68_probability_logit_surface_next_selector_{STAMP}.md"
    evidence_path = evidence_dir / "eml_d68_probability_logit_surface_next_selector.json"
    feed_path = command_feed_dir / f"eml_d68_probability_logit_surface_next_selector_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/eml_d68_probability_logit_surface_next_selector")
...
def main() -> int:
    ...
    print("EML_D68_PROBABILITY_LOGIT_SURFACE_NEXT_SELECTOR_OK")
    return 0
