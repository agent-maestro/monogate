#!/usr/bin/env python3
"""EML-D77 log1p-shifted private surface next-action selector."""
...
from scripts import eml_d76_log1p_shifted_witness_surface_review as d76  # noqa: E402

DATE = "2026-06-03"
STAMP = DATE.replace("-", "_")
SCHEMA_VERSION = "monogate.eml_log1p_shifted_surface_next_selector.v0"
...
def build_payload(atlas_gate_path: Path) -> dict[str, Any]:
    surface = d76.build_payload(atlas_gate_path)
    d76.validate_payload(surface)
    runtime_row = surface_row_by_id(surface, "log1p_shifted_runtime_control_guardrail")
    guard_row = surface_row_by_id(surface, "log1p_shifted_guard_boundary")
    public_row = surface_row_by_id(surface, "public_atlas_log1p_shifted_boundary")
    options = [
        selector_option(
            "log1p_shifted_checked_witness_copy_review_packet",
            "private_copy_review_lane",
            "selected_next",
            86,
            "EML-D78 log1p-shifted checked-witness copy review packet",
            [
                "D75 checked the witness and D76 surfaced five private rows, but public-safe wording is still unreviewed.",
        ...
        selector_option(
            ...
            [
                "A next bounded branch remains valuable after the log1p-shifted witness wording is stabilized.",
                "Starting another branch first would leave the checked log1p-shifted witness with only surface rows and no copy boundary.",
            ],
            [
                "requires exactly one non-duplicate candidate",
                "must not start MachLib proof work in the selector",
                "must preserve D76 public and runtime holds",
            ],
        ),
        selector_option(
            ...
            [
                ...
                "It carries higher guard and negative-control risk than stabilizing the already checked log1p-shifted witness copy.",
            ],
            [
        ...
        selector_option(
            ...
            [
                "D76 records public Atlas as held private.",
    ...
    summary = {
        ...
        "d76SurfaceRowCount": surface["summary"]["surfaceRowCount"],
        "d76CheckedWitnessRecordedPrivately": surface["summary"]["checkedWitnessRecordedPrivately"],
        "d76CandidateProved": surface["summary"]["candidateProved"],
        "d76BuildPassed": surface["summary"]["buildPassed"],
    ...
    return {
        ...
        "selectorType": "eml_log1p_shifted_surface_next_selector_v0",
        "artifactId": "eml-d77-log1p-shifted-surface-next-selector",
        "status": STATUS,
        "decision": "select_log1p_shifted_checked_witness_copy_review_packet",
        "date": DATE,
        "sourceSurfaceReview": surface["artifactId"],
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if payload["sourceSurfaceReview"] != "eml-d76-log1p-shifted-witness-surface-review":
        raise ValueError("D77 must consume D76")
    ...
    if summary["d76SurfaceRowCount"] != 5:
        raise ValueError("expected five D76 rows")
    for key in ["d76CheckedWitnessRecordedPrivately", "d76CandidateProved", "d76BuildPassed"]:
    ...
    if summary["selectedOptionId"] != "log1p_shifted_checked_witness_copy_review_packet":
        raise ValueError("unexpected selected option")
    if summary["selectedNextArtifact"] != "EML-D78 log1p-shifted checked-witness copy review packet":
...
def build_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "artifactType": "eml_log1p_shifted_surface_next_selector",
        "validationStatus": "pass",
        "semanticStrength": "private_selector_checked_witness_copy_review_next_no_public_update",
        "source": f"python/results/eml_d77_log1p_shifted_surface_next_selector/eml_d77_log1p_shifted_surface_next_selector_{STAMP}.json",
        "summary": payload["summary"],
        "claimFlags": dict(CLAIM_FLAGS),
        "nonClaims": list(NON_CLAIMS),
    }
...
def build_feed(payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "feedId": "eml_d77_log1p_shifted_surface_next_selector_feed",
        "date": DATE,
        "status": payload["status"],
        "selectedOptionId": payload["summary"]["selectedOptionId"],
        "selectedNextArtifact": payload["summary"]["selectedNextArtifact"],
        "nextAction": "Run EML-D78 as a private log1p-shifted checked-witness copy review packet; do not promote public copy.",
        "claimFlags": dict(CLAIM_FLAGS),
    }
...
def build_outputs(
    ...
    result_path = out_dir / f"eml_d77_log1p_shifted_surface_next_selector_{STAMP}.json"
    report_path = report_dir / f"eml_d77_log1p_shifted_surface_next_selector_{STAMP}.md"
    evidence_path = evidence_dir / "eml_d77_log1p_shifted_surface_next_selector.json"
    feed_path = command_feed_dir / f"eml_d77_log1p_shifted_surface_next_selector_feed_{STAMP}.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "python/results/eml_d77_log1p_shifted_surface_next_selector")
...
def main() -> int:
    ...
    print("EML_D77_LOG1P_SHIFTED_SURFACE_NEXT_SELECTOR_OK")
    return 0
