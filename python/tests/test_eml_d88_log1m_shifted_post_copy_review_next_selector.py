"""Tests for EML-D88 log1m-shifted post-copy-review next selector."""
...
from scripts.eml_d88_log1m_shifted_post_copy_review_next_selector import (
    CLAIM_FLAGS,
    ROOT,
    build_outputs,
    build_payload,
...
def test_d88_consumes_d87_copy_review():
    payload = build_payload(ATLAS_GATE)
    validate_payload(payload)
    assert payload["status"] == "EML_D88_LOG1M_SHIFTED_POST_COPY_REVIEW_NEXT_SELECTOR_PASS"
    assert payload["sourceReview"] == "eml-d87-log1m-shifted-checked-witness-copy-review-packet"
    assert payload["summary"]["sourceSelectedOptionId"] == "log1m_shifted_checked_witness_copy_review_packet"
    assert payload["summary"]["d87CopyReviewStarted"] is True
    assert payload["summary"]["d87PrivateCopyReviewOnly"] is True
    assert payload["summary"]["d87CheckedWitnessCopyReviewOnly"] is True


def test_d88_preserves_log1m_shifted_copy_boundaries():
    payload = build_payload(ATLAS_GATE)
...
def test_d88_selects_log1m_shifted_pause_freeze_packet():
    payload = build_payload(ATLAS_GATE)
    selected = option_by_id(payload, "log1m_shifted_branch_pause_freeze_packet")
    assert payload["decision"] == "select_log1m_shifted_branch_pause_freeze_packet"
...
def test_d88_parks_future_branch_trig_reviewer_and_public_gate():
    payload = build_payload(ATLAS_GATE)
...
def test_d88_preserves_d87_counts():
    payload = build_payload(ATLAS_GATE)
    ...
    assert payload["summary"]["d87WitnessRowCount"] == 1
    assert payload["summary"]["d87RequiredCaveatCount"] == 10
    assert payload["summary"]["d87BlockedGlobalPhraseCount"] == 13
    assert payload["summary"]["d87RowRequiredCaveatCount"] == 7
    assert payload["summary"]["d87RowBlockedPhraseCount"] == 12


def test_d88_starts_no_pause_public_copy_reviewer_or_implementation():
    payload = build_payload(ATLAS_GATE)
...
def test_d88_keeps_runtime_laptop_and_electronics_claims_blocked():
    payload = build_payload(ATLAS_GATE)
...
def test_d88_claim_flags_are_bounded():
    payload = build_payload(ATLAS_GATE)
...
def test_d88_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "reports",
        tmp_path / "evidence",
    ...
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# EML-D88")


def test_d88_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/eml_d88_log1m_shifted_post_copy_review_next_selector.py",
            "--build",
            "--atlas-gate-path",
            str(ATLAS_GATE),
            "--out-dir",
    ...
    assert "EML_D88_LOG1M_SHIFTED_POST_COPY_REVIEW_NEXT_SELECTOR_OK" in proc.stdout
