"""Tests for FEF-P117 compound-condition original C runtime gate."""
...
from scripts.fef_p117_compound_condition_original_c_runtime_gate import (
    CLAIM_FLAGS,
    build_outputs,
    build_payload,
    c_harness_source,
...
def test_fef_p117_records_original_c_runtime_without_support_claim():
    payload = build_payload()
    validate_payload(payload)
    summary = payload["summary"]
    assert payload["status"] == "FEF_P117_COMPOUND_CONDITION_ORIGINAL_C_RUNTIME_GATE_PASS"
...
def test_fef_p117_rows_match_original_c_outputs_and_expected_values():
    payload = build_payload()
...
def test_fef_p117_runtime_scope_is_original_c_only():
    payload = build_payload()
...
def test_fef_p117_c_harness_contains_selected_compound_condition_source():
    source = c_harness_source([{"sampleId": "sample_00", "inputs": {"x": 2.0, "y": 3.0}}])
    assert "static double selected_and_guard_return(double x, double y)" in source
    assert "if (x > 0.0 && y > 0.0)" in source
    assert "return x + y;" in source
    assert "return 0.0;" in source


def test_fef_p117_runtime_output_parser():
    observed = parse_runtime_output("sample_00 5\nsample_01 0\n")
    assert observed == {"sample_00": 5.0, "sample_01": 0.0}


def test_fef_p117_preserves_short_circuit_distribution_from_reference_gate():
    payload = build_payload()
    summary = payload["summary"]
    assert summary["rightPredicateEvaluatedCount"] == 4
    assert summary["shortCircuitCount"] == 3
    assert all(row["rightPredicateEvaluated"] in {True, False} for row in payload["runtimeComparison"]["rows"])


def test_fef_p117_boundaries_and_claim_flags_remain_false():
    payload = build_payload()
    summary = payload["summary"]
...
def test_fef_p117_writes_outputs(tmp_path):
    built = build_outputs(tmp_path / "results", tmp_path / "reports", tmp_path / "evidence", tmp_path / "feeds")
    for key in ["result_path", "evidence_path", "feed_path"]:
        json.loads(Path(built[key]).read_text(encoding="utf-8"))
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# FEF-P117")


def test_fef_p117_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/fef_p117_compound_condition_original_c_runtime_gate.py",
            "--build",
            "--out-dir",
    ...
    assert "FEF_P117_COMPOUND_CONDITION_ORIGINAL_C_RUNTIME_GATE_OK" in proc.stdout
