"""Tests for EML-D81 post log1p-shifted pause next selector."""
...
from scripts.eml_d81_post_log1p_shifted_pause_next_selector import (
    CLAIM_FLAGS,
    ROOT,
    build_outputs,
    build_payload,
...
def test_d81_consumes_d80_and_selects_next_bounded_identity_branch():
    payload = build_payload(ATLAS_GATE)
    validate_payload(payload)
    summary = payload["summary"]
    assert payload["status"] == "EML_D81_POST_LOG1P_SHIFTED_PAUSE_NEXT_SELECTOR_PASS"
    assert payload["sourceFreezePacket"] == "eml-d80-log1p-shifted-branch-pause-freeze-packet"
    assert summary["selectedOptionId"] == "next_bounded_identity_branch_selector"
    assert summary["selectedNextArtifact"] == "EML-D82 bounded identity branch candidate selector"
    assert summary["nextActionSelected"] is True
    assert summary["nextBoundedIdentityBranchSelected"] is True


def test_d81_observes_act_a16_private_handoff_without_acceptance():
    payload = build_payload(ATLAS_GATE)
    summary = payload["summary"]
...
def test_d81_preserves_d80_log1p_shifted_freeze_boundary():
    payload = build_payload(ATLAS_GATE)
    summary = payload["summary"]
    ...
    assert summary["frozenWitnessName"] == "MachLib.Real.log1p_shifted_boundary_coordinate_witness"
...
def test_d81_options_keep_later_paths_parked():
    payload = build_payload(ATLAS_GATE)
...
def test_d81_keeps_public_reviewer_proof_runtime_and_laptop_claims_blocked():
    payload = build_payload(ATLAS_GATE)
    summary = payload["summary"]
...
def test_d81_runtime_guard_and_public_hold_preserved():
    payload = build_payload(ATLAS_GATE)
    summary = payload["summary"]
...
def test_d81_claim_flags_are_selector_only():
    payload = build_payload(ATLAS_GATE)
...
def test_d81_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "reports",
        tmp_path / "evidence",
    ...
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# EML-D81")


def test_d81_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/eml_d81_post_log1p_shifted_pause_next_selector.py",
            "--build",
            "--atlas-gate-path",
            str(ATLAS_GATE),
            "--out-dir",
    ...
    assert "EML_D81_POST_LOG1P_SHIFTED_PAUSE_NEXT_SELECTOR_OK" in proc.stdout
