"""Tests for FEF-P24 mosfet full Lean proof-discharge validator."""

from __future__ import annotations

import pytest

# Blanket-marked heavy: CLI-contract test (subprocess.run of a
# script that loads large JSON evidence). Skipped from the fast
# dev loop via `pytest -m "not heavy"`; runs in CI by default.
# A follow-up measurement pass will UN-mark individual fast files.
pytestmark = pytest.mark.heavy

import json
from pathlib import Path
import subprocess
import sys

from scripts.fef_p24_mosfet_full_lean_proof_discharge import (
    CLAIM_FLAGS,
    build_outputs,
    build_payload,
    validate_payload,
)


def test_fef_p24_records_reviewed_mosfet_discharges():
    payload = build_payload()
    validate_payload(payload)
    summary = payload["summary"]
    assert payload["status"] == "FEF_P24_MOSFET_FULL_LEAN_PROOF_DISCHARGE_PASS"
    assert summary["selectedDischargedTheoremCount"] == 2
    assert summary["remainingPlaceholderTheoremCount"] == 0
    assert summary["generatedFileSorryCount"] == 2
    assert summary["dischargedFileSorryCount"] == 0
    assert summary["leanCheckStatuses"] == ["typecheck_selected_file_zero_sorry_pass"]


def test_fef_p24_packet_records_zero_sorry_selected_file():
    payload = build_payload()
    packet = payload["mosfetProofPackets"][0]
    theorem_names = [theorem["theoremName"] for theorem in packet["selectedDischargedTheorems"]]
    assert theorem_names == [
        "mosfet_zero_overdrive_zero_current",
        "mosfet_prefactor_positive",
    ]
    assert packet["remainingPlaceholderTheorems"] == []
    proofs = {theorem["theoremName"]: theorem["dischargedProofBody"] for theorem in packet["selectedDischargedTheorems"]}
    assert proofs["mosfet_zero_overdrive_zero_current"] == ["  unfold id_at_threshold", "  rfl"]
    assert proofs["mosfet_prefactor_positive"] == [
        "  unfold id_prefactor",
        "  exact mul_pos (mul_pos h1 h2) h3",
    ]


def test_fef_p24_keeps_broad_claims_false():
    payload = build_payload()
    summary = payload["summary"]
    assert summary["candidateReviewedProofClaim"] is False
    assert summary["leanProofClaim"] is False
    assert summary["compilerCorrectnessClaim"] is False
    assert summary["formalEquivalenceClaim"] is False
    assert all(value is False for value in CLAIM_FLAGS.values())
    assert all(value is False for value in payload["claimFlags"].values())
    for packet in payload["mosfetProofPackets"]:
        assert all(value is False for value in packet["claimFlags"].values())


def test_fef_p24_release_gates_keep_remaining_work_blocked():
    payload = build_payload()
    gates = {gate["id"]: gate["status"] for gate in payload["releaseGates"]}
    assert gates["selected_mosfet_obligations_reviewed"] == "pass"
    assert gates["selected_mosfet_file_typechecks"] == "pass"
    assert gates["selected_mosfet_file_zero_sorry"] == "pass"
    assert gates["remaining_generated_lean_proofs_discharged"] == "blocked"
    assert gates["machlib_foundational_audit"] == "blocked"


def test_fef_p24_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "packets",
        tmp_path / "reports",
        tmp_path / "evidence",
        tmp_path / "feeds",
    )
    for key in ["result_path", "evidence_path", "feed_path"]:
        json.loads(Path(built[key]).read_text(encoding="utf-8"))
    packets = sorted((tmp_path / "packets").glob("*.json"))
    assert len(packets) == 1
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# FEF-P24")


def test_fef_p24_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/fef_p24_mosfet_full_lean_proof_discharge.py",
            "--build",
            "--out-dir",
            str(tmp_path / "results"),
            "--packet-dir",
            str(tmp_path / "packets"),
            "--report-dir",
            str(tmp_path / "reports"),
            "--evidence-dir",
            str(tmp_path / "evidence"),
            "--command-feed-dir",
            str(tmp_path / "feeds"),
            "--strict",
        ],
        check=True,
        capture_output=True,
        text=True,
    )
    assert "FEF_P24_MOSFET_FULL_LEAN_PROOF_DISCHARGE_OK" in proc.stdout
