"""Tests for ATLAS-A38 private scoped trig pythagorean candidate packet."""
...
from scripts.atlas_a38_private_scoped_trig_pythagorean_candidate_packet import (
    CANDIDATE_ID,
    CLAIM_FLAGS,
...
def test_atlas_a38_consumes_a37_and_creates_scoped_candidate_packet():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    validate_payload(payload)
    summary = payload["summary"]
    assert payload["status"] == "ATLAS_A38_PRIVATE_SCOPED_TRIG_PYTHAGOREAN_CANDIDATE_PACKET_PASS"
    assert payload["sourceArtifact"] == "atlas-a37-private-trig-pythagorean-candidate-packet-selector"
    assert summary["sourceReviewedDirectionId"] == SOURCE_DIRECTION_ID
    assert summary["sourceSelectedDecision"] == "recommend_pure_trig_candidate_packet_without_creating_it"
...
def test_atlas_a38_records_guard_and_statement_shapes():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
    ...
    assert summary["pureCandidateStatement"] == (
    ...
    assert summary["emlCompanionDeferredRecorded"] is True
    assert summary["emlCompanionHint"] == "deferred_no_eml_shape_selected"
    assert summary["emlCompanionValidityStatus"] == "not_checked_not_proved_not_formal_equivalence_claim"
    assert candidate["statements"]["pureTrigStatement"]["sourceHint"] == "sin x * sin x + cos x * cos x = 1"
    assert candidate["statements"]["emlCompanion"]["sourceHint"] == "deferred_no_eml_shape_selected"


def test_atlas_a38_records_review_value_and_blockers():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    candidate = payload["candidatePacket"]
    assert len(candidate["reviewValue"]) == 3
    assert len(candidate["blockersBeforeProofSelection"]) == 4
    assert "perform theorem lookup before naming any Lean theorem dependency" in candidate[
        "blockersBeforeProofSelection"
    ]
    assert "check exact namespace/import surface before any MachLib edit" in candidate[
        "blockersBeforeProofSelection"
    ]


def test_atlas_a38_creates_no_proof_or_validity_claims():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
...
def test_atlas_a38_blocks_edit_lean_lookup_public_runtime_and_product_claims():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
...
def test_atlas_a38_preserves_target_accounting():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
...
def test_atlas_a38_claim_flags_stay_bounded():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
...
def test_atlas_a38_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "reports",
        tmp_path / "evidence",
    ...
    assert report.startswith("# ATLAS-A38 Private Scoped Trig Pythagorean Candidate Packet")
    assert "## Candidate Statements" in report
    assert "## Review Value" in report
    assert "## Blockers Before Proof Selection" in report


def test_atlas_a38_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/atlas_a38_private_scoped_trig_pythagorean_candidate_packet.py",
            "--build",
            "--atlas-gate-path",
            str(ATLAS_GATE),
            "--machlib-root",
    ...
    assert "ATLAS_A38_PRIVATE_SCOPED_TRIG_PYTHAGOREAN_CANDIDATE_PACKET_OK" in proc.stdout
