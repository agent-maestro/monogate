"""Tests for FEF-P53 C if early-return branch lowering gate."""
...
from scripts.fef_p53_c_if_early_return_branch_lowering_gate import (
    CLAIM_FLAGS,
    build_outputs,
    build_payload,
    validate_payload,
...
def test_fef_p53_records_selected_c_if_early_return_gate():
    payload = build_payload()
    validate_payload(payload)
    summary = payload["summary"]
    assert payload["status"] == "FEF_P53_C_IF_EARLY_RETURN_BRANCH_LOWERING_GATE_PASS"
    assert payload["decision"] == (
        "selected_c_if_early_return_lowering_reingest_passed_general_branch_blocked"
    )
    assert summary["selectedCIfEarlyReturnLoweringPass"] is True
    assert summary["cIfEarlyReturnBlockerClosed"] is True
...
def test_fef_p53_source_fixture_shape_is_guarded_selector():
    payload = build_payload()
    fixture = payload["sourceFixture"]
    assert fixture["sourceLanguage"] == "c"
    assert fixture["feature"] == "if_early_return"
...
def test_fef_p53_packets_compare_generated_runtime_to_reingested_python():
    payload = build_payload()
    summary = payload["summary"]
    ...
    for packet in payload["reingestPackets"]:
        ...
        assert packet["sourceFeature"] == "if_early_return"
        ...
        assert set(packet["frames"][0]["values"]) == {
            ...
            "expectedIfEarlyReturnReference",
        }


def test_fef_p53_claim_boundaries_and_gates_remain_blocked():
    payload = build_payload()
    summary = payload["summary"]
    gates = {gate["id"]: gate["status"] for gate in payload["releaseGates"]}
    assert gates["selected_c_if_early_return_lowering"] == "pass"
    assert gates["selected_c_if_early_return_generated_c_reingest"] == "pass"
    assert gates["selected_c_if_early_return_generated_rust_reingest"] == "pass"
...
def test_fef_p53_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "packets",
        tmp_path / "reports",
    ...
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# FEF-P53")


def test_fef_p53_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/fef_p53_c_if_early_return_branch_lowering_gate.py",
            "--build",
            "--out-dir",
    ...
    assert "FEF_P53_C_IF_EARLY_RETURN_BRANCH_LOWERING_GATE_OK" in proc.stdout
