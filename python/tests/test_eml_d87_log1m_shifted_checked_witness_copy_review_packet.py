"""Tests for EML-D87 log1m-shifted checked-witness copy review packet."""
...
from scripts.eml_d87_log1m_shifted_checked_witness_copy_review_packet import (
    CLAIM_FLAGS,
    ROOT,
    build_outputs,
    build_payload,
...
def test_d87_consumes_d86_selector():
    payload = build_payload(ATLAS_GATE)
    validate_payload(payload)
    assert payload["status"] == "EML_D87_LOG1M_SHIFTED_CHECKED_WITNESS_COPY_REVIEW_PACKET_PASS"
    assert payload["sourceSelector"] == "eml-d86-log1m-shifted-surface-next-selector"
    assert payload["summary"]["selectedOptionId"] == "log1m_shifted_checked_witness_copy_review_packet"


def test_d87_preserves_checked_witness_guard_and_duplicate_block():
    payload = build_payload(ATLAS_GATE)
...
def test_d87_records_private_copy_review_boundaries():
    payload = build_payload(ATLAS_GATE)
    ...
    assert payload["summary"]["blockedGlobalPhraseCount"] == 13
    assert payload["summary"]["rowRequiredCaveatCount"] == 7
    assert payload["summary"]["rowBlockedPhraseCount"] == 12
    assert row["copyStatus"] == "private_checked_witness_copy_reviewable"
    assert row["publicPromotionAllowed"] is False


def test_d87_freezes_guard_runtime_and_duplicate_copy_language():
    payload = build_payload(ATLAS_GATE)
    row = payload["witnessCopyRows"][0]
    ...
    assert "duplicate-log1p block" in " ".join(payload["requiredCaveats"])
...
def test_d87_blocks_public_runtime_and_broad_claims():
    payload = build_payload(ATLAS_GATE)
...
def test_d87_starts_no_machlib_proof_reviewer_or_laptop_work():
    payload = build_payload(ATLAS_GATE)
...
def test_d87_claim_flags_are_copy_review_only_with_duplicate_block():
    payload = build_payload(ATLAS_GATE)
    allowed_true = {
        "copy_review_started",
...
def test_d87_points_to_next_selector():
    payload = build_payload(ATLAS_GATE)
...
def test_d87_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "reports",
        tmp_path / "evidence",
    ...
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# EML-D87")


def test_d87_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/eml_d87_log1m_shifted_checked_witness_copy_review_packet.py",
            "--build",
            "--atlas-gate-path",
            str(ATLAS_GATE),
            "--out-dir",
    ...
    assert "EML_D87_LOG1M_SHIFTED_CHECKED_WITNESS_COPY_REVIEW_PACKET_OK" in proc.stdout
