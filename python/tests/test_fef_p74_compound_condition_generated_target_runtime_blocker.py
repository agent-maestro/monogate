"""Tests for FEF-P74 compound-condition generated-target runtime blocker."""
...
from scripts.fef_p74_compound_condition_generated_target_runtime_blocker import (
    CLAIM_FLAGS,
    build_outputs,
    build_payload,
    validate_gate,
...
def test_fef_p74_records_blocked_generated_target_gate():
    payload = build_payload()
    validate_payload(payload)
    summary = payload["summary"]
    assert payload["status"] == "FEF_P74_COMPOUND_CONDITION_GENERATED_TARGET_RUNTIME_BLOCKER_PASS"
...
def test_fef_p74_inherits_p73_original_runtime_evidence_without_rerun_claim():
    payload = build_payload()
    gate = payload["generatedTargetRuntimeGate"]
    validate_gate(gate)
    inherited = gate["inheritedOriginalRuntimeEvidence"]
    assert inherited["phase"] == "P73"
    ...
    assert payload["summary"]["p73OriginalRuntimeComparisons"] == 7
    assert payload["summary"]["p73OriginalRuntimePassCount"] == 7


def test_fef_p74_required_before_run_items_are_explicit():
    payload = build_payload()
...
def test_fef_p74_boundaries_and_claim_flags_remain_false():
    payload = build_payload()
    summary = payload["summary"]
...
def test_fef_p74_writes_outputs(tmp_path):
    built = build_outputs(tmp_path / "results", tmp_path / "reports", tmp_path / "evidence", tmp_path / "feeds")
    for key in ["result_path", "evidence_path", "feed_path"]:
        json.loads(Path(built[key]).read_text(encoding="utf-8"))
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# FEF-P74")


def test_fef_p74_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/fef_p74_compound_condition_generated_target_runtime_blocker.py",
            "--build",
            "--out-dir",
    ...
    assert "FEF_P74_COMPOUND_CONDITION_GENERATED_TARGET_RUNTIME_BLOCKER_OK" in proc.stdout
