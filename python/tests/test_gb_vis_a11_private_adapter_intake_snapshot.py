"""Tests for GB-VIS-A11 private adapter intake snapshot."""
...
from scripts.gb_vis_a11_private_adapter_intake_snapshot import (
    CLAIM_FLAGS,
    ROOT,
    build_outputs,
    build_payload,
...
def test_gb_vis_a11_consumes_gb_vis_a10_and_records_snapshots():
    payload = build_payload(ATLAS_GATE)
    validate_payload(payload)
    assert payload["status"] == "GB_VIS_A11_PRIVATE_ADAPTER_INTAKE_SNAPSHOT_PASS"
    assert payload["sourceAdapterIntakeGuard"] == "gb-vis-a10-private-adapter-intake-guard"
    assert payload["summary"]["sourceFeedGuardRowCount"] == 6
    assert payload["summary"]["intakeGuardRowCount"] == 7


def test_gb_vis_a11_snapshot_counts_preserve_intake_guard():
    payload = build_payload(ATLAS_GATE)
    assert payload["summary"]["sourceFeedGuardPassCount"] == 6
    assert payload["summary"]["sourceClaimFlagCount"] == 34
    assert payload["summary"]["blockedSourceClaimFlagCount"] == 21
    ...
    for snapshot in [payload["baselineSnapshot"], payload["observedSnapshot"]]:
        ...
        assert snapshot["sourceClaimFlagCount"] == 34
        assert snapshot["blockedSourceClaimFlagCount"] == 21
        assert snapshot["intakeGuardRowCount"] == 7
        assert snapshot["intakeGuardPassCount"] == 7


def test_gb_vis_a11_snapshot_digests_are_stable():
    payload = build_payload(ATLAS_GATE)
...
def test_gb_vis_a11_snapshot_checks_pass_exactly():
    payload = build_payload(ATLAS_GATE)
...
def test_gb_vis_a11_records_no_renderer_or_public_claim():
    payload = build_payload(ATLAS_GATE)
    assert payload["summary"]["privateAdapterIntakeSnapshotRecorded"] is True
    assert payload["summary"]["gbVisA10AdapterIntakeGuardConsumed"] is True
...
def test_gb_vis_a11_claim_flags_are_bounded():
    payload = build_payload(ATLAS_GATE)
...
def test_gb_vis_a11_next_action_stays_private():
    payload = build_payload(ATLAS_GATE)
...
def test_gb_vis_a11_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "reports",
        tmp_path / "evidence",
    ...
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# GB-VIS-A11")


def test_gb_vis_a11_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/gb_vis_a11_private_adapter_intake_snapshot.py",
            "--build",
            "--atlas-gate-path",
            str(ATLAS_GATE),
            "--out-dir",
    ...
    assert "GB_VIS_A11_PRIVATE_ADAPTER_INTAKE_SNAPSHOT_OK" in proc.stdout
