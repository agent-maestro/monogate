"""Tests for ATLAS-A42 private trig pythagorean wrapper-or-alias attempt gate."""
...
from scripts.atlas_a42_private_trig_pythagorean_wrapper_or_alias_attempt_gate import (
    CLAIM_FLAGS,
...
def test_atlas_a42_consumes_a41_and_selects_future_attempt_shape():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    validate_payload(payload)
    summary = payload["summary"]
    assert payload["status"] == "ATLAS_A42_PRIVATE_TRIG_PYTHAGOREAN_WRAPPER_OR_ALIAS_ATTEMPT_GATE_PASS"
    assert payload["sourceArtifact"] == "atlas-a41-private-trig-pythagorean-witness-wrapper-readiness-selector"
...
def test_atlas_a42_records_target_shape():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
    ...
    assert summary["proposedStatement"] == (
...
def test_atlas_a42_records_observed_surface_and_deferred_companion():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
    gate = payload["wrapperOrAliasAttemptGate"]
...
def test_atlas_a42_records_future_plan_and_blocked_alternatives():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    gate = payload["wrapperOrAliasAttemptGate"]
    ...
    assert "do not edit Trig.lean or ProofSpine.lean in this path" in gate["blockedAlternatives"]


def test_atlas_a42_creates_no_attempt_dependency_proof_or_validity_claims():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
...
def test_atlas_a42_blocks_edit_lean_public_runtime_and_product_claims():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
...
def test_atlas_a42_preserves_target_accounting():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
...
def test_atlas_a42_claim_flags_stay_bounded():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
...
def test_atlas_a42_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "reports",
        tmp_path / "evidence",
    ...
    assert report.startswith("# ATLAS-A42 Private Trig Pythagorean Wrapper-Or-Alias Attempt Gate")
    assert "## Target Shape" in report
    assert "## Future Attempt Plan" in report
    assert "## Blocked Alternatives" in report


def test_atlas_a42_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/atlas_a42_private_trig_pythagorean_wrapper_or_alias_attempt_gate.py",
            "--build",
            "--atlas-gate-path",
            str(ATLAS_GATE),
            "--machlib-root",
    ...
    assert "ATLAS_A42_PRIVATE_TRIG_PYTHAGOREAN_WRAPPER_OR_ALIAS_ATTEMPT_GATE_OK" in proc.stdout
