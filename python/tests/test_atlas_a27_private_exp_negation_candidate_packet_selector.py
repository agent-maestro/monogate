"""Tests for ATLAS-A27 private exp-negation candidate packet selector."""
...
from scripts.atlas_a27_private_exp_negation_candidate_packet_selector import (
    CLAIM_FLAGS,
...
def test_atlas_a27_consumes_a26_and_recommends_future_packet():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    validate_payload(payload)
    summary = payload["summary"]
    assert payload["status"] == "ATLAS_A27_PRIVATE_EXP_NEGATION_CANDIDATE_PACKET_SELECTOR_PASS"
    assert payload["sourceArtifact"] == "atlas-a26-private-exp-negation-boundary-feasibility-packet"
    assert summary["sourceReviewedDirectionId"] == SOURCE_DIRECTION_ID
    assert summary["selectedOptionId"] == SELECTED_OPTION_ID
    assert summary["selectedDecision"] == "recommend_scoped_candidate_packet_without_creating_it"
    assert summary["futureCandidatePacketRecommended"] is True
    assert summary["nextRecommendedArtifact"] == NEXT_RECOMMENDED_ARTIFACT


def test_atlas_a27_carries_source_guard_shape_and_blockers():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
    selected = payload["selectedOption"]
    assert summary["feasibilityReviewConsumed"] is True
    assert summary["scopeChoiceRequiredRecorded"] is True
    assert summary["sourceRequiredGuard"] == "all real x"
    assert summary["sourcePureShapeHint"] == "exp x * exp (-x) = 1"
    assert summary["sourcePossibleEmlBoundaryHint"] == "eml (x + (-x)) 1 = 1"
...
def test_atlas_a27_records_options():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    options = {item["optionId"]: item for item in payload["options"]}
    assert set(options) == {
        SELECTED_OPTION_ID,
        "hold_for_statement_scope_clarification",
        "pause_for_atlas_v0_document",
    }
    assert options[SELECTED_OPTION_ID]["selectionStatus"] == "selected_next"
    assert options["hold_for_statement_scope_clarification"]["selectionStatus"] == (
        "available_if_human_wants_scope_decision_first"
    )
    assert options["pause_for_atlas_v0_document"]["selectionStatus"] == "available_if_human_prefers_consolidation"


def test_atlas_a27_creates_no_candidate_packet_or_proof_claims():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
...
def test_atlas_a27_blocks_edit_lean_theorem_public_runtime_and_product_claims():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
...
def test_atlas_a27_preserves_target_accounting():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
...
def test_atlas_a27_claim_flags_stay_bounded():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
...
def test_atlas_a27_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "reports",
        tmp_path / "evidence",
    ...
    assert report.startswith("# ATLAS-A27 Private Exp-Negation Candidate Packet Selector")
    assert "## Readiness Reasons" in report
    assert "## Future Packet Scope Requirements" in report
    assert "## Options" in report


def test_atlas_a27_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/atlas_a27_private_exp_negation_candidate_packet_selector.py",
            "--build",
            "--atlas-gate-path",
            str(ATLAS_GATE),
            "--machlib-root",
    ...
    assert "ATLAS_A27_PRIVATE_EXP_NEGATION_CANDIDATE_PACKET_SELECTOR_OK" in proc.stdout
