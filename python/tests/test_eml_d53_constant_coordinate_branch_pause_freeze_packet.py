"""Tests for EML-D53 constant-coordinate branch pause freeze packet."""
...
from scripts.eml_d53_constant_coordinate_branch_pause_freeze_packet import (
    CLAIM_FLAGS,
    ROOT,
    build_outputs,
    build_payload,
...
def test_d53_consumes_d52_selected_pause_freeze_option():
    payload = build_payload(ATLAS_GATE)
    validate_payload(payload)
    assert payload["status"] == "EML_D53_CONSTANT_COORDINATE_BRANCH_PAUSE_FREEZE_PACKET_PASS"
    assert payload["sourceSelector"] == "eml-d52-constant-coordinate-review-next-selector"
    assert payload["summary"]["selectedOptionId"] == "constant_coordinate_branch_pause_freeze_packet"


def test_d53_freezes_constant_coordinate_checked_delta():
    payload = build_payload(ATLAS_GATE)
    ...
    assert row["sourceStatement"] == "eml 0 (exp 2) = -1"
    assert row["checkedStatement"] == "eml 0 (exp (1 + 1)) = -1"
    assert row["guards"] == []


def test_d53_preserves_local_spelling_and_non_duplicate_boundary():
    payload = build_payload(ATLAS_GATE)
    row = payload["freezeRows"][0]
    ...
    assert row["existingConstantWitnessName"] == "MachLib.Real.constants_zero_one_e_boundary_witness"
    assert row["duplicatesExistingConstantWitness"] is False
    assert "exp (1 + 1)" in row["localSpellingNote"]


def test_d53_preserves_d51_caveats_blockers_and_runtime_control():
    payload = build_payload(ATLAS_GATE)
    row = payload["freezeRows"][0]
    ...
    assert payload["summary"]["sourceD51RequiredCaveatCount"] == 8
    assert payload["summary"]["sourceD51BlockedGlobalPhraseCount"] == 10
    ...
    assert row["runtimeControl"] == "standard_log_exp_and_arithmetic_remain_runtime_controls"
    assert "The D47 source statement is eml 0 (exp 2) = -1." in row["frozenCaveats"]
    assert "duplicate constants bundle" in row["frozenBlockedPhrases"]


def test_d53_parks_future_branches_and_public_gate():
    payload = build_payload(ATLAS_GATE)
    assert parked_by_id(payload, "next_bounded_identity_branch_selector")["status"] == "parked_after_constant_coordinate_pause"
    assert parked_by_id(payload, "bounded_trig_identity_feasibility_selector")["status"] == "parked_after_constant_coordinate_pause"
...
def test_d53_starts_no_public_copy_or_implementation():
    payload = build_payload(ATLAS_GATE)
...
def test_d53_keeps_runtime_log_exp_and_laptop_claims_blocked():
    payload = build_payload(ATLAS_GATE)
...
def test_d53_claim_flags_are_freeze_only():
    payload = build_payload(ATLAS_GATE)
...
def test_d53_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "reports",
        tmp_path / "evidence",
    ...
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# EML-D53")


def test_d53_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/eml_d53_constant_coordinate_branch_pause_freeze_packet.py",
            "--build",
            "--atlas-gate-path",
            str(ATLAS_GATE),
            "--out-dir",
    ...
    assert "EML_D53_CONSTANT_COORDINATE_BRANCH_PAUSE_FREEZE_PACKET_OK" in proc.stdout
