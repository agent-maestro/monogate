"""Tests for EML-D91 bounded identity branch candidate selector."""
...
from scripts.eml_d91_bounded_identity_branch_candidate_selector import (
    CLAIM_FLAGS,
    ROOT,
...
def test_d91_consumes_d90_bounded_identity_selector():
    payload = build_payload(ATLAS_GATE)
    validate_payload(payload)
    summary = payload["summary"]
    assert payload["status"] == "EML_D91_BOUNDED_IDENTITY_BRANCH_CANDIDATE_SELECTOR_PASS"
    assert payload["sourceSelector"] == "eml-d90-post-log1m-shifted-pause-next-selector"
    assert summary["sourceSelectedOptionId"] == "next_bounded_identity_branch_selector"
    assert summary["sourceSelectedNextArtifact"] == "EML-D91 bounded identity branch candidate selector"
    assert summary["sourceD90SelectorObserved"] is True


def test_d91_preserves_d89_log1m_freeze_boundary():
    payload = build_payload(ATLAS_GATE)
    summary = payload["summary"]
...
def test_d91_observes_act_handoff_without_acceptance():
    payload = build_payload(ATLAS_GATE)
    summary = payload["summary"]
...
def test_d91_selects_fresh_affine_scaled_candidate():
    payload = build_payload(ATLAS_GATE)
    summary = payload["summary"]
    ...
    assert summary["selectedFamily"] == "guarded_log1p_affine_scaled_coordinate"
    ...
    assert summary["selectedGuards"] == ["0 < 1 + a * x"]
    assert summary["boundedIdentityCandidateSelected"] is True
    assert summary["log1pAffineScaledCandidateSelected"] is True


def test_d91_blocks_duplicate_shifted_candidates():
    payload = build_payload(ATLAS_GATE)
    duplicate_log1p = candidate_by_id(payload, "log1p_shifted_boundary_coordinate")
    duplicate_log1m = candidate_by_id(payload, "log1m_shifted_boundary_coordinate")
    assert duplicate_log1p["selectionStatus"] == "blocked_duplicate_checked_witness"
    assert duplicate_log1m["selectionStatus"] == "blocked_duplicate_checked_witness"
    assert duplicate_log1p["freshRelativeToCheckedWitnesses"] is False
    assert duplicate_log1m["freshRelativeToCheckedWitnesses"] is False
    assert payload["summary"]["blockedDuplicateCandidateIds"] == [
    ...
    assert payload["summary"]["log1mShiftedDuplicateReselected"] is False


def test_d91_records_candidates_and_parks_later_options():
    payload = build_payload(ATLAS_GATE)
...
def test_d91_keeps_selected_candidate_non_duplicate_and_runtime_controlled():
    payload = build_payload(ATLAS_GATE)
    summary = payload["summary"]
    selected = payload["selectedCandidate"]
...
def test_d91_starts_no_feasibility_proof_runtime_public_or_laptop_work():
    payload = build_payload(ATLAS_GATE)
    summary = payload["summary"]
...
def test_d91_claim_flags_are_candidate_selector_only():
    payload = build_payload(ATLAS_GATE)
...
def test_d91_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "reports",
        tmp_path / "evidence",
    ...
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# EML-D91")


def test_d91_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/eml_d91_bounded_identity_branch_candidate_selector.py",
            "--build",
            "--atlas-gate-path",
            str(ATLAS_GATE),
            "--out-dir",
    ...
    assert "EML_D91_BOUNDED_IDENTITY_BRANCH_CANDIDATE_SELECTOR_OK" in proc.stdout
