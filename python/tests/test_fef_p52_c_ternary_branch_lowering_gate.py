"""Tests for FEF-P52 C ternary branch lowering gate."""
...
from scripts.fef_p52_c_ternary_branch_lowering_gate import (
    CLAIM_FLAGS,
    build_outputs,
    build_payload,
    validate_payload,
...
def test_fef_p52_records_selected_c_ternary_gate():
    payload = build_payload()
    validate_payload(payload)
    summary = payload["summary"]
    assert payload["status"] == "FEF_P52_C_TERNARY_BRANCH_LOWERING_GATE_PASS"
    assert payload["decision"] == (
        "selected_c_ternary_lowering_reingest_passed_general_branch_blocked"
    )
    assert summary["selectedCTernaryLoweringPass"] is True
    assert summary["cTernaryBlockerClosed"] is True
...
def test_fef_p52_source_fixture_shape_is_guarded_selector():
    payload = build_payload()
    fixture = payload["sourceFixture"]
    assert fixture["sourceLanguage"] == "c"
    assert fixture["feature"] == "ternary_select"
    assert fixture["loweringForm"] == "affine_selector_with_step01_guard"
    assert {"fn step01", "clamp(", "step01(", "select_pos"} <= set(
        fixture["sourceEmlContains"]
    )


def test_fef_p52_packets_compare_generated_runtime_to_reingested_python():
    payload = build_payload()
    summary = payload["summary"]
    ...
    for packet in payload["reingestPackets"]:
        ...
        assert packet["sourceFeature"] == "ternary_select"
        ...
        assert set(packet["frames"][0]["values"]) == {
            ...
            "expectedTernaryReference",
        }


def test_fef_p52_claim_boundaries_and_gates_remain_blocked():
    payload = build_payload()
    summary = payload["summary"]
    gates = {gate["id"]: gate["status"] for gate in payload["releaseGates"]}
    assert gates["selected_c_ternary_lowering"] == "pass"
    assert gates["selected_c_ternary_generated_c_reingest"] == "pass"
    assert gates["selected_c_ternary_generated_rust_reingest"] == "pass"
...
def test_fef_p52_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "packets",
        tmp_path / "reports",
    ...
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# FEF-P52")


def test_fef_p52_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/fef_p52_c_ternary_branch_lowering_gate.py",
            "--build",
            "--out-dir",
    ...
    assert "FEF_P52_C_TERNARY_BRANCH_LOWERING_GATE_OK" in proc.stdout
