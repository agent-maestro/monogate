"""Tests for EML-D86 log1m shifted surface next selector."""
...
from scripts.eml_d86_log1m_shifted_surface_next_selector import (
    CLAIM_FLAGS,
    ROOT,
    build_outputs,
    build_payload,
...
def test_d86_consumes_d85_surface_review():
    payload = build_payload(ATLAS_GATE)
    validate_payload(payload)
    assert payload["status"] == "EML_D86_LOG1M_SHIFTED_SURFACE_NEXT_SELECTOR_PASS"
    assert payload["sourceSurfaceReview"] == "eml-d85-log1m-shifted-witness-surface-review"
    assert payload["summary"]["selectedWitnessName"] == "MachLib.Real.log1m_shifted_boundary_coordinate_witness"


def test_d86_preserves_log1m_shifted_surface_boundaries():
    payload = build_payload(ATLAS_GATE)
...
def test_d86_selects_copy_review_packet_next():
    payload = build_payload(ATLAS_GATE)
    selected = option_by_id(payload, "log1m_shifted_checked_witness_copy_review_packet")
    assert payload["decision"] == "select_log1m_shifted_checked_witness_copy_review_packet"
    assert payload["summary"]["selectedOptionId"] == "log1m_shifted_checked_witness_copy_review_packet"
    assert payload["summary"]["selectedNextArtifact"] == "EML-D87 log1m-shifted checked-witness copy review packet"
...
def test_d86_parks_other_options():
    payload = build_payload(ATLAS_GATE)
...
def test_d86_keeps_public_reviewer_runtime_proof_and_laptop_claims_blocked():
    payload = build_payload(ATLAS_GATE)
...
def test_d86_claim_flags_are_selector_only():
    payload = build_payload(ATLAS_GATE)
...
def test_d86_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "reports",
        tmp_path / "evidence",
    ...
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# EML-D86")


def test_d86_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/eml_d86_log1m_shifted_surface_next_selector.py",
            "--build",
            "--atlas-gate-path",
            str(ATLAS_GATE),
            "--out-dir",
    ...
    assert "EML_D86_LOG1M_SHIFTED_SURFACE_NEXT_SELECTOR_OK" in proc.stdout
