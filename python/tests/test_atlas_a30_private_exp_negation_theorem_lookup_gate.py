"""Tests for ATLAS-A30 private exp-negation theorem-lookup gate."""
...
from scripts.atlas_a30_private_exp_negation_theorem_lookup_gate import (
    CLAIM_FLAGS,
...
def test_atlas_a30_consumes_a29_and_records_lookup_gate():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    validate_payload(payload)
    summary = payload["summary"]
    assert payload["status"] == "ATLAS_A30_PRIVATE_EXP_NEGATION_THEOREM_LOOKUP_GATE_PASS"
    assert payload["sourceArtifact"] == "atlas-a29-private-exp-negation-proof-scope-feasibility-packet"
...
def test_atlas_a30_scopes_lookup_to_pure_exp_and_keeps_eml_deferred():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
    ...
    assert summary["pureExpStatementLookupScoped"] is True
    assert summary["emlCompanionKeptDeferred"] is True
    assert summary["deferredCompanionStatement"] == "eml (x + (-x)) 1 = 1"
    assert review["lookupScope"]["scopeStatus"] == "pure_exp_statement_only_eml_companion_deferred"


def test_atlas_a30_records_observed_identifier_candidates():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
    review = payload["theoremLookupReview"]
    ...
    assert identifiers == [
        "MachLib.Real.exp_mul_exp_neg",
        "MachLib.Real.exp_neg_self_mul",
        "MachLib.Real.exp_add",
    ]
...
def test_atlas_a30_records_lookup_method_and_blockers():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    review = payload["theoremLookupReview"]
...
def test_atlas_a30_creates_no_dependency_proof_or_validity_claims():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
...
def test_atlas_a30_blocks_edit_lean_public_runtime_and_product_claims():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
...
def test_atlas_a30_preserves_target_accounting():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
...
def test_atlas_a30_claim_flags_stay_bounded():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
...
def test_atlas_a30_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "reports",
        tmp_path / "evidence",
    ...
    assert report.startswith("# ATLAS-A30 Private Exp-Negation Theorem-Lookup Gate")
...
def test_atlas_a30_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/atlas_a30_private_exp_negation_theorem_lookup_gate.py",
            "--build",
            "--atlas-gate-path",
            str(ATLAS_GATE),
            "--machlib-root",
    ...
    assert "ATLAS_A30_PRIVATE_EXP_NEGATION_THEOREM_LOOKUP_GATE_OK" in proc.stdout
