"""Tests for FEF-P55 Rust if expression branch lowering gate."""
...
from scripts.fef_p55_rust_if_expression_branch_lowering_gate import (
    CLAIM_FLAGS,
    build_outputs,
    build_payload,
    validate_payload,
...
def test_fef_p55_records_selected_rust_if_expression_gate():
    payload = build_payload()
    validate_payload(payload)
    summary = payload["summary"]
    assert payload["status"] == "FEF_P55_RUST_IF_EXPRESSION_BRANCH_LOWERING_GATE_PASS"
    assert payload["decision"] == (
        "selected_rust_if_expression_lowering_reingest_passed_general_branch_blocked"
    )
    assert summary["selectedRustIfExpressionLoweringPass"] is True
    assert summary["rustIfExpressionBlockerClosed"] is True
    assert summary["sourceCaseCount"] == 1
    assert summary["packetCount"] == 2
    assert summary["passCount"] == 2
    assert summary["packetSampleCount"] == 10


def test_fef_p55_source_fixture_shape_is_guarded_selector():
    payload = build_payload()
    fixture = payload["sourceFixture"]
    assert fixture["sourceLanguage"] == "rust"
    assert fixture["feature"] == "if_expression"
    assert fixture["loweringForm"] == "affine_selector_with_step01_guard"
    assert {"fn step01", "clamp(", "step01(", "relu"} <= set(
        fixture["sourceEmlContains"]
    )


def test_fef_p55_packets_compare_generated_runtime_to_reingested_python():
    payload = build_payload()
    summary = payload["summary"]
    ...
    for packet in payload["reingestPackets"]:
        assert packet["sourceLanguage"] == "rust"
        assert packet["sourceFeature"] == "if_expression"
        ...
        assert set(packet["frames"][0]["values"]) == {
            ...
            "expectedRustIfExpressionReference",
        }


def test_fef_p55_claim_boundaries_and_gates_remain_blocked():
    payload = build_payload()
    summary = payload["summary"]
    gates = {gate["id"]: gate["status"] for gate in payload["releaseGates"]}
    assert gates["selected_rust_if_expression_lowering"] == "pass"
    assert gates["selected_rust_if_expression_generated_c_reingest"] == "pass"
    assert gates["selected_rust_if_expression_generated_rust_reingest"] == "pass"
...
def test_fef_p55_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "packets",
        tmp_path / "reports",
    ...
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# FEF-P55")


def test_fef_p55_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/fef_p55_rust_if_expression_branch_lowering_gate.py",
            "--build",
            "--out-dir",
    ...
    assert "FEF_P55_RUST_IF_EXPRESSION_BRANCH_LOWERING_GATE_OK" in proc.stdout
