"""Tests for ATLAS-A31 private exp-negation witness-wrapper readiness selector."""
...
from scripts.atlas_a31_private_exp_negation_witness_wrapper_readiness_selector import (
    CLAIM_FLAGS,
...
def test_atlas_a31_consumes_a30_and_recommends_future_gate():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    validate_payload(payload)
    summary = payload["summary"]
    assert payload["status"] == "ATLAS_A31_PRIVATE_EXP_NEGATION_WITNESS_WRAPPER_READINESS_SELECTOR_PASS"
    assert payload["sourceArtifact"] == "atlas-a30-private-exp-negation-theorem-lookup-gate"
...
def test_atlas_a31_reviews_observed_identifier_and_scope():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
    ...
    assert summary["lookupScopeStatement"] == "forall x : Real, Real.exp x * Real.exp (-x) = 1"
    ...
    assert selected["sourceSignals"]["primaryObservedIdentifier"] == "MachLib.Real.exp_mul_exp_neg"


def test_atlas_a31_records_options():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    options = {item["optionId"]: item for item in payload["options"]}
    assert set(options) == {
        SELECTED_DECISION_ID,
        "park_exp_negation_after_lookup",
        "request_human_scope_review",
    }
    assert options[SELECTED_DECISION_ID]["selectionStatus"] == "selected_next"
    assert options["park_exp_negation_after_lookup"]["selectionStatus"] == "available_if_reviewer_prefers_atlas_pause"
...
def test_atlas_a31_creates_no_wrapper_dependency_proof_or_validity_claims():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
...
def test_atlas_a31_blocks_edit_lean_public_runtime_and_product_claims():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
...
def test_atlas_a31_preserves_target_accounting():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
    summary = payload["summary"]
...
def test_atlas_a31_claim_flags_stay_bounded():
    payload = build_payload(ATLAS_GATE, MACHLIB_ROOT)
...
def test_atlas_a31_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "reports",
        tmp_path / "evidence",
    ...
    assert report.startswith("# ATLAS-A31 Private Exp-Negation Witness-Wrapper Readiness Selector")
    assert "## Readiness Reasons" in report
    assert "## Future Gate Requirements" in report
    assert "## Options" in report


def test_atlas_a31_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/atlas_a31_private_exp_negation_witness_wrapper_readiness_selector.py",
            "--build",
            "--atlas-gate-path",
            str(ATLAS_GATE),
            "--machlib-root",
    ...
    assert "ATLAS_A31_PRIVATE_EXP_NEGATION_WITNESS_WRAPPER_READINESS_SELECTOR_OK" in proc.stdout
