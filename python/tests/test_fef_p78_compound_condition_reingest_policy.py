"""Tests for FEF-P78 compound-condition re-ingest policy."""
...
from scripts.fef_p78_compound_condition_reingest_policy import (
    CLAIM_FLAGS,
    build_outputs,
    build_payload,
...
def test_fef_p78_records_selected_reingest_policy_without_execution():
    payload = build_payload()
    validate_payload(payload)
    summary = payload["summary"]
    assert payload["status"] == "FEF_P78_COMPOUND_CONDITION_REINGEST_POLICY_PASS"
    assert payload["decision"] == "selected_compound_condition_reingest_policy_recorded_execution_blocked"
    ...
    assert summary["policyScope"] == "selected_generated_c_fixture_only"
    assert summary["reingestPolicyRecorded"] is True
    assert summary["reingestExecuted"] is False


def test_fef_p78_policy_accepts_only_selected_generated_surface():
    payload = build_payload()
    ...
    assert accepted_ids == [
        "static_helper_step01",
        "static_helper_nonzero01",
        "static_helper_guarded_div",
        "selected_if_guard_shape",
        "selected_return_shape",
    ]
    assert rejected_ids == [
        "arbitrary_boolean_expression",
        "side_effect_condition",
        "nested_condition_tree",
        "helper_runtime_import",
    ]


def test_fef_p78_policy_validation_rows_match_codegen_tokens():
    payload = build_payload()
...
def test_fef_p78_required_comparison_rows_are_pending_reingest():
    payload = build_payload()
...
def test_fef_p78_policy_builder_is_deterministic_from_p77_payload():
    payload = build_payload()
    ...
    assert policy["selectedFixtureId"] == "c_and_short_circuit_guard_v0"
    assert len(rows) == 9


def test_fef_p78_boundaries_and_claim_flags_remain_false():
    payload = build_payload()
    summary = payload["summary"]
    gates = {gate["id"]: gate["status"] for gate in payload["releaseGates"]}
    assert gates["selected_reingest_policy"] == "recorded_execution_blocked"
    assert gates["selected_reingest_execution"] == "not_performed"
...
def test_fef_p78_writes_outputs(tmp_path):
    built = build_outputs(tmp_path / "results", tmp_path / "reports", tmp_path / "evidence", tmp_path / "feeds")
    for key in ["result_path", "evidence_path", "feed_path"]:
        json.loads(Path(built[key]).read_text(encoding="utf-8"))
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# FEF-P78")


def test_fef_p78_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/fef_p78_compound_condition_reingest_policy.py",
            "--build",
            "--out-dir",
    ...
    assert "FEF_P78_COMPOUND_CONDITION_REINGEST_POLICY_OK" in proc.stdout
