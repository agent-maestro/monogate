"""Tests for GB-VIS-A12 private adapter intake feed guard."""
...
from scripts.gb_vis_a12_private_adapter_intake_feed_guard import (
...
def test_gb_vis_a12_consumes_gb_vis_a11_and_rebuilds_feed():
    payload = build_payload(ATLAS_GATE)
    validate_payload(payload)
    assert payload["status"] == "GB_VIS_A12_PRIVATE_ADAPTER_INTAKE_FEED_GUARD_PASS"
    assert payload["sourceAdapterIntakeSnapshot"] == "gb-vis-a11-private-adapter-intake-snapshot"
...
def test_gb_vis_a12_feed_guard_rows_pass_exactly():
    payload = build_payload(ATLAS_GATE)
    rows = {row["guardRowId"]: row for row in payload["feedGuardRows"]}
    assert set(rows) == {
        "gb_vis_a12_feed_guard:feed_id",
        "gb_vis_a12_feed_guard:status",
        "gb_vis_a12_feed_guard:decision",
        "gb_vis_a12_feed_guard:next_action_is_private",
        "gb_vis_a12_feed_guard:allowed_true_claim_flags",
        "gb_vis_a12_feed_guard:blocked_claim_flags_false",
    }
...
def test_gb_vis_a12_source_claim_flags_are_guarded():
    payload = build_payload(ATLAS_GATE)
...
def test_gb_vis_a12_records_no_renderer_or_public_claim():
    payload = build_payload(ATLAS_GATE)
    assert payload["summary"]["privateAdapterIntakeFeedGuardRecorded"] is True
    assert payload["summary"]["gbVisA11AdapterIntakeSnapshotConsumed"] is True
...
def test_gb_vis_a12_claim_flags_are_bounded():
    payload = build_payload(ATLAS_GATE)
...
def test_gb_vis_a12_next_action_stays_private():
    payload = build_payload(ATLAS_GATE)
...
def test_gb_vis_a12_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "reports",
        tmp_path / "evidence",
    ...
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# GB-VIS-A12")


def test_gb_vis_a12_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/gb_vis_a12_private_adapter_intake_feed_guard.py",
            "--build",
            "--atlas-gate-path",
            str(ATLAS_GATE),
            "--out-dir",
    ...
    assert "GB_VIS_A12_PRIVATE_ADAPTER_INTAKE_FEED_GUARD_OK" in proc.stdout
