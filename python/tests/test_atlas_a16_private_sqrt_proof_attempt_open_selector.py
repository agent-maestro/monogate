"""Tests for ATLAS-A16 private sqrt proof-attempt open selector."""
...
from scripts.atlas_a16_private_sqrt_proof_attempt_open_selector import (
    ATTEMPT_PACKET_ID,
    CANDIDATE_ID,
    CLAIM_FLAGS,
    ROOT,
...
def test_atlas_a16_consumes_a15_and_recommends_bounded_attempt_artifact():
    payload = build_payload(ATLAS_GATE)
    validate_payload(payload)
    summary = payload["summary"]
    assert payload["status"] == "ATLAS_A16_PRIVATE_SQRT_PROOF_ATTEMPT_OPEN_SELECTOR_PASS"
    assert payload["sourceArtifact"] == "atlas-a15-private-scoped-sqrt-proof-attempt-packet"
    assert summary["sourceAttemptPacketId"] == ATTEMPT_PACKET_ID
    assert summary["candidateId"] == CANDIDATE_ID
    assert summary["selectedOptionId"] == SELECTED_OPTION_ID
    assert summary["selectedDecision"] == "recommend_bounded_attempt_artifact_without_starting_attempt"
    assert summary["nextRecommendedArtifact"] == "ATLAS-A17 private bounded sqrt proof-attempt artifact"


def test_atlas_a16_reviews_scope_route_budget_and_outputs():
    payload = build_payload(ATLAS_GATE)
    summary = payload["summary"]
    assert summary["scopedAttemptPacketReviewed"] is True
    assert summary["attemptScopeReviewed"] is True
    assert summary["attemptRouteReviewed"] is True
    assert summary["attemptBudgetReviewed"] is True
    ...
    assert summary["expectedFutureOutputCount"] == 4


def test_atlas_a16_opens_next_artifact_but_starts_no_attempt():
    payload = build_payload(ATLAS_GATE)
    summary = payload["summary"]
    assert summary["proofAttemptOpenSelectorCreated"] is True
    assert summary["futureBoundedAttemptArtifactRecommended"] is True
    assert summary["boundedProofAttemptArtifactCreated"] is False
...
def test_atlas_a16_options_keep_pause_and_parking_available():
    payload = build_payload(ATLAS_GATE)
...
def test_atlas_a16_preserves_runtime_public_and_product_blocks():
    payload = build_payload(ATLAS_GATE)
    summary = payload["summary"]
...
def test_atlas_a16_preserves_target_gap():
    payload = build_payload(ATLAS_GATE)
    summary = payload["summary"]
...
def test_atlas_a16_claim_flags_stay_bounded():
    payload = build_payload(ATLAS_GATE)
...
def test_atlas_a16_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "reports",
        tmp_path / "evidence",
    ...
    assert report.startswith("# ATLAS-A16 Private Sqrt Proof-Attempt Open Selector")
    assert "## Open Rationale" in report
    assert "## Strict Future Attempt Limits" in report
    assert "## Options" in report
    assert "## Remaining Blocks" in report


def test_atlas_a16_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/atlas_a16_private_sqrt_proof_attempt_open_selector.py",
            "--build",
            "--atlas-gate-path",
            str(ATLAS_GATE),
            "--out-dir",
    ...
    assert "ATLAS_A16_PRIVATE_SQRT_PROOF_ATTEMPT_OPEN_SELECTOR_OK" in proc.stdout
