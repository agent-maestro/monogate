"""Tests for ACT-A15 reviewer promotion hold feed guard."""
...
from scripts.act_a15_reviewer_promotion_hold_feed_guard import (
...
def test_act_a15_consumes_act_a14_and_rebuilds_feed():
    payload = build_payload(ATLAS_GATE)
    validate_payload(payload)
    assert payload["status"] == "ACT_A15_REVIEWER_PROMOTION_HOLD_FEED_GUARD_PASS"
    assert payload["sourceReviewerPromotionHoldSnapshot"] == "act-a14-reviewer-promotion-hold-snapshot"
...
def test_act_a15_feed_guard_rows_pass_exactly():
    payload = build_payload(ATLAS_GATE)
    rows = {row["guardRowId"]: row for row in payload["feedGuardRows"]}
    assert set(rows) == {
        "act_a15_feed_guard:feed_id",
        "act_a15_feed_guard:status",
        "act_a15_feed_guard:decision",
        "act_a15_feed_guard:next_action_is_private",
        "act_a15_feed_guard:allowed_true_claim_flags",
        "act_a15_feed_guard:blocked_claim_flags_false",
    }
...
def test_act_a15_source_claim_flags_are_guarded():
    payload = build_payload(ATLAS_GATE)
...
def test_act_a15_records_no_acceptance_soundness_or_public_claim():
    payload = build_payload(ATLAS_GATE)
    assert payload["summary"]["reviewerPromotionHoldFeedGuardRecorded"] is True
    assert payload["summary"]["actA14ReviewerPromotionHoldSnapshotConsumed"] is True
    ...
    assert payload["summary"]["concreteArtifactAccepted"] is False
...
def test_act_a15_claim_flags_are_bounded():
    payload = build_payload(ATLAS_GATE)
...
def test_act_a15_next_action_stays_private():
    payload = build_payload(ATLAS_GATE)
...
def test_act_a15_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "reports",
        tmp_path / "evidence",
    ...
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# ACT-A15")


def test_act_a15_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/act_a15_reviewer_promotion_hold_feed_guard.py",
            "--build",
            "--atlas-gate-path",
            str(ATLAS_GATE),
            "--out-dir",
    ...
    assert "ACT_A15_REVIEWER_PROMOTION_HOLD_FEED_GUARD_OK" in proc.stdout
