"""Tests for ACT-A9 reviewer report feed guard."""
...
from scripts.act_a9_reviewer_report_feed_guard import (
...
def test_act_a9_consumes_act_a8_and_rebuilds_feed():
    payload = build_payload(ATLAS_GATE)
    validate_payload(payload)
    assert payload["status"] == "ACT_A9_REVIEWER_REPORT_FEED_GUARD_PASS"
    assert payload["sourceReviewerReportSnapshot"] == "act-a8-reviewer-report-snapshot"
...
def test_act_a9_feed_guard_rows_pass_exactly():
    payload = build_payload(ATLAS_GATE)
    rows = {row["guardRowId"]: row for row in payload["feedGuardRows"]}
    assert set(rows) == {
        "act_a9_feed_guard:feed_id",
        "act_a9_feed_guard:status",
        "act_a9_feed_guard:decision",
        "act_a9_feed_guard:next_action_is_private",
        "act_a9_feed_guard:allowed_true_claim_flags",
        "act_a9_feed_guard:blocked_claim_flags_false",
    }
...
def test_act_a9_source_claim_flags_are_guarded():
    payload = build_payload(ATLAS_GATE)
...
def test_act_a9_records_no_production_validator_or_soundness_claim():
    payload = build_payload(ATLAS_GATE)
    assert payload["summary"]["reviewerReportFeedGuardRecorded"] is True
    assert payload["summary"]["actA8ReviewerReportSnapshotConsumed"] is True
...
def test_act_a9_claim_flags_are_bounded():
    payload = build_payload(ATLAS_GATE)
...
def test_act_a9_next_action_stays_private():
    payload = build_payload(ATLAS_GATE)
...
def test_act_a9_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "reports",
        tmp_path / "evidence",
    ...
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# ACT-A9")


def test_act_a9_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/act_a9_reviewer_report_feed_guard.py",
            "--build",
            "--atlas-gate-path",
            str(ATLAS_GATE),
            "--out-dir",
    ...
    assert "ACT_A9_REVIEWER_REPORT_FEED_GUARD_OK" in proc.stdout
