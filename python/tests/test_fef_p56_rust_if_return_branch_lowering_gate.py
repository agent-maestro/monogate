"""Tests for FEF-P56 Rust if-return branch lowering gate."""
...
from scripts.fef_p56_rust_if_return_branch_lowering_gate import (
    CLAIM_FLAGS,
    build_outputs,
    build_payload,
    validate_payload,
...
def test_fef_p56_records_selected_rust_if_return_clamp_gate():
    payload = build_payload()
    validate_payload(payload)
    summary = payload["summary"]
    assert payload["status"] == "FEF_P56_RUST_IF_RETURN_BRANCH_LOWERING_GATE_PASS"
    assert payload["decision"] == (
        "selected_rust_if_return_clamp_lowering_reingest_passed_general_branch_blocked"
    )
    assert summary["selectedRustIfReturnLoweringPass"] is True
    assert summary["rustIfReturnBlockerClosed"] is True
...
def test_fef_p56_source_fixture_shape_is_guarded_selector():
    payload = build_payload()
    fixture = payload["sourceFixture"]
    assert fixture["sourceLanguage"] == "rust"
    assert fixture["feature"] == "if_return_clamp"
...
def test_fef_p56_packets_compare_generated_runtime_to_reingested_python():
    payload = build_payload()
    summary = payload["summary"]
    ...
    for packet in payload["reingestPackets"]:
        ...
        assert packet["sourceFeature"] == "if_return_clamp"
...
def test_fef_p56_claim_boundaries_and_gates_remain_blocked():
    payload = build_payload()
    summary = payload["summary"]
    gates = {gate["id"]: gate["status"] for gate in payload["releaseGates"]}
    assert gates["selected_rust_if_return_clamp_lowering"] == "pass"
    assert gates["selected_rust_if_return_clamp_generated_c_reingest"] == "pass"
    assert gates["selected_rust_if_return_clamp_generated_rust_reingest"] == "pass"
...
def test_fef_p56_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "packets",
        tmp_path / "reports",
    ...
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# FEF-P56")


def test_fef_p56_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/fef_p56_rust_if_return_branch_lowering_gate.py",
            "--build",
            "--out-dir",
    ...
    assert "FEF_P56_RUST_IF_RETURN_BRANCH_LOWERING_GATE_OK" in proc.stdout
