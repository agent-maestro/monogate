"""Tests for FEF-P31 free-target emission guard."""
...
from scripts.fef_p31_free_target_emission_guard import (
    CLAIM_FLAGS,
    build_outputs,
    build_payload,
    validate_payload,
...
def test_fef_p31_all_free_targets_emit_and_validate():
    payload = build_payload()
    validate_payload(payload)
    summary = payload["summary"]
    assert payload["status"] == "FEF_P31_FREE_TARGET_EMISSION_GUARD_PASS"
...
def test_fef_p31_expected_free_target_set():
    payload = build_payload()
...
def test_fef_p31_records_validation_tiers():
    payload = build_payload()
...
def test_fef_p31_keeps_broad_claims_false():
    payload = build_payload()
    summary = payload["summary"]
...
def test_fef_p31_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "reports",
        tmp_path / "evidence",
    ...
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# FEF-P31")


def test_fef_p31_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/fef_p31_free_target_emission_guard.py",
            "--build",
            "--out-dir",
    ...
    assert "FEF_P31_FREE_TARGET_EMISSION_GUARD_OK" in proc.stdout
