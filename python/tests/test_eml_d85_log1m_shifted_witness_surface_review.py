"""Tests for EML-D85 log1m shifted witness surface review."""
...
from scripts.eml_d85_log1m_shifted_witness_surface_review import (
    CLAIM_FLAGS,
    ROOT,
    build_outputs,
    build_payload,
...
def test_d85_consumes_d84_witness_attempt():
    payload = build_payload(ATLAS_GATE)
    validate_payload(payload)
    assert payload["status"] == "EML_D85_LOG1M_SHIFTED_WITNESS_SURFACE_REVIEW_PASS"
    assert payload["sourceWitnessAttempt"] == "eml-d84-log1m-shifted-boundary-coordinate-witness-attempt"
    assert payload["summary"]["selectedWitnessName"] == "MachLib.Real.log1m_shifted_boundary_coordinate_witness"


def test_d85_preserves_checked_statement_guard_runtime_and_duplicate_block():
    payload = build_payload(ATLAS_GATE)
    ...
    assert payload["summary"]["sourceDuplicateLog1pBlockPreserved"] is True
    ...
    runtime = row_by_id(payload, "log1m_shifted_runtime_control_guardrail")
...
def test_d85_records_five_private_surface_rows():
    payload = build_payload(ATLAS_GATE)
    assert payload["summary"]["surfaceRowCount"] == 5
    assert row_by_id(payload, "machlib_witness_index_log1m_shifted_boundary")["surfaceKind"] == "machlib_private_index"
    assert row_by_id(payload, "log1m_shifted_guard_boundary")["surfaceKind"] == "candidate_boundary"
    assert row_by_id(payload, "log1m_shifted_runtime_control_guardrail")["surfaceKind"] == "runtime_control_guardrail"
    assert row_by_id(payload, "advantage_lab_log1m_shifted_boundary")["surfaceKind"] == "advantage_lab"
    assert row_by_id(payload, "public_atlas_log1m_shifted_boundary")["surfaceKind"] == "public_surface"


def test_d85_preserves_d83_negative_controls_and_blockers():
    payload = build_payload(ATLAS_GATE)
    boundary = row_by_id(payload, "log1m_shifted_guard_boundary")
    assert payload["summary"]["sourceNegativeControlCount"] == 4
    assert payload["summary"]["sourceBlockerCount"] == 4
    assert boundary["surfaceStatus"] == "shifted_positive_domain_boundary_required"
    assert "x = 1 boundary" in boundary["blockedClaims"]
    assert "x > 1 boundary" in boundary["blockedClaims"]
    assert "unguarded statements remain blocked" in " ".join(boundary["rationale"])


def test_d85_records_checked_witness_privately_without_new_proof_work():
    payload = build_payload(ATLAS_GATE)
...
def test_d85_keeps_public_reviewer_advantage_runtime_and_laptop_claims_blocked():
    payload = build_payload(ATLAS_GATE)
...
def test_d85_claim_flags_are_all_false():
    payload = build_payload(ATLAS_GATE)
...
def test_d85_points_to_next_selector():
    payload = build_payload(ATLAS_GATE)
    assert payload["summary"]["nextAction"] == (
        "EML-D86 choose log1m-shifted checked-witness copy review, next bounded branch, or human-approved public copy gate without public promotion."
    )


def test_d85_writes_outputs(tmp_path):
    built = build_outputs(
        tmp_path / "results",
        tmp_path / "reports",
        tmp_path / "evidence",
    ...
    assert Path(built["report_path"]).read_text(encoding="utf-8").startswith("# EML-D85")


def test_d85_cli_build_strict(tmp_path):
    proc = subprocess.run(
        [
            sys.executable,
            "python/scripts/eml_d85_log1m_shifted_witness_surface_review.py",
            "--build",
            "--atlas-gate-path",
            str(ATLAS_GATE),
            "--out-dir",
    ...
    assert "EML_D85_LOG1M_SHIFTED_WITNESS_SURFACE_REVIEW_OK" in proc.stdout
